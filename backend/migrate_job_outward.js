...
const queries = [
    `CREATE TABLE IF NOT EXISTS job_outward (
    ...
    `CREATE TABLE IF NOT EXISTS job_outward_items (
        id INT AUTO_INCREMENT PRIMARY KEY,
        outward_id INT,
        inward_item_id INT,
        ...
        FOREIGN KEY (outward_id) REFERENCES job_outward(id) ON DELETE CASCADE,
        FOREIGN KEY (inward_item_id) REFERENCES job_inward_items(id) ON DELETE SET NULL
    )`,
    `CREATE TABLE IF NOT EXISTS job_outward_item_processes (
        id INT AUTO_INCREMENT PRIMARY KEY,
        outward_item_id INT,
        process_name VARCHAR(255),
        machine_name VARCHAR(255),
        FOREIGN KEY (outward_item_id) REFERENCES job_outward_items(id) ON DELETE CASCADE
    )`
];
