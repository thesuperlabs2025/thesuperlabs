...
router.get("/purchase/:id", async (req, res) => {
    ...
    try {
        const purchase = await new Promise((resolve, reject) => {
            db.query("SELECT * FROM purchases WHERE id = ?", [id], (e, r) =>
        ...
        const discount = Number(purchase.discount_total || 0);
        ...
        const roundedTotal = Math.round(purchase.grand_total);
        const roundOff = roundedTotal - purchase.grand_total;
...
`;
        ...
        res.setHeader("Content-Disposition", `inline; filename=purchase-${id}.pdf`);
        res.send(pdf);

    } catch (err) {
        console.error(err);
        res.status(500).send("Purchase print failed");
    }
});
