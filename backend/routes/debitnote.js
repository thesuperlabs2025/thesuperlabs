...
router.get("/next-debit-note-no", async (req, res) => {
...
router.post("/", async (req, res) => {
  ...
  const finaldebit_noteDate = fromDMYtoYMD(debit_note_date);
  const finalDueDate = fromDMYtoYMD(due_date);

  if (!items || items.length === 0) {
    return res.status(400).json({ error: "No debit note items provided" });
  }
  ...
  try {
    ...
    if (!templateResult.length) {
      return res.status(400).json({ error: "Debit note template not found" });
    }
    ...
    const debitnoteSql = `
      ...
      (customer_name, ship_to, mobile, sales_person, debit_note_date, due_date, billing_address, total_qty, discount_total, grand_total, year_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;

    const debitnoteResult = await new Promise((resolve, reject) => {
      db.query(
        debitnoteSql,
        [
          customer_name,
    ...
    const debitnote_id = debitnoteResult.insertId;

    // Insert items
    const itemSql = `
      INSERT INTO debit_note_items
      (debit_note_id, sku, qty, rate, disc_val, disc_percent, gst_percent, total)
      VALUES ?
    `;
    ...
    res.json({ success: true, message: "Debit note saved successfully!" });
  } catch (err) {
...
router.get("/:id", (req, res) => {
  ...
  const sql = `
    SELECT 
      ...
      debit_note.sales_person,
      debit_note.debit_note_date,
      debit_note.mobile,
      debit_note.due_date,
      debit_note.discount_total,
       debit_note.gst_total,
      debit_note.net_total,
      debit_note.grand_total,
     debit_note.created_at,
      debit_note.round_off,
      debit_note.igst,
      debit_note.ship_to,
     debit_note.billing_address,
      debit_note.total_qty,

      debit_note_items.sku,
      debit_note_items.qty,
      debit_note_items.rate,
     debit_note_items.disc_val,
      debit_note_items.disc_percent,
      debit_note_items.gst_percent,
      debit_note_items.total AS item_total

    FROM debit_note
    LEFT JOIN debit_note_items
      ON debit_note.id = debit_note_items.debit_note_id
    WHERE debit_note.id = ?
  `;
  ...
  db.query(sql, [id], (err, rows) => {
    ...
    if (rows.length === 0)
      return res.json({ message: "debit note not found" });
    ...
    const header = {
      ...
      debit_note_date: formatDateDMY(rows[0].debit_note_date),
...
router.put("/debitnote/:id", (req, res) => {
  const { id } = req.params;
  const debitnote = req.body;

  const totalQty = debitnote.items.reduce((sum, row) => sum + Number(row.qty || 0), 0);

  const sql = `
    UPDATE debit_note
    SET customer_id=?, debit_note_date=?, total_qty=?, total_amount=? 
    WHERE id=?
  `;
  ...
  db.query(sql, data, (err, result) => {
    if (err) return res.status(500).json({ error: err });
    res.json({ message: "Debit note updated successfully!", totalQty });
  });
});
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    const oldItems = await new Promise((resolve, reject) => {
      db.query(
        "SELECT sku, qty FROM debit_note_items WHERE debit_note_id = ?",
        [id],
        (err, results) => {
          if (err) return reject(err);
          resolve(results || []);
    ...
    res.json({ success: true, message: "debit note updated successfully!" });
  } catch (err) {
...
router.get("/", (req, res) => {
  try {
    ...
    // ✅ same validation as Credit Note / Estimate
    if (!usertype_id) {
    ...
    if (debit_note_date) {
      where.push("debit_note_date = ?");
      params.push(debit_note_date);
    }
    if (due_date) {
      where.push("due_date = ?");
    ...
    // 1️⃣ Privilege fetch (SAME PATTERN AS CREDIT NOTE)
    const privQ = `
      SELECT 
        p.can_add,
        p.can_update,
    ...
    db.query(privQ, [usertype_id], (perr, privRows) => {
      ...
      // 2️⃣ Debit Note list (SQL SAME)
      const sql = `
        SELECT
          id,
          customer_name,
          DATE_FORMAT(debit_note_date,'%d-%m-%Y') AS debit_note_date,
          total_qty,
          sales_person,
          grand_total,
          mobile,
      ...
      db.query(sql, [...params, limit, offset], (err, rows) => {
        if (err) {
          ...
          return res.status(500).json({ message: "Debit Note DB Error" });
        }

        // count (UNCHANGED)
        db.query(
          `SELECT COUNT(*) AS total FROM debit_note ${whereSql}`,
          params,
          (err2, count) => {
            if (err2) {
              console.error("Debit Note count error:", err2);
              return res.status(500).json({ message: "Debit Note DB Error" });
            }
            ...
            // ✅ FINAL RESPONSE (MATCHES CREDIT NOTE / ESTIMATE)
            res.json({
              data: rows,
              privileges: debitNotePriv,
              pagination: {
                page,
                limit,
                total,
    ...
    console.error("Unexpected error in GET /debit_note:", e);
...
//       whereParts.push("debit_note_date = ?");
//       params.push(debit_note_date);
//     }
//     if (due_date) {
...
//       // total count
//       const countSql = `SELECT COUNT(*) AS total FROM debit_note ${whereClause}`;
...
//     });
//   } catch (e) {
//     console.error("Unexpected error in GET /debit note:", e);
//     res.status(500).json({ error: "Unexpected server error" });
//   }
// });
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    await queryPromise("DELETE FROM debit_note_items WHERE debit_note_id = ?", [id]);

    // 4️⃣ Delete invoice
    await queryPromise("DELETE FROM debit_note WHERE id = ?", [id]);

    res.json({ success: true, message: "debit note deleted & stock restored." });
  } catch (err) {
    console.error("DELETE /debit_note/:id error:", err);
