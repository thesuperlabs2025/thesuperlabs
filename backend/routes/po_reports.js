...
router.get("/ledger", (req, res) => {
    ...
    let sql = `
        SELECT 
            ...
            p.grand_total,
            p.po_date as tx_date,
            p.sales_person
        FROM po p
        LEFT JOIN supplier s ON p.customer_name = s.name
        LEFT JOIN po_items it ON p.id = it.po_id
        WHERE 1=1
    `;
    const params = [];
    if (customer_name) { sql += " AND p.customer_name LIKE ?"; params.push(`%${customer_name}%`); }
    if (from_date && to_date) { sql += " AND p.po_date BETWEEN ? AND ?"; params.push(from_date, to_date); }
    sql += " GROUP BY p.id ORDER BY p.po_date DESC";
...
router.get("/summary", (req, res) => {
    ...
    let sql = `
        ...
        FROM po p
        JOIN po_items it ON p.id = it.po_id
        WHERE 1=1
    `;
    const params = [];
    if (customer_name) { sql += " AND p.customer_name LIKE ?"; params.push(`%${customer_name}%`); }
