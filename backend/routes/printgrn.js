...
router.get("/grn/:id", async (req, res) => {
    ...
    try {
        const grn = await new Promise((resolve, reject) => {
            db.query("SELECT * FROM grn WHERE id = ?", [id], (e, r) =>
                e ? reject(e) : resolve(r[0])
            );
        });
        if (!grn) return res.status(404).send("GRN not found");
        ...
        const discount = Number(grn.discount_total || 0);
        ...
        const roundedTotal = Math.round(grn.grand_total);
        const roundOff = roundedTotal - grn.grand_total;
...
`;
        ...
        res.setHeader("Content-Disposition", `inline; filename=grn-${id}.pdf`);
        res.send(pdf);

    } catch (err) {
        console.error(err);
        res.status(500).send("GRN print failed");
    }
});
