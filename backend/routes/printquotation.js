...
router.get("/quotation/:id", async (req, res) => {
    ...
    try {
        const quotation = await new Promise((resolve, reject) => {
            db.query("SELECT * FROM quotation WHERE id = ?", [id], (e, r) =>
        ...
        const discount = Number(quotation.discount_total || 0);
        ...
        const roundedTotal = Math.round(quotation.grand_total);
        const roundOff = roundedTotal - quotation.grand_total;
...
`;
        ...
        console.error(err);
        res.status(500).send("Quotation print failed");
    }
});
