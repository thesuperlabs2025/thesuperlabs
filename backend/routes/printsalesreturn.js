...
router.get("/salesreturn/:id", async (req, res) => {
    ...
    try {
        const sales_return = await new Promise((resolve, reject) => {
            db.query("SELECT * FROM sales_return WHERE id = ?", [id], (e, r) =>
                e ? reject(e) : resolve(r[0])
            );
        });
        if (!sales_return) return res.status(404).send("Sales Return not found");
        ...
        const discount = Number(sales_return.discount_total || 0);
        ...
        const roundedTotal = Math.round(sales_return.grand_total);
        const roundOff = roundedTotal - sales_return.grand_total;
...
<div class="header">
  ...
  <div class="invoice-title">
    SALES RETURN<br>
    <span class="small">
      Return No: ${sales_return.id}<br>
      Date: ${formatDate(sales_return.sales_return_date)}
    </span>
  </div>
</div>
...
`;
        ...
        res.setHeader("Content-Disposition", `inline; filename=salesreturn-${id}.pdf`);
        res.send(pdf);

    } catch (err) {
        console.error(err);
        res.status(500).send("Sales Return print failed");
    }
});
