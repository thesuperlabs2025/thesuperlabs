...
router.get("/purchasereturn/:id", async (req, res) => {
    const { id } = req.params;

    try {
        const purchase_return = await new Promise((resolve, reject) => {
            db.query("SELECT * FROM purchase_return WHERE id = ?", [id], (e, r) =>
                e ? reject(e) : resolve(r[0])
            );
        });
        if (!purchase_return) return res.status(404).send("Purchase Return not found");
        ...
        const discount = Number(purchase_return.discount_total || 0);
        ...
        const roundedTotal = Math.round(purchase_return.grand_total);
        const roundOff = roundedTotal - purchase_return.grand_total;
...
<div class="header">
  ...
  <div class="invoice-title">
    PURCHASE RETURN<br>
    <span class="small">
      Return No: ${purchase_return.id}<br>
      Date: ${formatDate(purchase_return.purchase_return_date)}
    </span>
  </div>
</div>
...
<div class="bill-ship small">
  <div class="col">
    <b>Bill To</b><br>
    ${purchase_return.customer_name}<br>
    ${customer?.mobile || ""}<br>
...
`;
        ...
        res.setHeader("Content-Disposition", `inline; filename=purchasereturn-${id}.pdf`);
        res.send(pdf);

    } catch (err) {
        console.error(err);
        res.status(500).send("Purchase Return print failed");
    }
});
