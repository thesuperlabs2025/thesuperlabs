...
router.get("/next-grn-no", (req, res) => {
    const yearId = req.headers['x-year-id'];
    const query = "SELECT MAX(id) as maxId FROM grn WHERE year_id = ?";
    db.query(query, [yearId], (err, data) => {
        if (err) return res.status(500).json(err);
        const nextId = (data[0].maxId || 0) + 1;
        res.status(200).json({ grnNo: nextId });
    });
});
...
router.post("/", async (req, res) => {
  ...
  const finalGrnDate = fromDMYtoYMD(grn_date);
  const finalDueDate = fromDMYtoYMD(due_date);

  if (!items || items.length === 0) {
    return res.status(400).json({ error: "No grn items provided" });
  }
  ...
  try {
    ...
    if (!templateResult.length) {
      return res.status(400).json({ error: "grn template not found" });
    }
    ...
    const grnSql = `
      INSERT INTO grn
      (customer_name, ship_to, mobile, sales_person, grn_date, due_date, billing_address, total_qty, discount_total, grand_total, year_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    const itemSql = `
      ...
      (grn_id, sku, qty, rate, disc_val, disc_percent, gst_percent, total)
      VALUES ?
    `;
...
router.get("/:id", (req, res) => {
  ...
  const sql = `
    SELECT 
      ...
      grn_items.rate,
      grn_items.disc_val,
      grn_items.disc_percent,
      grn_items.gst_percent,
      grn_items.total AS item_total

    FROM grn
    LEFT JOIN grn_items
      ON grn.id = grn_items.grn_id
    WHERE grn.id = ?
  `;
  ...
  db.query(sql, [id], (err, rows) => {
    ...
    if (rows.length === 0)
      return res.json({ message: "grn not found" });
    ...
    const header = {
      ...
      grn_date: formatDateDMY(rows[0].grn_date),
...
router.put("/grn/:id", (req, res) => {
  ...
  const totalQty = grn.items.reduce((sum, row) => sum + Number(row.qty || 0), 0);

  const sql = `
    UPDATE grn 
    SET customer_id=?, grn_date=?, total_qty=?, total_amount=? 
    WHERE id=?
  `;
  ...
  db.query(sql, data, (err, result) => {
    if (err) return res.status(500).json({ error: err });
    res.json({ message: "Grn updated successfully!", totalQty });
  });
});
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    const oldItems = await new Promise((resolve, reject) => {
      db.query(
        "SELECT sku, qty FROM grn_items WHERE grn_id = ?",
        [id],
        (err, results) => {
          if (err) return reject(err);
          resolve(results || []);
...
//       whereParts.push("grn_date = ?");
//       params.push(grn_date);
//     }
//     if (due_date) {
...
//         DATE_FORMAT(grn_date, '%d-%m-%Y') AS grn_date,
//         total_qty,
//         sales_person,
//         grand_total,
//         mobile,
...
//       // total count
//       const countSql = `SELECT COUNT(*) AS total FROM grn ${whereClause}`;
//       db.query(countSql, params, (err2, countRows) => {
//         if (err2) {
//           console.error("grn count error:", err2);
...
//     });
//   } catch (e) {
//     console.error("Unexpected error in GET /grn:", e);
  ...
  try {
    ...
    db.query(privQ, [usertype_id], (err, privRows) => {
      ...
      db.query(sql, queryParams, (err2, rows) => {
        if (err2) {
          console.error("GRN Error:", err2);
          return res.status(500).json({ message: "GRN DB Error" });
        }
        ...
        db.query(countSql, params, (err3, countRows) => {
          if (err3) {
            console.error("GRN count error:", err3);
            return res.status(500).json({ message: "GRN DB Error" });
          }
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    await queryPromise("DELETE FROM grn_items WHERE grn_id = ?", [id]);

    // 4️⃣ Delete invoice
    await queryPromise("DELETE FROM grn WHERE id = ?", [id]);

    res.json({ success: true, message: "grn deleted & stock restored." });
  } catch (err) {
