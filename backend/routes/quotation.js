...
router.post("/", async (req, res) => {
  ...
  const finalquotationDate = fromDMYtoYMD(quotation_date);
  const finalDueDate = fromDMYtoYMD(due_date);

  if (!items || items.length === 0) {
    return res.status(400).json({ error: "No quotation items provided" });
  }
  ...
  try {
    ...
    const quotationSql = `
      ...
      (customer_name, ship_to, mobile, sales_person, quotation_date, due_date, billing_address, total_qty, discount_total, grand_total, year_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    const itemSql = `
      ...
      (quotation_id, sku, qty, rate, disc_val, disc_percent, gst_percent, total)
      VALUES ?
    `;
...
router.get("/:id", (req, res) => {
  ...
  const sql = `
    SELECT 
      ...
      quotation_items.disc_val,
      quotation_items.disc_percent,
      quotation_items.gst_percent,
      quotation_items.total AS item_total

    FROM quotation
    LEFT JOIN quotation_items
      ON quotation.id = quotation_items.quotation_id
    WHERE quotation.id = ?
  `;
  ...
  db.query(sql, [id], (err, rows) => {
    ...
    const header = {
      ...
      quotation_date: formatDateDMY(rows[0].quotation_date),
...
router.put("/quotation/:id", (req, res) => {
  ...
  const totalQty = quotation.items.reduce((sum, row) => sum + Number(row.qty || 0), 0);

  const sql = `
    UPDATE quotation 
    SET customer_id=?, quotation_date=?, total_qty=?, total_amount=? 
    WHERE id=?
  `;
  ...
  db.query(sql, data, (err, result) => {
    if (err) return res.status(500).json({ error: err });
    res.json({ message: "quotation updated successfully!", totalQty });
  });
});
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    const oldItems = await new Promise((resolve, reject) => {
      db.query(
        "SELECT sku, qty FROM quotation_items WHERE quotation_id = ?",
        [id],
        (err, results) => {
          if (err) return reject(err);
          resolve(results || []);
...
//         DATE_FORMAT(quotation_date, '%d-%m-%Y') AS quotation_date,
//         total_qty,
//         sales_person,
//         grand_total,
//         mobile,
...
//       // total count
//       const countSql = `SELECT COUNT(*) AS total FROM quotation ${whereClause}`;
//       db.query(countSql, params, (err2, countRows) => {
//         if (err2) {
//           console.error("quotation count error:", err2);
...
//     });
//   } catch (e) {
//     console.error("Unexpected error in GET /quotation:", e);
...
router.get("/", (req, res) => {
  try {
    ...
    // 1️⃣ Fetch privileges (same as Invoice)
    const privQ = `
      SELECT 
        p.can_add,
        p.can_update,
    ...
    db.query(privQ, [usertype_id], (err, privRows) => {
      ...
      db.query(sql, queryParams, (err2, rows) => {
        if (err2) {
          console.error("Quotation Error:", err2);
          return res.status(500).json({ message: "Quotation DB Error" });
        }
        ...
        db.query(countSql, params, (err3, countRows) => {
          if (err3) {
            console.error("Quotation count error:", err3);
            return res.status(500).json({ message: "Quotation DB Error" });
          }
          ...
          // ✅ Final response (same structure as Invoice)
          res.json({
            data: rows,
            privileges: quotationPriv,
            pagination: {
              page,
              limit,
              total,
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    await queryPromise("DELETE FROM quotation_items WHERE quotation_id = ?", [id]);

    // 4️⃣ Delete invoice
    await queryPromise("DELETE FROM quotation WHERE id = ?", [id]);

    res.json({ success: true, message: "quotation deleted & stock restored." });
  } catch (err) {
