...
router.get("/next-sales-return-no", async (req, res) => {
...
router.post("/", async (req, res) => {
  ...
  const finalsales_returnDate = fromDMYtoYMD(sales_return_date);
  const finalDueDate = fromDMYtoYMD(due_date);

  if (!items || items.length === 0) {
    return res.status(400).json({ error: "No sales return items provided" });
  }
  ...
  try {
    ...
    if (!templateResult.length) {
      return res.status(400).json({ error: "Sales return template not found" });
    }
    ...
    const salesreturnSql = `
      ...
      (customer_name, ship_to, mobile, sales_person, sales_return_date, due_date, billing_address, total_qty, discount_total, grand_total, year_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;

    const salesreturnResult = await new Promise((resolve, reject) => {
      db.query(
        salesreturnSql,
        [
          customer_name,
    ...
    const salesreturn_id = salesreturnResult.insertId;

    // Insert items
    const itemSql = `
      INSERT INTO sales_return_items
      (sales_return_id, sku, qty, rate, disc_val, disc_percent, gst_percent, total)
      VALUES ?
    `;
    ...
    res.json({ success: true, message: "salesreturn saved successfully!" });
  } catch (err) {
    console.error("❌ Error saving salesreturn:", err);
...
router.get("/:id", (req, res) => {
  ...
  const sql = `
    SELECT 
      ...
      sales_return.sales_person,
      sales_return.sales_return_date,
      sales_return.mobile,
      sales_return.due_date,
      sales_return.discount_total,
      sales_return.gst_total,
      sales_return.net_total,
      sales_return.grand_total,
      sales_return.created_at,
      sales_return.round_off,
      sales_return.igst,
      sales_return.ship_to,
      sales_return.billing_address,
      sales_return.total_qty,

      sales_return_items.sku,
      sales_return_items.qty,
      sales_return_items.rate,
      sales_return_items.disc_val,
      sales_return_items.disc_percent,
      sales_return_items.gst_percent,
      sales_return_items.total AS item_total

    FROM sales_return
    LEFT JOIN sales_return_items
      ON sales_return.id = sales_return_items.sales_return_id
    WHERE sales_return.id = ?
  `;
  ...
  db.query(sql, [id], (err, rows) => {
    ...
    if (rows.length === 0)
      return res.json({ message: "Sales return not found" });
    ...
    const header = {
      ...
      sales_return_date: formatDateDMY(rows[0].sales_return_date),
...
router.put("/salesreturn/:id", (req, res) => {
  const { id } = req.params;
  const salesreturn = req.body;

  const totalQty = salesreturn.items.reduce((sum, row) => sum + Number(row.qty || 0), 0);

  const sql = `
    UPDATE sales_return 
    SET customer_id=?, sales_return_date=?, total_qty=?, total_amount=? 
    WHERE id=?
  `;
  ...
  db.query(sql, data, (err, result) => {
    if (err) return res.status(500).json({ error: err });
    res.json({ message: "Sales return updated successfully!", totalQty });
  });
});
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    const oldItems = await new Promise((resolve, reject) => {
      db.query(
        "SELECT sku, qty FROM sales_return_items WHERE sales_return_id = ?",
        [id],
        (err, results) => {
          if (err) return reject(err);
          resolve(results || []);
    ...
    await new Promise((resolve, reject) => {
      db.query(
        "DELETE FROM sales_return_items WHERE sales_return_id = ?",
        [id],
        (err) => {
          if (err) return reject(err);
          resolve();
...
//       whereParts.push("sales_return_date = ?");
//       params.push(sales_return_date);
//     }
//     if (due_date) {
...
//         DATE_FORMAT(sales_return_date, '%d-%m-%Y') AS sales_return_date,
//         total_qty,
//         sales_person,
//         grand_total,
//         mobile,
...
//       // total count
//       const countSql = `SELECT COUNT(*) AS total FROM sales_return ${whereClause}`;
//       db.query(countSql, params, (err2, countRows) => {
//         if (err2) {
//           console.error("sales return count error:", err2);
...
//     });
//   } catch (e) {
//     console.error("Unexpected error in GET /sales return:", e);
...
router.get("/", (req, res) => {
  try {
    ...
    // 🔹 WHERE clause (SALES RETURN LOGIC SAME)
    const yearId = req.headers['x-year-id'];
    ...
    // 1️⃣ Fetch privileges (SALES RETURN)
    const privQ = `
      SELECT 
        p.can_add,
        p.can_update,
      ...
      WHERE p.usertype_id = ?
        AND m.module_name = 'SALES_RETURN'
      LIMIT 1
    `;
    ...
    db.query(privQ, [usertype_id], (err, privRows) => {
      ...
      // 2️⃣ Fetch SALES RETURN list (same logic)
      const sql = `
        SELECT
          id,
          customer_name,
      ...
      db.query(sql, queryParams, (err2, rows) => {
        if (err2) {
          console.error("Sales Return Error:", err2);
          return res.status(500).json({ message: "Sales Return DB Error" });
        }
        ...
        db.query(countSql, params, (err3, countRows) => {
          if (err3) {
            console.error("Sales Return count error:", err3);
            return res.status(500).json({ message: "Sales Return DB Error" });
          }
          ...
          // ✅ FINAL RESPONSE (same as ESTIMATE / DC / GRN)
          res.json({
            data: rows,
            privileges: salesReturnPriv,
            pagination: {
              page,
              limit,
              total,
    ...
    console.error("Unexpected error in GET /sales_return:", e);
    res.status(500).json({ error: "Unexpected server error" });
  }
});
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    await queryPromise("DELETE FROM sales_return_items WHERE sales_return_id = ?", [id]);

    // 4️⃣ Delete invoice
    await queryPromise("DELETE FROM sales_return WHERE id = ?", [id]);

    res.json({ success: true, message: "sales return deleted & stock restored." });
  } catch (err) {
    console.error("DELETE /sales_return/:id error:", err);
