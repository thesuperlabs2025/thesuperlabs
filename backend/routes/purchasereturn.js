...
router.get("/next-purchase-return-no", async (req, res) => {
  ...
  try {
    ...
    res.json({ purchaseReturnNo: nextNo });
  } catch (err) {
    console.error(err);
...
router.post("/", async (req, res) => {
  ...
  const finalpurchase_returnDate = fromDMYtoYMD(purchase_return_date);
  const finalDueDate = fromDMYtoYMD(due_date);

  if (!items || items.length === 0) {
    return res.status(400).json({ error: "No purchase return items provided" });
  }
  ...
  try {
    ...
    if (!templateResult.length) {
      return res.status(400).json({ error: "Purchase return template not found" });
    }
    ...
    const purchasereturnSql = `
      ...
      (customer_name, ship_to, mobile, sales_person, purchase_return_date, due_date, billing_address, total_qty, discount_total, grand_total, year_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;

    const purchasereturnResult = await new Promise((resolve, reject) => {
      db.query(
        purchasereturnSql,
        [
          customer_name,
          ...
          mobile,
          sales_person,
          finalpurchase_returnDate,   // <-- FIXED
          finalDueDate,       // <-- FIXED
          billing_address,
          total_qty,
          discount_total,
    ...
    const purchasereturn_id = purchasereturnResult.insertId;

    // Insert items
    const itemSql = `
      INSERT INTO purchase_return_items
      (purchase_return_id, sku, qty, rate, disc_val, disc_percent, gst_percent, total)
      VALUES ?
    `;
...
router.get("/:id", (req, res) => {
  ...
  const sql = `
    SELECT 
      ...
      purchase_return.sales_person,
      purchase_return.purchase_return_date,
      purchase_return.mobile,
      purchase_return.due_date,
      purchase_return.discount_total,
      purchase_return.gst_total,
      purchase_return.net_total,
      purchase_return.grand_total,
      purchase_return.created_at,
      purchase_return.round_off,
      purchase_return.igst,
      purchase_return.ship_to,
     purchase_return.billing_address,
      purchase_return.total_qty,

      purchase_return_items.sku,
      purchase_return_items.qty,
      purchase_return_items.rate,
      purchase_return_items.disc_val,
      purchase_return_items.disc_percent,
      purchase_return_items.gst_percent,
      purchase_return_items.total AS item_total

    FROM purchase_return
    LEFT JOIN purchase_return_items
      ON purchase_return.id = purchase_return_items.purchase_return_id
    WHERE purchase_return.id = ?
  `;
  ...
  db.query(sql, [id], (err, rows) => {
    ...
    if (rows.length === 0)
      return res.json({ message: "Purchase return not found" });
    ...
    const header = {
      ...
      purchase_return_date: formatDateDMY(rows[0].purchase_return_date),
...
router.put("/purchasereturn/:id", (req, res) => {
  const { id } = req.params;
  const purchasereturn = req.body;

  const totalQty = purchasereturn.items.reduce((sum, row) => sum + Number(row.qty || 0), 0);

  const sql = `
    UPDATE purchase_return 
    SET customer_id=?, purchase_return_date=?, total_qty=?, total_amount=? 
    WHERE id=?
  `;

  const data = [
    purchasereturn.customer_id,
    purchasereturn.purchase_return_date,
    totalQty,
  ...
  db.query(sql, data, (err, result) => {
    if (err) return res.status(500).json({ error: err });
    res.json({ message: "Purchase return updated successfully!", totalQty });
  });
});
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    const oldItems = await new Promise((resolve, reject) => {
      db.query(
        "SELECT sku, qty FROM purchase_return_items WHERE purchase_return_id = ?",
        [id],
        (err, results) => {
          if (err) return reject(err);
          resolve(results || []);
    ...
    // ⭐⭐⭐ FIXED SQL QUERY ⭐⭐⭐
    const updatepurchase_returnSql = `
...
//       whereParts.push("purchase_return_date = ?");
//       params.push(purchase_return_date);
//     }
//     if (due_date) {
...
//         DATE_FORMAT(purchase_return_date, '%d-%m-%Y') AS purchase_return_date,
//         total_qty,
//         sales_person,
//         grand_total,
//         mobile,
...
//       // total count
//       const countSql = `SELECT COUNT(*) AS total FROM purchase_return ${whereClause}`;
//       db.query(countSql, params, (err2, countRows) => {
//         if (err2) {
//           console.error("purchase return count error:", err2);
...
//     });
//   } catch (e) {
//     console.error("Unexpected error in GET /purchase return:", e);
...
router.get("/", (req, res) => {
  try {
    ...
    if (purchase_return_date) {
      where.push("purchase_return_date = ?");
      params.push(purchase_return_date);
    }
    if (due_date) {
      where.push("due_date = ?");
    ...
    const privQ = `
      ...
      WHERE p.usertype_id = ?
        AND m.module_name = 'PURCHASE RETURN'
      LIMIT 1
    `;
    ...
    db.query(privQ, [usertype_id], (perr, privRows) => {
      ...
      db.query(sql, [...params, limit, offset], (err, rows) => {
        if (err) {
          ...
          return res.status(500).json({ message: "purchase return DB Error" });
        }

        // count (UNCHANGED)
        db.query(
          `SELECT COUNT(*) AS total FROM purchase_return ${whereSql}`,
          params,
          (err2, count) => {
            if (err2) {
              console.error("purchase return count error:", err2);
              return res.status(500).json({ message: "purchase returnDB Error" });
            }
    ...
    console.error("Unexpected error in GET /purchasereturn:", e);
    res.status(500).json({ error: "Unexpected server error" });
  }
});
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    await queryPromise("DELETE FROM purchase_return_items WHERE purchase_return_id = ?", [id]);

    // 4️⃣ Delete invoice
    await queryPromise("DELETE FROM purchase_return WHERE id = ?", [id]);

    res.json({ success: true, message: "purchase return deleted & stock restored." });
  } catch (err) {
    console.error("DELETE /purchase_return/:id error:", err);
