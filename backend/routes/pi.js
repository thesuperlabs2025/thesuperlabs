...
router.get("/next-pi-no", (req, res) => {
    ...
    const query = "SELECT MAX(id) as maxId FROM pi WHERE year_id = ?";
    db.query(query, [yearId], (err, data) => {
        if (err) return res.status(500).json(err);
        const nextId = (data[0].maxId || 0) + 1;
        res.status(200).json({ piNo: nextId });
    });
});
...
router.post("/", async (req, res) => {
  ...
  const finalPiDate = fromDMYtoYMD(pi_date);
  const finalDueDate = fromDMYtoYMD(due_date);

  if (!items || items.length === 0) {
    return res.status(400).json({ error: "No pi items provided" });
  }
  ...
  try {
    ...
    if (!templateResult.length) {
      return res.status(400).json({ error: "PI template not found" });
    }
    ...
    const piSql = `
      INSERT INTO pi
      (customer_name, ship_to, mobile, sales_person, pi_date, due_date, billing_address, total_qty, discount_total, grand_total, is_inclusive, year_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;

    const piResult = await new Promise((resolve, reject) => {
      db.query(
        piSql,
        [
          customer_name,
    ...
    const itemSql = `
      ...
      (pi_id, sku, qty, rate, disc_val, disc_percent, gst_percent, total)
      VALUES ?
    `;
    ...
    res.json({ success: true, message: "pi saved successfully!" });
  } catch (err) {
...
router.get("/:id", (req, res) => {
  ...
  const sql = `
    SELECT 
      ...
      pi_items.rate,
      pi_items.disc_val,
      pi_items.disc_percent,
      pi_items.gst_percent,
      pi_items.total AS item_total

    FROM pi
    LEFT JOIN pi_items
      ON pi.id = pi_items.pi_id
    WHERE pi.id = ?
  `;
  ...
  db.query(sql, [id], (err, rows) => {
    ...
    if (rows.length === 0)
      return res.json({ message: "Pi not found" });
    ...
    const header = {
      ...
      pi_date: formatDateDMY(rows[0].pi_date),
...
router.put("/pi/:id", (req, res) => {
  const { id } = req.params;
  const pi = req.body;

  const totalQty = pi.items.reduce((sum, row) => sum + Number(row.qty || 0), 0);

  const sql = `
    UPDATE pi 
    SET customer_id=?, pi_date=?, total_qty=?, total_amount=? 
    WHERE id=?
  `;
  ...
  db.query(sql, data, (err, result) => {
    if (err) return res.status(500).json({ error: err });
    res.json({ message: "PI updated successfully!", totalQty });
  });
});
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    const oldItems = await new Promise((resolve, reject) => {
      db.query(
        "SELECT sku, qty FROM pi_items WHERE pi_id = ?",
        [id],
        (err, results) => {
          if (err) return reject(err);
          resolve(results || []);
    ...
    await new Promise((resolve, reject) => {
      db.query(
        "DELETE FROM pi_items WHERE pi_id = ?",
        [id],
        (err) => {
          if (err) return reject(err);
          resolve();
    ...
    res.json({ success: true, message: "PI updated successfully!" });
  } catch (err) {
...
//         DATE_FORMAT(pi_date, '%d-%m-%Y') AS pi_date,
//         total_qty,
//         sales_person,
//         grand_total,
//         mobile,
...
//       // total count
//       const countSql = `SELECT COUNT(*) AS total FROM pi ${whereClause}`;
//       db.query(countSql, params, (err2, countRows) => {
//         if (err2) {
//           console.error("pi count error:", err2);
...
//     });
//   } catch (e) {
//     console.error("Unexpected error in GET /pi:", e);
  ...
  try {
    ...
    db.query(privQ, [usertype_id], (err, privRows) => {
      ...
      db.query(sql, queryParams, (err2, rows) => {
        if (err2) {
          console.error("PI Error:", err2);
          return res.status(500).json({ message: "PI DB Error" });
        }
        ...
        db.query(countSql, params, (err3, countRows) => {
          if (err3) {
            console.error("PI count error:", err3);
            return res.status(500).json({ message: "PI DB Error" });
          }
          ...
          // ✅ Final response (same as Invoice & Quotation)
          res.json({
            data: rows,
            privileges: piPriv,
            pagination: {
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    await queryPromise("DELETE FROM pi_items WHERE pi_id = ?", [id]);

    // 4️⃣ Delete invoice
    await queryPromise("DELETE FROM pi WHERE id = ?", [id]);

    res.json({ success: true, message: "pi deleted & stock restored." });
  } catch (err) {
    console.error("DELETE /pi/:id error:", err);
