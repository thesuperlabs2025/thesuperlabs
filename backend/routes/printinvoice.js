...
router.get("/invoice/:id", async (req, res) => {
  ...
  try {
    const invoice = await new Promise((resolve, reject) => {
      db.query("SELECT * FROM invoices WHERE id = ?", [id], (e, r) =>
    ...
    const roundedTotal = Math.round(invoice.grand_total);
    const roundOff = roundedTotal - invoice.grand_total;
...
`;
    ...
    console.error(err);
    res.status(500).send("Invoice print failed");
  }
});
