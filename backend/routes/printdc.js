...
router.get("/dc/:id", async (req, res) => {
    ...
    try {
        const dc = await new Promise((resolve, reject) => {
            db.query("SELECT * FROM dc WHERE id = ?", [id], (e, r) =>
                e ? reject(e) : resolve(r[0])
            );
        });
        if (!dc) return res.status(404).send("DC not found");
        ...
        const discount = Number(dc.discount_total || 0);
        ...
        const roundedTotal = Math.round(dc.grand_total);
        const roundOff = roundedTotal - dc.grand_total;
...
`;
        ...
        res.setHeader("Content-Disposition", `inline; filename=dc-${id}.pdf`);
        res.send(pdf);

    } catch (err) {
        console.error(err);
        res.status(500).send("DC print failed");
    }
});
