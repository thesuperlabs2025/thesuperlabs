...
router.get("/po/:id", async (req, res) => {
    ...
    try {
        const po = await new Promise((resolve, reject) => {
            db.query("SELECT * FROM po WHERE id = ?", [id], (e, r) =>
        ...
        const roundedTotal = Math.round(po.grand_total);
        const roundOff = roundedTotal - po.grand_total;
...
`;
        ...
        console.error(err);
        res.status(500).send("PO print failed");
    }
});
