...
router.get("/pi/:id", async (req, res) => {
    ...
    try {
        const pi = await new Promise((resolve, reject) => {
            db.query("SELECT * FROM pi WHERE id = ?", [id], (e, r) =>
        ...
        const discount = Number(pi.discount_total || 0);
        ...
        const roundedTotal = Math.round(pi.grand_total);
        const roundOff = roundedTotal - pi.grand_total;
...
`;
        ...
        res.setHeader("Content-Disposition", `inline; filename=pi-${id}.pdf`);
        res.send(pdf);

    } catch (err) {
        console.error(err);
        res.status(500).send("PI print failed");
    }
});
