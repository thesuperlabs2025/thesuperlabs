...
/* ===== PRINT CREDIT NOTE ===== */
router.get("/creditnote/:id", async (req, res) => {
    const { id } = req.params;

    try {
        const credit_note = await new Promise((resolve, reject) => {
            db.query("SELECT * FROM credit_note WHERE id = ?", [id], (e, r) =>
                e ? reject(e) : resolve(r[0])
            );
        });
        if (!credit_note) return res.status(404).send("Credit Note not found");
        ...
        const discount = Number(credit_note.discount_total || 0);
        ...
        const roundedTotal = Math.round(credit_note.grand_total);
        const roundOff = roundedTotal - credit_note.grand_total;
...
<div class="header">
  ...
  <div class="invoice-title">
    CREDIT NOTE<br>
    <span class="small">
      Note No: ${credit_note.id}<br>
      Date: ${formatDate(credit_note.credit_note_date)}
    </span>
  </div>
</div>
...
`;
        ...
        res.setHeader("Content-Disposition", `inline; filename=creditnote-${id}.pdf`);
        res.send(pdf);

    } catch (err) {
        console.error(err);
        res.status(500).send("Credit Note print failed");
    }
});
