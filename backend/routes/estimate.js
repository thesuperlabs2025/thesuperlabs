...
router.get("/next-estimate-no", async (req, res) => {
...
router.post("/", async (req, res) => {
  ...
  const finalEstimateDate = fromDMYtoYMD(estimate_date);
  const finalDueDate = fromDMYtoYMD(due_date);

  if (!items || items.length === 0) {
    return res.status(400).json({ error: "No estimate items provided" });
  }
  ...
  try {
    ...
    if (!templateResult.length) {
      return res.status(400).json({ error: "Estimate template not found" });
    }
    ...
    const estimateSql = `
      ...
      (customer_name, ship_to, mobile, sales_person, estimate_date, due_date, billing_address, total_qty, discount_total, grand_total, year_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;

    const estimateResult = await new Promise((resolve, reject) => {
      db.query(
        estimateSql,
        [
          customer_name,
    ...
    const estimate_id = estimateResult.insertId;

    // Insert items
    const itemSql = `
      INSERT INTO estimate_items
      (estimate_id, sku, qty, rate, disc_val, disc_percent, gst_percent, total)
      VALUES ?
    `;
...
router.get("/:id", (req, res) => {
  ...
  const sql = `
    SELECT 
      ...
      estimate_items.qty,
      estimate_items.rate,
      estimate_items.disc_val,
      estimate_items.disc_percent,
      estimate_items.gst_percent,
      estimate_items.total AS item_total

    FROM estimate
    LEFT JOIN estimate_items
      ON estimate.id = estimate_items.estimate_id
    WHERE estimate.id = ?
  `;
  ...
  db.query(sql, [id], (err, rows) => {
    ...
    if (rows.length === 0)
      return res.json({ message: "Estimate not found" });
    ...
    const header = {
      ...
      estimate_date: formatDateDMY(rows[0].estimate_date),
...
router.put("/estimate/:id", (req, res) => {
  ...
  const totalQty = estimate.items.reduce((sum, row) => sum + Number(row.qty || 0), 0);

  const sql = `
    UPDATE estimate 
    SET customer_id=?, estimate_date=?, total_qty=?, total_amount=? 
    WHERE id=?
  `;
  ...
  db.query(sql, data, (err, result) => {
    if (err) return res.status(500).json({ error: err });
    res.json({ message: "Estimate updated successfully!", totalQty });
  });
});
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    const oldItems = await new Promise((resolve, reject) => {
      db.query(
        "SELECT sku, qty FROM estimate_items WHERE estimate_id = ?",
        [id],
        (err, results) => {
          if (err) return reject(err);
          resolve(results || []);
...
//       whereParts.push("estimate_date = ?");
//       params.push(estimate_date);
//     }
//     if (due_date) {
...
//         DATE_FORMAT(estimate_date, '%d-%m-%Y') AS estimate_date,
//         total_qty,
//         sales_person,
//         grand_total,
//         mobile,
...
//       // total count
//       const countSql = `SELECT COUNT(*) AS total FROM estimate ${whereClause}`;
//       db.query(countSql, params, (err2, countRows) => {
//         if (err2) {
//           console.error("estimate count error:", err2);
...
//     });
//   } catch (e) {
//     console.error("Unexpected error in GET /estimate:", e);
...
router.get("/", (req, res) => {
  try {
    ...
    // 🔹 WHERE clause (ESTIMATE LOGIC SAME)
    const yearId = req.headers['x-year-id'];
    ...
    db.query(privQ, [usertype_id], (err, privRows) => {
      ...
      // 2️⃣ Fetch ESTIMATE list (same logic)
      const sql = `
        SELECT
          id,
          customer_name,
      ...
      db.query(sql, queryParams, (err2, rows) => {
        if (err2) {
          console.error("Estimate Error:", err2);
          return res.status(500).json({ message: "Estimate DB Error" });
        }
        ...
        db.query(countSql, params, (err3, countRows) => {
          if (err3) {
            console.error("Estimate count error:", err3);
            return res.status(500).json({ message: "Estimate DB Error" });
          }
          ...
          // ✅ FINAL RESPONSE (same as DC / GRN / PO)
          res.json({
            data: rows,
            privileges: estimatePriv,
            pagination: {
              page,
              limit,
              total,
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    await queryPromise("DELETE FROM estimate_items WHERE estimate_id = ?", [id]);

    // 4️⃣ Delete invoice
    await queryPromise("DELETE FROM estimate WHERE id = ?", [id]);

    res.json({ success: true, message: "estimate deleted & stock restored." });
  } catch (err) {
    console.error("DELETE /estimate/:id error:", err);
