...
/* ===== PRINT DEBIT NOTE ===== */
router.get("/debitnote/:id", async (req, res) => {
    const { id } = req.params;

    try {
        const debit_note = await new Promise((resolve, reject) => {
            db.query("SELECT * FROM debit_note WHERE id = ?", [id], (e, r) =>
                e ? reject(e) : resolve(r[0])
            );
        });
        if (!debit_note) return res.status(404).send("Debit Note not found");
        ...
        const discount = Number(debit_note.discount_total || 0);
        ...
        const roundedTotal = Math.round(debit_note.grand_total);
        const roundOff = roundedTotal - debit_note.grand_total;
...
<div class="header">
  ...
  <div class="invoice-title">
    DEBIT NOTE<br>
    <span class="small">
      Note No: ${debit_note.id}<br>
      Date: ${formatDate(debit_note.debit_note_date)}
    </span>
  </div>
</div>
...
`;
        ...
        res.setHeader("Content-Disposition", `inline; filename=debitnote-${id}.pdf`);
        res.send(pdf);

    } catch (err) {
        console.error(err);
        res.status(500).send("Debit Note print failed");
    }
});
