...
function PoForm() {
  ...
  useEffect(() => {
    document.title = `PO No: ${poNo || ""} - TSL ERP`;
  }, [poNo]);
  ...
  useEffect(() => {
    axios.get(`${API}/templates`)
      .then((res) => {
        ...
        // ⭐ Find DC template
        const poTpl = templates.find(
          (t) => t.template_name.toLowerCase() === "po"
        );

        if (!poTpl || !poTpl.stock_action) {
          toast.error("No valid 'po' template found.");
          setPO((prev) => ({
            ...prev,
            template_id: null,
            stock_action: null,
  ...
  const fetchPoNo = async () => {
    try {
      const res = await axios.get(`${API}/po/next-po-no`);
      setPoNo(res.data.poNo);
    } catch (err) {
      console.error("Failed to load PO number", err);
    }
  };
  ...
  const selectProduct = (index, product) => {
    ...
    const { total } = calculateItemTotal(newItems[index], po.is_inclusive);
  ...
  // --- IGST toggle ---
  const toggleIGST = () =>
    setPO((prev) => ({ ...prev, igst: !prev.igst }));

  // --- Save DC ---
  const savePO = async () => {
    // 🛑 Stop if no template
    if (po.template_id == null) {
      toast.error("Cannot save po: valid 'po' template not selected.");
      return;
    }

    if (!po.items || po.items.length === 0) {
      toast.error("Cannot save po: No items added.");
      return;
    }
    ...
    const poDateVal = new Date(po.header.po_date);
    ...
    try {
      const res = await axios.post(`${API}/po`, {
        ...
        ship_to: po.ship_to,
        mobile: po.mobile,
        po_date: toDMY(po.po_date),
        due_date: toDMY(po.due_date),
      ...
      console.error("Error saving po:", err.response?.data || err.message);
      toast.error(err.response?.data?.error || "Error saving po");
    }
  };
  ...
  // --- Clear form ---
  const clearForm = () => {
    if (window.confirm("Clear form?")) setPO(getInitialState());
  };
      ...
      <div className="card shadow p-4 w-90">
        <div className="mb-3 d-flex justify-content-between align-items-center border-bottom pb-3">
          ...
          <div className="text-center flex-grow-1">
            <h3 className="fw-bold mb-0">PO Form</h3>
          </div>
        ...
        <div className="d-flex justify-content-between align-items-center mb-3 mt-4">
          ...
          {/* Removed the old PO # NEW display */}
        </div>
            ...
            <div className="col-md-6 mb-2 position-relative">
              ...
              {shipToSuggestions.length > 0 && (
                ...
                >
                  {shipToSuggestions
                    .filter(cust =>
                      (cust.customer_name ?? cust.name ?? "").toLowerCase().includes((po.ship_to || "").toLowerCase())
                    )
                ...
                <td className="fw-bold">
                  {
                    (() => {
                      ...
                      po.total_qty = qtyTotal; // ✅ store inside DC object
        ...
        <div className="border p-3 bg-light">
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={po.discount_total.toFixed(2)}
              readOnly
              className="form-control form-control-sm text-end w-25"
            />
          </div>
          ...
          {po.igst ? (
            ...
            <>
              <div className="d-flex justify-content-between mb-2">
                <label>CGST:</label>
                <input
                  value={(po.gst_total / 2).toFixed(2)}
                  readOnly
                  className="form-control form-control-sm text-end w-25"
                />
              </div>
        ...
        <div className="text-center mt-4">
          {selectedYear.is_closed ? (
            ...
            <>
              <button className="btn btn-success px-5 py-2 me-2 fw-bold shadow-sm rounded-pill" onClick={savePO}>
                Save PO
              </button>
