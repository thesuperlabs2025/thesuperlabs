...
export default function QuotationLedger() {
    ...
    const handleSearch = async (f = filters) => {
        try {
            const res = await axios.get(`${API}/reports/quotation/ledger`, { params: f });
            setData(res.data);
        } catch (err) { console.error(err); }
    };
                    ...
                    <tbody>
                        {data.map((item, i) => (
                            <tr key={i}>
                                ...
                                <td>{parseFloat(item.gst_total).toFixed(2)}</td>
                                <td className="fw-bold">{parseFloat(item.grand_total).toFixed(2)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
