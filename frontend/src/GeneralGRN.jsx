...
const GeneralGRN = () => {
    ...
    const [items, setItems] = useState([
        { counts: "", yarn_name: "", fabric_name: "", trims_name: "", color: "", size: "", gsm: "", dia: "", per_bag: "", per_bag_qty: "", rolls: "", qty: "" }
    ]);
        ...
        const fetchInitialData = async () => {
            try {
                const [grnRes, supRes, staffRes] = await Promise.all([
                    axios.get(`${API}/general-grn/next-no/gen`),
                ...
                if (location.state && location.state.po_data) {
                    ...
                    const fullPoRes = await axios.get(`${API}/general-po/${po.id}`);
                    const fullPo = fullPoRes.data;

                    setHeader(prev => ({
                        ...prev,
                        supplier_name: fullPo.ship_to || fullPo.supplier_name,
                        grn_type: fullPo.po_type || "General",
                        po_no: fullPo.po_no
                    }));
                    setItems(fullPo.items
                        .map(it => ({
                            ...
                            trims_name: it.trims_name,
                            color: it.color,
                            size: it.size,
                            gsm: it.gsm,
                            dia: it.dia,
                            per_bag: it.per_bag,
                            per_bag_qty: it.per_bag_qty,
                            rolls: it.rolls,
    ...
    const saveGRN = async () => {
        ...
        if (header.po_no) {
            for (const item of items) {
                ...
                if (item.po_qty && Number(item.qty || 0) > Number(remaining.toFixed(3)) + 0.0001) {
                    const itemName = item.yarn_name || item.fabric_name || item.trims_name || "Item";
                    return toast.error(`Quantity for ${itemName} exceeds remaining PO quantity (${remaining.toFixed(3)})`);
                }
            }
        }
        ...
        try {
            await axios.post(`${API}/general-grn`, {
                grn_no: grnNo,
                ...header,
                year_id: selectedYear.year_id,
                items
            });
            toast.success("General GRN Created Successfully!");
                ...
                <div className="card-header bg-white py-3 border-bottom shadow-sm">
                    <div className="d-flex justify-content-between align-items-center">
                        ...
                        <div className="text-center flex-grow-1">
                            <h4 className="fw-bold mb-0 text-uppercase tracking-wider">General GRN</h4>
                        </div>
                ...
                <div className="card-body p-4">
                    ...
                    <div className="row g-3 mb-4">
                        ...
                        <div className="col-md-3">
                            ...
                            <Select options={grnTypes} defaultValue={grnTypes[0]} onChange={(opt) => setHeader(prev => ({ ...prev, grn_type: opt?.value || "General" }))} />
                        </div>
                        <div className="col-md-3">
                    ...
                    <div className="d-flex justify-content-end gap-2">
                        <button className="btn btn-secondary rounded-pill px-4" onClick={() => navigate('/general-grn-list')}>Cancel</button>
                        {selectedYear.is_closed ? (
                            ...
                            <button className="btn btn-success rounded-pill px-4 fw-bold shadow-sm" onClick={saveGRN}>
                                <i className="bi bi-check-lg me-2"></i>Save General GRN
                            </button>
                        )}
                    </div>
                </div>
