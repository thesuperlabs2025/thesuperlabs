...
function Estimatemy() {
  ...
  const [estimatePriv, setEstimatePriv] = useState(null);
  ...
  //     if (filters.estimate_date) params.estimate_date = filters.estimate_date;
  //     if (filters.due_date) params.due_date = filters.due_date;
  //     if (filters.status) params.status = filters.status;

  //     const res = await axios.get(`${API}/estimate`, { params });
  ...
  const fetchEstimate = useCallback(async () => {
    ...
    try {
      const params = {
        ...
        sort_by: sortBy,
        order,
        usertype_id: user.usertype_id, // ✅ same as DC / GRN / PO
      };
      ...
      // ✅ same pagination fallback logic as DC / GRN
      setPagination(
        res.data.pagination || {
          page: 1,
          limit: 20,
      ...
      setEstimatePriv(res.data.privileges || {});

      setSelectedIds([]);
      setSelectAll(false);
    } catch (err) {
      console.error("Fetch Estimate Error:", err);
      alert(err.response?.data?.message || "Error loading Estimate");
    } finally {
      setLoading(false);
    }
  ...
  const handleConvert = () => {
    if (selectedIds.length === 0) {
      alert("Please select at least one Estimate to convert.");
      return;
    }

    const selectedEstimates = estimate.filter(e => selectedIds.includes(e.id));
    const sameCustomer = selectedEstimates.every(e => e.customer_name === selectedEstimates[0].customer_name);

    if (!sameCustomer) {
      alert("Please select Estimates from the same customer for conversion.");
      return;
    }
  ...
  const confirmConvert = async () => {
    ...
    try {
      const fullDetails = await Promise.all(
        selectedIds.map(id => axios.get(`${API}/estimate/${id}`).then(res => res.data))
      );

      const first = fullDetails[0];
      const combinedItems = fullDetails.flatMap(e => e.items);
      ...
      alert("Failed to fetch estimate details for conversion.");
    }
  };
  ...
  const handleSelectAll = () => {
    if (!selectAll) {
      setSelectedIds(estimate.map((estimate) => estimate.id));
    } else {
      setSelectedIds([]);
    }
    setSelectAll((s) => !s);
  ...
  const handleDeleteSelected = async () => {
    if (selectedIds.length === 0) {
      alert("Select at least one estimate");
      return;
    }
    ...
    <div className="container-fluid py-4 px-md-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="fw-bold text-primary mb-0">
          <i className="bi bi-calculator-fill me-2"></i>Estimate Management
        </h2>
        <div className="d-flex align-items-center gap-3">
          ...
          <div className="btn-group shadow-sm">
            {estimatePriv?.can_add === 1 && (
              <button className="btn btn-primary d-flex align-items-center fw-bold px-4 shadow-sm" onClick={() => navigate("/estimateform")}>
                <i className="bi bi-plus-lg me-2"></i>New
              </button>
            )}
            {estimatePriv?.can_delete === 1 && (
      ...
      <div className="card shadow-sm border-0 mb-4 no-print">
        <div className="card-body p-4">
          <div className="row g-3">
            ...
            <div className="col-md-2">
              <label className="form-label fw-bold small text-muted">Estimate Date</label>
              <input type="date" name="estimate_date" className="form-control" value={filters.estimate_date} onChange={handleFilterChange} />
            </div>
            <div className="col-md-3">
        ...
        <div className="card shadow-sm border-0">
          <div className="card-body p-0 text-truncate">
            <div className="table-responsive">
              <table className="table table-hover table-striped mb-0 align-middle" style={{ fontSize: '16px' }}>
                <thead className="table-dark small">
                  <tr>
                    ...
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("id")}>
                      EST NO {sortBy === "id" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("estimate_date")}>
                      DATE {sortBy === "estimate_date" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                ...
                <tbody>
                  {estimate.length === 0 ? (
                    <tr><td colSpan="8" className="text-center py-5 text-muted fw-bold">No estimates found.</td></tr>
                  ) : (
                    estimate.map((est) => (
                      <tr key={est.id}>
                        <td className="ps-4 no-print"><input className="form-check-input" type="checkbox" checked={selectedIds.includes(est.id)} onChange={() => handleCheckboxChange(est.id)} /></td>
                        <td className="fw-bold text-dark">#{est.id}</td>
                        <td className="small">{est.estimate_date}</td>
                        <td className="fw-bold text-dark">{est.customer_name}</td>
                        <td className="text-end fw-bold">₹{est.grand_total}</td>
                        <td className="text-center small">{est.total_qty}</td>
                        <td><span className="badge bg-light text-dark fw-normal border">{est.sales_person}</span></td>
                        <td className="text-end pe-4 no-print">
                          <div className="d-flex justify-content-end gap-2">
                            <button className="btn btn-outline-secondary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                              ...
                              onClick={() => window.open(`${API}/printestimate/estimate/${est.id}`, "_blank")}>
                              <i className="bi bi-printer-fill"></i>
                            </button>
                            {estimatePriv?.can_update === 1 && (
                              <button className="btn btn-outline-primary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                                ...
                                onClick={() => navigate(`/Editestimateform/${est.id}`)}>
          ...
          <div className="card-footer bg-white py-3 border-0 d-flex justify-content-between align-items-center mb-5 no-print">
            <span className="text-muted small">
              Page {pagination?.page || 1} of {pagination?.pages || 1} &bull; Total {pagination?.total || 0} Estimates
            </span>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg">
              <div className="modal-header bg-primary text-white border-0">
                <h5 className="modal-title fw-bold">Convert Estimate(s)</h5>
                <button type="button" className="btn-close btn-close-white" onClick={() => setShowConvertModal(false)}></button>
              </div>
              <div className="modal-body p-4">
                <p className="text-muted small mb-4">Select the target module to convert selected Estimates into. Data like customer info and items will be carried over.</p>
                <div className="mb-3">
