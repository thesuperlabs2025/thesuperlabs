...
function QuotationForm() {
  ...
  useEffect(() => {
    document.title = `Quotation Form No: ${quotationNo || ""} - TSL ERP`;
  }, [quotationNo]);
  ...
  useEffect(() => {
    axios.get(`${API}/templates`)
      .then((res) => {
        ...
        // ⭐ Find quotation template
        const quotationTpl = templates.find(
          (t) => t.template_name.toLowerCase() === "quotation"
        );

        if (!quotationTpl || !quotationTpl.stock_action) {
          toast.error("No valid 'quotation' template found.");
          setQuotation((prev) => ({
            ...prev,
            template_id: null,
            stock_action: null,
        ...
        // ✅ Valid quotation template
        setQuotation((prev) => ({
          ...prev,
          stock_action: quotationTpl.stock_action,
          template_id: quotationTpl.id,
        }));
  ...
  const fetchQuotationNo = async () => {
    try {
      const res = await axios.get(`${API}/quotation/next-quotation-no`);
      setQuotationNo(res.data.quotationNo);
    } catch (err) {
      console.error("Failed to load Quotation number", err);
    }
  };
  ...
  const selectProduct = (index, product) => {
    const isDuplicate = quotation.items.some(
  ...
  // --- IGST toggle ---
  const toggleIGST = () =>
    setQuotation((prev) => ({ ...prev, igst: !prev.igst }));

  const saveQuotation = async () => {
    // 🛑 Stop if no template
    if (quotation.template_id == null) {
      toast.error("Cannot save quotation: valid 'quotation' template not selected.");
      return;
    }

    if (!quotation.items || quotation.items.length === 0) {
      toast.error("Cannot save quotation: No items added.");
      return;
    }
    ...
    if (qDate < startDate || qDate > endDate) {
      if (!window.confirm(`Warning: Quotation date is outside the selected Accounting Year (AY ${selectedYear.year_name}). Do you want to proceed?`)) {
        return;
      }
    }
    ...
    try {
      const res = await axios.post(`${API}/quotation`, {
        ...
        quotation_date: toDMY(quotation.quotation_date),
        due_date: toDMY(quotation.due_date),
      ...
      toast.success(res.data.message || "Quotation saved successfully!");
      ...
      console.error("Error saving quotation:", err.response?.data || err.message);
  ...
  // --- Clear form ---
  const clearForm = () => {
    if (window.confirm("Clear form?")) setQuotation(getInitialState());
  };
      ...
      <div className="card shadow p-4 w-90">
        <div className="mb-3 d-flex justify-content-between align-items-center border-bottom pb-3">
          <div className="d-flex align-items-center gap-4">
            <div>
              ...
              <div className="text-primary fw-bold fs-5">{quotationNo || "Loading..."}</div>
            </div>
          ...
          <div className="text-center flex-grow-1">
            <h3 className="fw-bold mb-0">Quotation Form</h3>
          </div>
                ...
                <td className="fw-bold">
                  {
                    (() => {
                      ...
                      quotation.total_qty = qtyTotal; // ✅ store inside quotation object
        ...
        <div className="border p-3 bg-light">
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={quotation.net_total.toFixed(2)}
              readOnly
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={quotation.discount_total.toFixed(2)}
              readOnly
              className="form-control form-control-sm text-end w-25"
            />
          </div>
          ...
          {quotation.igst ? (
            <div className="d-flex justify-content-between mb-2">
              <label>IGST:</label>
              <input
                value={quotation.gst_total.toFixed(2)}
                readOnly
                className="form-control form-control-sm text-end w-25"
              />
            </div>
            ...
            <>
              <div className="d-flex justify-content-between mb-2">
                <label>CGST:</label>
                <input
                  value={(quotation.gst_total / 2).toFixed(2)}
                  readOnly
                  className="form-control form-control-sm text-end w-25"
                />
              </div>
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={quotation.round_off.toFixed(2)}
              readOnly
              className="form-control form-control-sm text-end w-25"
            />
          </div>
        ...
        <div className="text-center mt-4">
          {selectedYear.is_closed ? (
            ...
            <>
              <button className="btn btn-success px-5 py-2 me-2 fw-bold shadow-sm rounded-pill" onClick={saveQuotation}>
                Save Quotation
              </button>
