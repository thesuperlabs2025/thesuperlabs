...
function EstimateForm() {
  ...
  useEffect(() => {
    document.title = `Estimate Form No: ${estimateNo || ""} - TSL ERP`;
  }, [estimateNo]);
  ...
  useEffect(() => {
    axios.get(`${API}/templates`)
      .then((res) => {
        ...
        // ⭐ Find estimate template
        const estimateTpl = templates.find(
          (t) => t.template_name.toLowerCase() === "estimate"
        );

        if (!estimateTpl || !estimateTpl.stock_action) {
          toast.error("No valid 'estimate' template found.");
          setEstimate((prev) => ({
            ...prev,
            template_id: null,
            stock_action: null,
        ...
        // ✅ Valid estimate template
        setEstimate((prev) => ({
          ...prev,
          stock_action: estimateTpl.stock_action,
          template_id: estimateTpl.id,
        }));
  ...
  const fetchEstimateNo = async () => {
    try {
      const res = await axios.get(`${API}/estimate/next-estimate-no`);
      setEstimateNo(res.data.estimateNo);
    } catch (err) {
      console.error("Failed to load Estimate number", err);
    }
  };
  ...
  const selectProduct = (index, product) => {
    const isDuplicate = estimate.items.some(
  ...
  // --- IGST toggle ---
  const toggleIGST = () =>
    setEstimate((prev) => ({ ...prev, igst: !prev.igst }));

  // --- Save estimate ---
  const saveEstimate = async () => {
    // 🛑 Stop if no template
    if (estimate.template_id == null) {
      toast.error("Cannot save estimate: valid 'estimate' template not selected.");
      return;
    }

    if (!estimate.items || estimate.items.length === 0) {
      toast.error("Cannot save estimate: No items added.");
      return;
    }
    ...
    const estDate = new Date(estimate.estimate_date);
    const startDate = new Date(selectedYear.start_date);
    const endDate = new Date(selectedYear.end_date);
    if (estDate < startDate || estDate > endDate) {
      if (!window.confirm(`Warning: Estimate date is outside the selected Accounting Year (AY ${selectedYear.year_name}). Do you want to proceed?`)) {
        return;
      }
    }
    ...
    try {
      const res = await axios.post(`${API}/estimate`, {
        ...
        estimate_date: toDMY(estimate.estimate_date),
        due_date: toDMY(estimate.due_date),
        sales_person: estimate.sales_person,
      ...
      toast.success(res.data.message || "estimate saved successfully!");
      ...
      console.error("Error saving estimate:", err.response?.data || err.message);
      toast.error(err.response?.data?.error || "Error saving estimate");
    }
  };
  ...
  // --- Clear form ---
  const clearForm = () => {
    if (window.confirm("Clear form?")) setEstimate(getInitialState());
  };
      ...
      <div className="card shadow p-4 w-90">
        <div className="mb-3 d-flex justify-content-between align-items-center border-bottom pb-3">
          <div className="d-flex align-items-center gap-4">
            <div>
              ...
              <div className="text-primary fw-bold fs-5">{estimateNo || "Loading..."}</div>
            </div>
          ...
          <div className="text-center flex-grow-1">
            <h3 className="fw-bold mb-0">Estimate Form</h3>
          </div>
          ...
          <div className="row mb-3">
            ...
            <div className="col-md-12 mb-2">
              ...
              <textarea
                ...
                value={estimate.billing_address}
                onChange={handleHeaderChange}
                className="form-control"
                ...
                <td className="fw-bold">
                  {
                    (() => {
                      ...
                      estimate.total_qty = qtyTotal; // ✅ store inside Estimate object
        ...
        <div className="border p-3 bg-light">
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={estimate.net_total.toFixed(2)}
              readOnly
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={estimate.discount_total.toFixed(2)}
              readOnly
              className="form-control form-control-sm text-end w-25"
            />
          </div>
          ...
          {estimate.igst ? (
            <div className="d-flex justify-content-between mb-2">
              <label>IGST:</label>
              <input
                value={estimate.gst_total.toFixed(2)}
                readOnly
                className="form-control form-control-sm text-end w-25"
              />
            </div>
            ...
            <>
              <div className="d-flex justify-content-between mb-2">
                <label>CGST:</label>
                <input
                  value={(estimate.gst_total / 2).toFixed(2)}
                  readOnly
                  className="form-control form-control-sm text-end w-25"
                />
              </div>
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={estimate.round_off.toFixed(2)}
              readOnly
              className="form-control form-control-sm text-end w-25"
            />
          </div>
        ...
        <div className="text-center mt-4">
          {selectedYear.is_closed ? (
            ...
            <>
              <button className="btn btn-success px-5 py-2 me-2 fw-bold shadow-sm rounded-pill" onClick={saveEstimate}>
                Save Estimate
              </button>
