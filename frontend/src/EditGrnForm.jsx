...
function EditGRNForm() {
  ...
  useEffect(() => {
    document.title = `Edit GRN No: ${id || ""} - TSL ERP`;
  }, [id]);
  ...
  useEffect(() => {
    axios.get(`${API}/templates`)
      .then((res) => {
        ...
        const grnTpl = templates.find((t) => t.template_name?.toLowerCase() === "grn");
        if (!grnTpl || !grnTpl.stock_action) {
          setGRN((prev) => ({ ...prev, template_id: null, stock_action: null }));
          setError("No valid 'grn' template found.");
          return;
        }
        setGRN((prev) => ({ ...prev, stock_action: grnTpl.stock_action, template_id: grnTpl.id }));
      })
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/grn/${id}`)
      .then((res) => {


        const raw = res.data;
        const data = raw.grn ? raw.grn : raw;       // supports both formats
  ...
  const addItem = () => setGRN((prev) => ({ ...prev, items: [...prev.items, { ...initialItem }] }));
  const removeItem = (index) => grn.items.length > 1 && setGRN((prev) => ({
    ...prev,
    items: prev.items.filter((_, i) => i !== index),
  }));

  const toggleIGST = () => setGRN((prev) => ({ ...prev, igst: !prev.igst }));

  // Save invoice
  const saveGRN = async () => {
    if (!grn.template_id) return setError("Cannot save grn: valid 'grn' template not selected.");
    if (!grn.items.length) return setError("Cannot save grn: No items added.");

    try {
      const payload = {
        ...grn,
        grn_date: inputToDMY(grn.grn_date),
        due_date: inputToDMY(grn.due_date),
      };
      if (id) {
        const res = await axios.put(`${API}/grn/${id}`, payload);


        setSuccess(res.data.message || "grn updated successfully!");
      } else {
        const res = await axios.post(`${API}/grn`, payload);
        setSuccess(res.data.message || "Grn saved successfully!");
      }
      setTimeout(() => navigate("/grnmy"), 1000);
    } catch (err) {
      console.error(err.response?.data || err.message);
      setError(err.response?.data?.error || "Error saving grn");
    }
  };

  const clearForm = () => window.confirm("Clear form?") && setGRN(getInitialState());
  ...
  return (
    <div className="container my-4">
      ...
      <div className="card shadow p-4">
        ...
        <h3 className="mb-3 text-center">GRN Form</h3>
      ...
      <span className="badge bg-success text-white">
        Outstanding: ₹ {outstanding.toFixed(2)}
            ...
            <div className="col-md-6 mb-2">
              ...
              <input type="text" name="ship_to" value={grn.ship_to} onChange={handleHeaderChange} className="form-control" />
            </div>

            <div className="col-md-6 mb-2">
              <label className="form-label">Sales Person</label>
              <select name="sales_person" value={grn.sales_person} onChange={handleHeaderChange} className="form-select">
            ...
            <div className="col-md-6 mb-2">
              ...
              <input type="text" name="mobile" value={grn.mobile} onChange={handleHeaderChange} className="form-control" />
            </div>
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                type="date"
                name="grn_date"
                value={grn.grn_date ? dmyToInput(grn.grn_date) : ""}
                onChange={(e) =>
                  setGRN({
                    ...grn,
                    grn_date: inputToDMY(e.target.value),
                  })
                }
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                type="date"
                name="due_date"
                value={grn.due_date ? dmyToInput(grn.due_date) : ""}
                onChange={(e) =>
                  setGRN({
                    ...grn,
            ...
            <div className="col-md-12 mb-2">
              ...
              <textarea name="billing_address" value={grn.billing_address} onChange={handleHeaderChange} className="form-control" />
            </div>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end mb-3">
          <div className="form-check">
            <input type="checkbox" className="form-check-input" checked={grn.igst} onChange={toggleIGST} />
              ...
              <tr>
                ...
                <td className="fw-bold">
                  {(() => {
                    ...
                    grn.total_qty = qtyTotal; // store in invoice
                    return qtyTotal;
                  })()}
                </td>
                <td colSpan="6"></td>
        ...
        <div className="border p-3 bg-light">
          <div className="d-flex justify-content-between mb-2">
            ...
            <input value={grn.discount_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          {grn.igst ? (
            <div className="d-flex justify-content-between mb-2">
              <label>IGST:</label>
              <input value={grn.gst_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
            </div>
          ) : (
            <>
              <div className="d-flex justify-content-between mb-2">
                <label>CGST:</label>
                <input value={(grn.gst_total / 2).toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
              </div>
              <div className="d-flex justify-content-between mb-2">
                <label>SGST:</label>
                <input value={(grn.gst_total / 2).toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input value={grn.net_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          <div className="d-flex justify-content-between mb-2">
            <label>Round Off:</label>
            <input value={grn.round_off.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>
        ...
        <div className="d-flex justify-content-end mt-3">
          ...
          <button onClick={saveGRN} className="btn btn-success">
            Save GRN
          </button>
        </div>
      </div>
