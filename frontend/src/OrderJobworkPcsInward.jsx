...
export default function OrderJobworkPcsInward() {
    ...
    const fetchInitial = useCallback(async () => {
        try {
            const [noRes, orderRes, empRes, procRes, chartRes, contractRes] = await Promise.all([
                axios.get(`${API}/pcs-inward/next-no/order`),
    ...
    const handleLoadOutward = async () => {
        ...
        try {
            const outRes = await axios.get(`${API}/pcs-outward/by-order/${encodeURIComponent(form.order_no)}`);
            ...
            if (items.length > 0) {
                ...
                if (form.size_chart_name) {
                    const groupedMap = {};
                    filtered.forEach(it => {
                        const styleKey = (it.style_name || it.item_name || "").trim().toLowerCase();
                        const colorKey = (it.style_color || it.color || "").trim().toLowerCase();
                        const key = `${styleKey}-${colorKey}`;
                        if (!groupedMap[key]) {
                            groupedMap[key] = {
                                ...it,
                                item_name: it.style_name || it.item_name || "",
                        ...
                        if (Object.keys(szData).length > 0) {
                            ...
                            groupedMap[key].sizes_data[it.size] = (groupedMap[key].sizes_data[it.size] || 0) + bPcs;
                        } else {
                            groupedMap[key].pcs += bPcs;
                        }
                    });
                    finalItems = Object.values(groupedMap).map(it => {
                        const rowTotal = Object.values(it.sizes_data).reduce((a, b) => a + b, 0) + (it.pcs || 0);
                        return {
                            ...it,
                            pcs: rowTotal,
                            max_pcs: rowTotal
                    ...
                    finalItems = items.map(it => {
                        const rowTotal = it.balanced_pcs || it.pending_pcs || it.pcs || it.qty || 0;
                        return { ...it, checked: true, max_pcs: rowTotal };
                    });
                }
                setLoadableItems(finalItems);
    ...
    const handleLoadPrevious = async () => {
        ...
        if (!form.previous_process) {
            toast("No previous process found in lifecycle.", "info");
            return;
        }
        try {
            let res;
            if (form.previous_process.toLowerCase() === "cutting") {
                res = await axios.get(`${API}/fabric-to-pcs-inward/order-process-items?order_no=${encodeURIComponent(form.order_no)}&process=Cutting`);
            } else {
                res = await axios.get(`${API}/pcs-inward/order-process-items?order_no=${encodeURIComponent(form.order_no)}&process=${encodeURIComponent(form.previous_process)}`);
            }
            ...
            if (res.data && res.data.length > 0) {
                ...
                if (form.size_chart_name) {
                    ...
                    items = Object.values(groupedMap).map(it => ({
                        ...it,
                        pcs: Object.values(it.sizes_data).reduce((a, b) => a + b, 0) + (it.pcs || 0),
                        max_pcs: Object.values(it.sizes_data).reduce((a, b) => a + b, 0) + (it.pcs || 0)
                    }));
                } else {
                ...
                setLoadableItems(items);
                setShowLoadModal(true);
            } else {
                toast(`No inward data found for ${form.previous_process}.`, "info");
            }
        } catch (err) {
            console.error("Error loading previous process items:", err);
            toast("Failed to fetch previous process items.", "danger");
        }
    };
    ...
    const handleApplyLoad = () => {
        ...
        const newRows = selected.map(it => {
            ...
            return {
                ...
                size: it.size || "",
                pcs: it.max_pcs || it.pcs || it.qty || "",
                max_pcs: it.max_pcs || it.pcs || it.qty || "",
                sizes_data: szData,
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const res = await axios.post(`${API}/pcs-inward`, payload);
            toast(`Saved! Inward No: ${res.data.inward_no}`, "success");
            setTimeout(() => navigate("/order-jobwork-pcs-inward-list"), 1500);
        } catch (err) {
    ...
    return (
        <div className="container-fluid mt-3 mb-5 px-3 px-md-4">
            ...
            <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom">
                <button className="btn btn-outline-secondary btn-sm rounded-pill px-3" onClick={() => navigate("/order-jobwork-pcs-inward-list")}>
                    <i className="bi bi-arrow-left me-1"></i>Back
                </button>
                <div>
                    <div className="d-flex align-items-center gap-2">
                        <span className="badge bg-success rounded-pill px-3" style={{ fontSize: "0.7rem", letterSpacing: "0.06em" }}>ORDER JOBWORK</span>
                        <span className="badge bg-success-subtle text-success rounded-pill px-3" style={{ fontSize: "0.7rem" }}>PCS INWARD</span>
                    </div>
                    <h4 className="fw-bold mb-0 mt-1 text-dark">
                        <i className="bi bi-box-arrow-in-down-left me-2 text-success"></i>Pcs Inward
                    </h4>
                </div>
            ...
            <div className="card border-0 shadow-sm rounded-4">
                <div className="card-body p-4">
                    ...
                    <div className="row g-3 mb-3">
                        ...
                        <div className="col-md-2">
                            ...
                            <div className="d-flex gap-2">
                                <select className={`form-select ${errors.process ? "is-invalid" : ""} fw-bold text-success`} name="process" value={form.process}
                                ...
                                {form.order_no && (
                                    <button className="btn btn-dark btn-sm rounded-3 px-3 italic" onClick={handleLoadOutward} disabled={form.rows.some(r => r.item_name || r.pcs)}>Load</button>
                                )}
                            </div>
                    ...
                    <div className="row g-3 mb-4 align-items-end">
                        <div className="col-md-3">
                            ...
                            <div className="btn-group w-100 shadow-sm" style={{ borderRadius: "10px", overflow: "hidden" }}>
                                ...
                                <label className="btn btn-outline-success py-2 fw-bold" htmlFor="wt_jobwork">Jobwork</label>
                                ...
                                <label className="btn btn-outline-success py-2 fw-bold" htmlFor="wt_contractor">Contractor</label>
                            </div>
                        </div>
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-2">
                        <h6 className="fw-bold mb-0 text-dark"><i className="bi bi-table me-2 text-success"></i>Pcs Items</h6>
                            ...
                            <thead className="table-dark">
                                {form.size_chart_name ? (
                                    <tr style={{ fontSize: "0.75rem" }}>
                                        ...
                                        <th style={{ width: "100px" }} className="text-center text-success fw-bold">TOTAL PCS</th>
                                    ...
                                    if (form.size_chart_name) {
                                        return (
                                            <tr key={idx}>
                                                ...
                                                <td className="text-center fw-bold text-success" style={{ backgroundColor: "#f0fff4" }}>{row.pcs || 0}</td>
                    ...
                    <div className="mt-4 pt-3 border-top d-flex gap-2">
                        ...
                        <button className="btn btn-outline-secondary rounded-pill px-4" onClick={() => navigate("/order-jobwork-pcs-inward-list")}>Cancel</button>
                    </div>
                </div>
            </div>
                    ...
                    <div className="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
                        <div className="modal-content border-0 shadow-lg rounded-4">
                            <div className="modal-header border-bottom-0 p-4 pb-0">
                                <h5 className="modal-title fw-bold"><i className="bi bi-download me-2 text-success"></i>Load from Outwards</h5>
                                        ...
                                        <tbody>
                                            {loadableItems.map((it, idx) => (
                                                <tr key={idx} style={{ cursor: "pointer" }} onClick={() => {
                                                    ...
                                                    <td className="fw-bold">{it.outward_no}</td>
                                                    <td><span className="badge bg-success-subtle text-success">{it.process}</span></td>
                                                    <td>{it.item_name}</td>
                                                    <td>{it.color}</td>
                                                    <td>{it.size}</td>
                            ...
                            <div className="modal-footer border-top-0 p-4 pt-0 gap-2">
                                ...
                                <button className="btn btn-success rounded-pill px-5 fw-bold" onClick={handleApplyLoad}>
