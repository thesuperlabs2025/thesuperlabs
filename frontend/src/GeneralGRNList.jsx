...
const GeneralGRNList = () => {
    ...
    const fetchGRNs = async () => {
        try {
            const res = await axios.get(`${API}/general-grn`);
            setGrns(res.data);
            setLoading(false);
        } catch (err) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this GRN?")) {
            try {
                await axios.delete(`${API}/general-grn/${id}`);
        ...
        <div className="container-fluid mt-4">
            ...
            <div className="card shadow border-0 rounded-4">
                <div className="card-header bg-white py-3 d-flex justify-content-between align-items-center border-bottom-0">
                    <h4 className="mb-0 fw-bold text-primary"><i className="bi bi-list-ul me-2"></i>General GRN List</h4>
                    <button className="btn btn-primary rounded-pill px-4" onClick={() => navigate('/general-grn-add')}>
                                ...
                                {loading ? (
                                    ...
                                    grns.map(grn => (
                                        <tr key={grn.id}>
                                            ...
                                            <td><span className="badge bg-secondary">{grn.grn_type}</span></td>
                                            <td className="fw-bold">{Number(grn.total_qty || 0).toFixed(3)}</td>
                                            <td className="text-end pe-4">
                                                <button className="btn btn-sm btn-outline-secondary me-2 rounded-circle" onClick={() => window.open(`${API}/printgeneralgrn/grn/${grn.id}`, '_blank')} title="Print">
                                                    <i className="bi bi-printer"></i>
                                                </button>
                                                <button className="btn btn-sm btn-outline-primary me-2 rounded-circle" onClick={() => navigate(`/general-grn-edit/${grn.id}`)} title="Edit">
