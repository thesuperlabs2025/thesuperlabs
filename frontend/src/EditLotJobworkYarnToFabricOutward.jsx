...
export default function EditLotJobworkYarnToFabricOutward() {
    const navigate = useNavigate();
    const { id } = useParams();
    const { toasts, add: toast, remove } = useToast();
    const [form, setForm] = useState({
        outward_date: new Date().toISOString().substring(0, 10),
        lot_no: "", lot_name: "", party_name: "", ref_no: "",
        process: "Yarn Dyeing",
    ...
    const handleSave = async () => {
        ...
        try {
            await axios.put(`${API}/yarn-dyeing-outward/${id}`, {
                outward_type: "lot", ...form, total_qty: totalQty, items: form.rows,
            });
            toast("Updated successfully!", "success");
            setTimeout(() => navigate("/lot-jobwork-yarn-to-fabric-list"), 1500);
        } catch (err) {
                ...
                <div className="card-body p-4">
                    <div className="row g-3 mb-3">
                        ...
                        <div className="col-md-2">
                            ...
                            <input type="text" className="form-control bg-light" name="lot_name" value={form.lot_name} readOnly={!!form.lot_no} onChange={handleHeader} placeholder="Auto-filled from lot" />
                        </div>
                        <div className="col-md-2">
