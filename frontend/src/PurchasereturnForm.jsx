...
const getInitialState = () => ({
  header: {
    ...
    gst: "",
    sales_person: "",
    purchase_return_date: new Date().toISOString().substring(0, 10),
...
function PurchasereturnForm() {
  ...
  useEffect(() => {
    axios.get(`${API}/templates`)
      .then((res) => {
        ...
        // ⭐ Find purchasereturn template
        const purchasereturnTpl = templates.find(
          (t) => t.template_name.toLowerCase() === "purchasereturn"
        );

        if (!purchasereturnTpl || !purchasereturnTpl.stock_action) {
          toast.error("No valid 'purchasereturn' template found.");
          setPurchasereturn((prev) => ({
            ...prev,
            template_id: null,
            stock_action: null,
        ...
        // ✅ Valid purchasereturn template
        setPurchasereturn((prev) => ({
          ...prev,
          stock_action: purchasereturnTpl.stock_action,
          template_id: purchasereturnTpl.id,
        }));
      })
      .catch((err) => {
  ...
  useEffect(() => {
    fetchPurchaseReturnNo();
  }, []);

  const fetchPurchaseReturnNo = async () => {
    try {
      const res = await axios.get(`${API}/purchasereturn/next-purchase-return-no`);
      setPurchaseReturnNo(res.data.purchaseReturnNo);
    } catch (err) {
      console.error("Failed to load Purchase Return number", err);
    }
  };
  ...
  const selectSupplier = async (supplier) => {
    ...
    setPurchasereturn((prev) => ({
      ...prev,
      supplier_name: Name, // for purchase returns we call it supplier_name
      ship_to: Name,
  ...
  const selectProduct = (index, product) => {
    const isDuplicate = purchasereturn.items.some(
  ...
  const removeItem = (index) => {
    if (purchasereturn.items.length > 1) {
      const newItems = purchasereturn.items.filter((_, i) => i !== index);
      setPurchasereturn((prev) => ({ ...prev, items: newItems }));
    }
  };
  ...
  // --- IGST toggle ---
  const toggleIGST = () =>
    setPurchasereturn((prev) => ({ ...prev, igst: !prev.igst }));

  // --- Save purchasereturn ---
  const savePurchasereturn = async () => {
    // 🛑 Stop if no template
    if (purchasereturn.template_id == null) {
      toast.error("Cannot save purchasereturn: valid 'purchasereturn' template not selected.");
      return;
    }

    if (!purchasereturn.items || purchasereturn.items.length === 0) {
      toast.error("Cannot save purchasereturn: No items added.");
      return;
    }
    ...
    const prDate = new Date(purchasereturn.purchase_return_date);
    ...
    if (prDate < startDate || prDate > endDate) {
      if (!window.confirm(`Warning: Purchase Return date is outside the selected Accounting Year (AY ${selectedYear.year_name}). Do you want to proceed?`)) {
        return;
      }
    }

    try {
      const res = await axios.post(`${API}/purchasereturn`, {
        customer_name: purchasereturn.customer_name,
        ship_to: purchasereturn.ship_to,
        mobile: purchasereturn.mobile,
        purchase_return_date: toDMY(purchasereturn.purchase_return_date),
        due_date: toDMY(purchasereturn.due_date),
        sales_person: purchasereturn.sales_person,
        billing_address: purchasereturn.billing_address,
        sku: purchasereturn.sku,
        total_qty: purchasereturn.total_qty,
        discount_total: purchasereturn.discount_total,
        grand_total: purchasereturn.grand_total,
        items: purchasereturn.items,
        template_id: purchasereturn.template_id,
        year_id: selectedYear.year_id
      });

      toast.success(res.data.message || "purchasereturn saved successfully!");
      ...
      console.error("Error saving purchase return:", err.response?.data || err.message);
      toast.error(err.response?.data?.error || "Error saving purchase return");
    }
  };
  ...
  // --- Clear form ---
  const clearForm = () => {
    if (window.confirm("Clear form?")) setPurchasereturn(getInitialState());
  };
      ...
      <div className="card shadow p-4 w-90">
        <div className="mb-3 d-flex justify-content-between align-items-center border-bottom pb-3">
          <div className="d-flex align-items-center gap-4">
            <div>
              ...
              <div className="text-primary fw-bold fs-5">{purchaseReturnNo || "Loading..."}</div>
            </div>
          ...
          <div className="text-center flex-grow-1">
            <h3 className="fw-bold mb-0">Purchase Return Form</h3>
          </div>
          ...
          <div className="row mb-3">
            ...
            <div className="col-md-6 mb-2 position-relative">
              ...
              {shipToSuggestions.length > 0 && (
                ...
                >
                  {shipToSuggestions
                    .filter(supp =>
                      (supp.SupplierName ?? supp.name ?? "").toLowerCase().includes((purchasereturn.ship_to || "").toLowerCase())
                    )
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                ...
                value={purchasereturn.purchase_return_date}
            ...
            <div className="col-md-6 mb-2 position-relative">
              ...
              <select
                ...
                value={purchasereturn.sales_person}
                onChange={handleHeaderChange}
                className="form-select"
              >
            ...
            <div className="col-md-12 mb-2">
              ...
              <textarea
                ...
                value={purchasereturn.billing_address}
                onChange={handleHeaderChange}
                className="form-control"
                ...
                <td className="fw-bold">
                  {
                    (() => {
                      ...
                      purchasereturn.total_qty = qtyTotal; // ✅ store inside purchase return object
        ...
        <div className="border p-3 bg-light">
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={purchasereturn.discount_total.toFixed(2)}
              readOnly
              className="form-control form-control-sm text-end w-25"
            />
          </div>
          ...
          {purchasereturn.igst ? (
            <div className="d-flex justify-content-between mb-2">
              <label>IGST:</label>
              <input
                value={purchasereturn.gst_total.toFixed(2)}
                readOnly
                className="form-control form-control-sm text-end w-25"
              />
            </div>
            ...
            <>
              <div className="d-flex justify-content-between mb-2">
                <label>CGST:</label>
                <input
                  value={(purchasereturn.gst_total / 2).toFixed(2)}
                  readOnly
                  className="form-control form-control-sm text-end w-25"
                />
              </div>
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={purchasereturn.round_off.toFixed(2)}
              readOnly
              className="form-control form-control-sm text-end w-25"
            />
          </div>
        ...
        <div className="text-center mt-4">
          {selectedYear.is_closed ? (
            ...
            <>
              <button className="btn btn-success px-5 py-2 me-2 fw-bold shadow-sm rounded-pill" onClick={savePurchasereturn}>
