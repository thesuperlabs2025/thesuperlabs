...
function PrintModal({ rec, items, onClose }) {
    ...
    const downloadPDF = () => {
        ...
        const opt = {
            margin: 0,
            filename: `Lot-Inward-${rec.inward_no || rec.id}.pdf`,
...
export default function LotJobworkYarnToFabricInwardList() {
    const navigate = useNavigate();
    const today = new Date().toISOString().substring(0, 10);
    const [allRecords, setAllRecords] = useState([]);
    const [loading, setLoading] = useState(true);
    const [selected, setSelected] = useState(new Set());
    const [filters, setFilters] = useState({ party: "", inward_no: "", lot_no: "", from_date: "", to_date: today });
    ...
    }, [allRecords, filters.party, filters.inward_no, filters.lot_no]);
        ...
        <div className="container-fluid py-4 px-4 h-100 d-flex flex-column">
            ...
            <div className="d-flex align-items-center justify-content-between mb-4 pb-3 border-bottom flex-wrap gap-3">
                <div className="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h4 className="fw-bold mb-0 text-dark">Lot Yarn to Fabric Inward</h4>
                        <div className="text-muted small">Manage and track your lot jobwork yarn dyeing inward receipts</div>
                    </div>
                </div>
                <div className="d-flex gap-2 bg-white p-1 rounded-pill shadow-sm border">
                    ...
                    <button className="btn btn-success rounded-pill px-4 btn-sm fw-bold shadow-sm" onClick={() => navigate("/lot-jobwork-yarn-to-fabric-inward")}>
            ...
            <div className="card border-0 shadow-sm rounded-4 mb-4">
                <div className="card-body p-3">
                    <div className="row g-2 align-items-center">
                        ...
                        <div className="col-md-2">
                            <button className="btn btn-outline-secondary btn-sm rounded-pill w-100" onClick={() => setFilters({ party: "", inward_no: "", lot_no: "", from_date: "", to_date: today })}>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr key={r.id} className={selected.has(r.id) ? "table-light" : ""}>
                                    ...
                                    <td>
                                        <div className="badge bg-primary-subtle text-primary me-1">{r.lot_no}</div>
                                    ...
                                    <td className="text-center">
                                        <button className="btn btn-outline-primary btn-sm rounded-circle me-1 border-0 shadow-sm" onClick={() => navigate(`/edit-lot-jobwork-yarn-to-fabric-inward/${r.id}`)} title="Edit Record">
