...
export default function CustomerBulkImport() {
    ...
    const customerHeaders = [
        ...
        "discount", "contact_type", "credit_limit", "credit_days", "agent_name", "tds", "tcs", "price_list",
    ...
    const handleFileChange = async (e) => {
        ...
        try {
            ...
            const res = await axios.get(`${API}/customers/names`).catch(err => {
            ...
            reader.onload = (evt) => {
                try {
                    ...
                    const data = rawData.map(item => {
                        ...
                        if (!normalized.name) {
                            normalized.name = item.Name || item.NAME || item["Customer Name"] || item["Customer"] || "";
                        }
                        return normalized;
                    });
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            for (const row of importData) {
                ...
                // (Note: dbErrors already contains these, we could skip them)
                const isAlreadyInDb = dbErrors.some(e => canonicalName(e) === name);
                if (!isAlreadyInDb) {
                    await axios.post(`${API}/customers`, row);
                    successCount++;
                }
            }
            alert(`Successfully added ${successCount} customers!`);
            navigate("/customermy");
        } catch (err) {
    ...
    return (
        <div className="container-fluid my-4 px-4 px-md-5" style={{ fontFamily: "'Segoe UI', sans-serif" }}>
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h3 className="fw-bold text-dark mb-0">Customer Bulk Import</h3>
                    <p className="text-muted small mb-0">Add many customers at once using an Excel file</p>
                </div>
            ...
            <div className="mt-5 pb-5">
                ...
                <ul className="small text-muted mb-0">
                    <li>Download our template to see how to list your customers.</li>
