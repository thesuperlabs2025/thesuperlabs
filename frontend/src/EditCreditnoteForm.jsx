...
function EditCreditnoteForm() {
  ...
  useEffect(() => {
    document.title = `Edit Credit Note No: ${id || ""} - TSL ERP`;
  }, [id]);
  ...
  useEffect(() => {
    axios.get(`${API}/templates`)
      .then((res) => {
        ...
        const creditnoteTpl = templates.find((t) => t.template_name?.toLowerCase() === "creditnote");
        if (!creditnoteTpl || !creditnoteTpl.stock_action) {
          setCreditnote((prev) => ({ ...prev, template_id: null, stock_action: null }));
          setError("No valid 'credit note' template found.");
          return;
        }
        setCreditnote((prev) => ({ ...prev, stock_action: creditnoteTpl.stock_action, template_id: creditnoteTpl.id }));
      })
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/creditnote/${id}`)
      .then((res) => {


        const raw = res.data;
        const data = raw.creditnote ? raw.creditnote : raw;       // supports both formats
      ...
      .catch((err) => {
        ...
        setError("Failed to load credit note data.");
      });
  }, [id]);
  ...
  const handleHeaderChange = (e) => {
    ...
    setCreditnote((prev) => ({ ...prev, [name]: value }));
  };
  ...
  const addItem = () => setCreditnote((prev) => ({ ...prev, items: [...prev.items, { ...initialItem }] }));
  const removeItem = (index) => creditnote.items.length > 1 && setCreditnote((prev) => ({
    ...prev,
    items: prev.items.filter((_, i) => i !== index),
  }));

  const toggleIGST = () => setCreditnote((prev) => ({ ...prev, igst: !prev.igst }));

  // Save creditnote
  const saveCreditnote = async () => {
    if (!creditnote.template_id) return setError("Cannot save credit note: valid 'credit note' template not selected.");
    if (!creditnote.items.length) return setError("Cannot save credit note: No items added.");

    try {
      const payload = {
        ...creditnote,
        credit_note_date: inputToDMY(creditnote.credit_note_date),
        due_date: inputToDMY(creditnote.due_date),
      };
      if (id) {
        const res = await axios.put(`${API}/creditnote/${id}`, payload);


        setSuccess(res.data.message || "credit note updated successfully!");
      } else {
        const res = await axios.post(`${API}/creditnote`, payload);
        setSuccess(res.data.message || "credit note saved successfully!");
      }
      setTimeout(() => navigate("/creditnotemy"), 1000);
    } catch (err) {
      console.error(err.response?.data || err.message);
      setError(err.response?.data?.error || "Error saving credit note");
    }
  };

  const clearForm = () => window.confirm("Clear form?") && setCreditnote(getInitialState());
  ...
  return (
    <div className="container my-4">
      ...
      <div className="card shadow p-4">
        ...
        <h3 className="mb-3 text-center">Credit Note Form</h3>
      ...
      <span className="badge bg-success text-white">
        Outstanding: ₹ {outstanding.toFixed(2)}
            ...
            <div className="col-md-6 mb-2">
              ...
              <input type="text" name="ship_to" value={creditnote.ship_to} onChange={handleHeaderChange} className="form-control" />
            </div>

            <div className="col-md-6 mb-2">
              <label className="form-label">Purchase Person</label>
              <select name="sales_person" value={creditnote.sales_person} onChange={handleHeaderChange} className="form-select">
            ...
            <div className="col-md-6 mb-2">
              ...
              <input type="text" name="mobile" value={creditnote.mobile} onChange={handleHeaderChange} className="form-control" />
            </div>
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                ...
                value={creditnote.credit_note_date ? dmyToInput(creditnote.credit_note_date) : ""}
                onChange={(e) =>
                  setCreditnote({
                    ...creditnote,
                    credit_note_date: inputToDMY(e.target.value),
                  })
                }
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                type="date"
                name="due_date"
                value={creditnote.due_date ? dmyToInput(creditnote.due_date) : ""}
                onChange={(e) =>
                  setCreditnote({
                    ...creditnote,
            ...
            <div className="col-md-12 mb-2">
              ...
              <textarea name="billing_address" value={creditnote.billing_address} onChange={handleHeaderChange} className="form-control" />
            </div>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end mb-3">
          <div className="form-check">
            <input type="checkbox" className="form-check-input" checked={creditnote.igst} onChange={toggleIGST} />
              ...
              <tr>
                ...
                <td className="fw-bold">
                  {(() => {
                    ...
                    creditnote.total_qty = qtyTotal; // store in invoice
                    return qtyTotal;
                  })()}
                </td>
                <td colSpan="6"></td>
        ...
        <div className="border p-3 bg-light">
          <div className="d-flex justify-content-between mb-2">
            ...
            <input value={creditnote.discount_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          {creditnote.igst ? (
            <div className="d-flex justify-content-between mb-2">
              <label>IGST:</label>
              <input value={creditnote.gst_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
            </div>
          ) : (
            <>
              <div className="d-flex justify-content-between mb-2">
                <label>CGST:</label>
                <input value={(creditnote.gst_total / 2).toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
              </div>
              <div className="d-flex justify-content-between mb-2">
                <label>SGST:</label>
                <input value={(creditnote.gst_total / 2).toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input value={creditnote.net_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          <div className="d-flex justify-content-between mb-2">
            <label>Round Off:</label>
            <input value={creditnote.round_off.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>
        ...
        <div className="d-flex justify-content-end mt-3">
          ...
          <button onClick={saveCreditnote} className="btn btn-success">
            Save Credit Note
          </button>
        </div>
      </div>
