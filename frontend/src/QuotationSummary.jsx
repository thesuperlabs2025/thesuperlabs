...
export default function QuotationSummary() {
    ...
    const handleSearch = async (f = filters) => {
        try {
            const res = await axios.get(`${API}/reports/quotation/summary`, { params: f });
            setData(res.data);
        } catch (err) { console.error(err); }
    };
    ...
    const handlePartyInput = async (e) => {
        ...
        setFilters({ ...filters, customer_name: value });
        if (value.length > 0) {
            try {
                const res = await axios.get(`${API}/customers?term=${value}`);
        ...
        <div className="container-fluid my-4">
            <style>{`
                ...
                @media print {
                    ...
                    .print-header { display: flex !important; justify-content: space-between; border-bottom: 2px solid #000; padding-bottom: 10px; }
                    .report-title-print { display: flex !important; justify-content: space-between; font-weight: bold; margin: 10px 0; border-bottom: 1px solid #ddd; }
            ...
            {companyProfile && (
                <div className="print-header">
                    <div className="text-start">
                        ...
                        <p>GST NO: {companyProfile.gst_no}<br />{companyProfile.address}<br />{companyProfile.email}</p>
                    </div>
            ...
            <div className="report-title-print">
                <span>Quotation Summary Report</span>
                <span>Period: {formatDate(filters.from_date)} To {formatDate(filters.to_date)}</span>
            </div>

            <div className="d-flex justify-content-between align-items-center mb-4 no-print">
                <h2 className="fw-bold">Quotation Summary Report</h2>
                <button className="btn btn-outline-danger" onClick={printReport}>Print PDF</button>
            </div>
                        ...
                        {partySuggestions.length > 0 && (
                            <div className="list-group position-absolute w-100 shadow" style={{ zIndex: 1000 }}>
                                {partySuggestions.map((s, i) => (
                                    <button key={i} className="list-group-item list-group-item-action" onClick={() => { setFilters({ ...filters, customer_name: s.name }); setPartySuggestions([]); }}>{s.name}</button>
                                ))}
                            </div>
                        )}
                    </div>
                    <div className="col-md-2">
                        <input type="text" className="form-control" placeholder="SKU" value={filters.sku} onChange={(e) => setFilters({ ...filters, sku: e.target.value })} />
                    </div>
                    <div className="col-md-2">
                ...
                <table className="table table-bordered table-hover shadow-sm">
                    ...
                    <tbody>
                        {data.map((item, i) => (
                            <tr key={i}>
                                <td>{item.tx_id}</td>
                                <td>{formatDate(item.tx_date)}</td>
                                <td>{item.customer_name}</td>
                                <td>{item.sku}</td>
                                <td>{item.qty}</td>
                                <td>{item.rate}</td>
                                <td>{item.gst_percent}%</td>
                                <td className="fw-bold">{parseFloat(item.item_total).toFixed(2)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
