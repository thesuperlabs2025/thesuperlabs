...
const initialRow = () => ({ yarn_name: "", counts: "", color: "", fabric_sku: "", fabric_name: "", gsm: "", dia: "", qty: "" });
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 10000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div className="bg-white rounded-4 shadow-lg w-100" style={{ maxWidth: 700, maxHeight: "80vh", overflow: "hidden", display: "flex", flexDirection: "column" }}>
                <div className="p-3 border-bottom d-flex justify-content-between align-items-center bg-light rounded-top-4">
                    <h6 className="mb-0 fw-bold">Load Outward Details ({no})</h6>
                    <button className="btn-close" onClick={onClose}></button>
                </div>
                <div className="p-3 overflow-auto" style={{ flex: 1 }}>
                    {loading ? <div className="text-center py-4"><span className="spinner-border spinner-border-sm text-primary"></span> Loading...</div> : (
                        list.length === 0 ? <div className="text-center py-4 text-muted">No outward records found for this {type} and process.</div> : (
                            <div className="table-responsive">
                                <table className="table table-hover table-sm align-middle small">
                                    ...
                                    <tbody>
                                        {list.map(r => (
                                            <tr key={r.id}>
                                                <td className="fw-bold text-primary">{r.outward_no}</td>
                                                <td>{new Date(r.outward_date).toLocaleDateString()}</td>
                                                <td>{r.party_name}</td>
                                                <td className="text-end fw-bold">{r.total_qty}</td>
                                                <td className="text-center">
                                                    <button className="btn btn-primary btn-sm rounded-pill px-3" onClick={() => onSelect(r)}>Load</button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
...
export default function OrderJobworkYarnToFabricInward() {
    ...
    const fetchInitial = useCallback(async () => {
        try {
            const [noRes, orderRes, empRes] = await Promise.all([
                axios.get(`${API}/yarn-dyeing-inward/next-no/order`),
                axios.get(`${API}/yarn-dyeing-inward/orders`),
    ...
    const handleLoadPrevious = async () => {
        ...
        try {
            // Check yarn_dyeing_outward first
            let res = await axios.get(`${API}/yarn-dyeing-outward/order-process-items`, {
                params: { order_no: form.order_no, process: form.previous_process }
            });

            // If no data, maybe check fabric_to_pcs_inward (for bridge processes like Cutting)
            if (!res.data || res.data.length === 0) {
                res = await axios.get(`${API}/fabric-to-pcs-inward/order-process-items`, {
                    params: { order_no: form.order_no, process: form.previous_process }
                });
            }

            if (res.data && res.data.length > 0) {
                const filteredItems = res.data.filter(it => {
                    const bQty = parseFloat(it.balanced_qty || it.pending_qty || it.qty || 0);
                    return bQty > 0;
                });

                if (filteredItems.length === 0) {
                    toast(`All quantities from ${form.previous_process} have already been received.`, "info");
                    return;
                }

                let loadedRows = [];
                filteredItems.forEach(it => {
                    const sizesData = it.sizes_data ? (typeof it.sizes_data === 'string' ? JSON.parse(it.sizes_data) : it.sizes_data) : null;
                    const bQty = parseFloat(it.balanced_qty || it.pending_qty || it.qty || 0);

                    if (sizesData && Object.keys(sizesData).length > 0) {
                        // Expand sizes_data into individual rows
                ...
                setForm(prev => ({ ...prev, rows: loadedRows }));
                toast(`Loaded ${loadedRows.length} items from ${form.previous_process} outward.`, "success");
            } else {
                toast(`No outward data found for ${form.previous_process} in this order.`, "info");
            }
        } catch (err) {
            console.error("Load previous error:", err);
            toast("Failed to load previous process data.", "danger");
        }
    };
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const res = await axios.post(`${API}/yarn-dyeing-inward`, {
                inward_type: "order", ...form,
                total_qty: totalQty, items: form.rows,
            });
            toast(`Saved! Inward No: ${res.data.inward_no}`, "success");
            setTimeout(() => navigate("/order-jobwork-yarn-to-fabric-inward-list"), 1500);
        } catch (err) {
    ...
    return (
        <div className="container-fluid mt-2 mb-3 px-2 px-md-3">
            ...
            {showLoadModal && <LoadOutwardModal type="order" no={form.order_no} process={form.process} onClose={() => setShowLoadModal(false)} onSelect={handleLoadSelect} />}

            {/* Page header */}
            <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom">
                <button className="btn btn-outline-secondary btn-sm rounded-pill px-3 py-1 fw-bold" onClick={() => navigate("/order-jobwork-yarn-to-fabric-inward-list")}>
                    <i className="bi bi-arrow-left me-1"></i>Back
                </button>
                <div>
                    <div className="d-flex align-items-center gap-2 mb-1">
                        <span className="badge bg-dark rounded-pill">ORDER JOBWORKS</span>
                        <span className="badge bg-success-subtle text-success rounded-pill">INWARD</span>
                    </div>
                    <h4 className="fw-extrabold mb-0 text-dark" style={{ letterSpacing: "-0.02em" }}>
                        <i className="bi bi-box-arrow-in-down-right me-2 text-success"></i>Yarn to Fabric Inward
                    </h4>
                </div>
                ...
                <div className="card-body p-4">
                    ...
                    <div className="row g-3 mb-3">
                        <div className="col-md-2">
                            ...
                            <input type="date" className="form-control fw-semibold" name="inward_date" value={form.inward_date} onChange={handleHeader} />
                        </div>
                        <div className="col-md-2">
                            <label className="form-label">ORDER NO <span className="text-danger">*</span></label>
                            ...
                            {form.order_no && (
                                <div className="d-flex flex-column gap-1 mt-1">
                                    <button className="btn btn-link btn-sm p-0 text-start fw-bold text-decoration-none" onClick={() => setShowLoadModal(true)}>
                                        <i className="bi bi-download me-1"></i>Load Details
                                    </button>
                                    {form.previous_process && (
                                        <button className="btn btn-link btn-sm p-0 text-start fw-bold text-decoration-none text-success" onClick={handleLoadPrevious}>
                                            <i className="bi bi-arrow-down-left-circle me-1"></i>Load from {form.previous_process}
                                        </button>
                                    )}
                                </div>
                            )}
                        ...
                        <div className="col-md-2">
                            <label className="form-label text-success">CURRENT PROCESS</label>
                        ...
                        <div className="col-md-2">
                            <label className="form-label">REF. / DC NO.</label>
                        ...
                        <div className="col-md-2">
                            ...
                            <input className="form-control fw-bold fs-5 text-center bg-light" readOnly value={dispTotal} style={{ color: "#16a34a" }} />
                        </div>
                    </div>
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-1">
                        ...
                        <button className="btn btn-success btn-sm rounded-pill px-2" onClick={addRow}>
                    ...
                    <div className="table-responsive rounded-2 border" ref={yarnRef}>
                        <table className="table table-hover table-bordered align-middle mb-0" style={{ fontSize: "0.85rem" }}>
                            <thead className="table-dark">
                                <tr>
                                    <th style={{ width: "18%" }}>YARN NAME</th>
                                    <th style={{ width: "8%" }}>COUNTS</th>
                                    <th style={{ width: "8%" }}>COLOR</th>
                                    {(form.process === "Knitting" || form.process === "Dyeing" || form.process === "Compacting") && (
                                        <>
                                            <th style={{ width: "10%" }}>FABRIC SKU</th>
                                            <th style={{ width: "12%" }}>FABRIC NAME</th>
                                            <th style={{ width: "6%" }}>GSM</th>
                                            <th style={{ width: "6%" }}>DIA</th>
                                        </>
                                    )}
                                    <th style={{ width: "10%" }}>QTY (KG)</th>
                                    <th className="text-center" style={{ width: "4%" }}>DEL</th>
                                </tr>
                            </thead>
                            <tbody>
                                {form.rows.map((row, idx) => (
                                    <tr key={idx}>
                                        ...
                                        {(form.process === "Knitting" || form.process === "Dyeing" || form.process === "Compacting") && (
                                            <>
                                                <td><input type="text" className="form-control form-control-sm" name="fabric_sku" value={row.fabric_sku} onChange={(e) => handleRowChange(idx, e)} placeholder="SKU" /></td>
                            ...
                            <tfoot>
                                <tr style={{ background: "#166534", color: "#fff" }}>
                                    <td colSpan={(form.process === "Knitting" || form.process === "Dyeing" || form.process === "Compacting") ? 7 : 3} className="text-end fw-bold">Total Qty (KG):</td>
                    ...
                    <div className="d-flex justify-content-end gap-2 pt-3 border-top">
                        ...
                        <button className="btn btn-secondary rounded-pill px-4" onClick={() => navigate("/order-jobwork-yarn-to-fabric-inward-list")}>
                            <i className="bi bi-x-lg me-1"></i>Cancel
                        </button>
                        <button className="btn btn-success rounded-pill px-5 fw-semibold" onClick={handleSave} disabled={saving}>
                            {saving ? <><span className="spinner-border spinner-border-sm me-2" />Saving...</> : <><i className="bi bi-check2-circle me-2"></i>Save Inward</>}
                        </button>
                    </div>
                </div>
            </div>
