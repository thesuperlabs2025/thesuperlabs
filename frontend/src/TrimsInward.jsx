...
const TrimsInward = () => {
    ...
    const [trimsSuggestions, setTrimsSuggestions] = useState([]);
    ...
    const fetchNextNo = async () => {
        try {
            const res = await axios.get(`${API}/trims-direct-inward/next-no`);
    ...
    const fetchInward = async (id) => {
        try {
            const res = await axios.get(`${API}/trims-direct-inward/${id}`);
            const data = res.data;
            setFormData({
                ...data,
                ...
                items: data.items.map(i => ({
                    ...
                    qty: i.qty,
                    sku: i.sku,
                    display_value: i.sku || i.trims_name
                }))
            });
        } catch (err) {
            console.error(err);
    ...
    const handleTrimsInput = async (index, e) => {
        const value = e.target.value;
        const newItems = [...formData.items];
        ...
        if (value.length > 1) {
            try {
                const res = await axios.get(`${API}/trims?term=${value}`);
                setTrimsSuggestions(res.data || []);
            } catch (err) { console.error(err); }
        } else {
            setTrimsSuggestions([]);
        }
    ...
    const selectTrims = (index, item) => {
        const newItems = [...formData.items];
        newItems[index].trims_name = item.trims_name;
        newItems[index].sku = item.trims_sku; // Assuming trims has SKU field
        newItems[index].display_value = item.trims_sku || item.trims_name;
        setFormData(prev => ({ ...prev, items: newItems }));
        setTrimsSuggestions([]);
        setActiveIndex(null);
    ...
    const handleSubmit = async () => {
        ...
        if (formData.items.some(i => !i.trims_name || !i.qty)) return toast.error("Please complete all items");
        ...
        try {
            ...
            if (location.state && location.state.id) {
                await axios.put(`${API}/trims-direct-inward/${location.state.id}`, payload);
                toast.success("Updated successfully");
            } else {
                await axios.post(`${API}/trims-direct-inward`, payload);
                toast.success("Created successfully");
            }
            navigate("/trims-inward-list");
        } catch (err) {
            console.error(err);
    ...
    const handleKeyDown = (e, index, type) => {
        if (type === 'supplier') {
            ...
            if (e.key === 'ArrowDown') {
                setHighlightedIndex(prev => Math.min(prev + 1, trimsSuggestions.length - 1));
            } else if (e.key === 'ArrowUp') {
                setHighlightedIndex(prev => Math.max(prev - 1, 0));
            } else if (e.key === 'Enter' && highlightedIndex >= 0) {
                selectTrims(index, trimsSuggestions[highlightedIndex]);
            }
        }
    };
    ...
    return (
        <div className="container-fluid py-4 bg-light min-vh-100">
            ...
            <div className="mx-auto" style={{ maxWidth: '1200px' }}>
                {/* Header Card */}
                <div className="mb-4 shadow-lg p-3" style={{
                    ...
                    <div className="d-flex justify-content-between align-items-center px-2">
                        <div className="d-flex align-items-center gap-4">
                            <div>
                                <h4 className="fw-bold mb-1">Trims Inward</h4>
                        ...
                        <div className="card border-0 shadow-sm h-100" style={{ borderRadius: '20px' }}>
                            <div className="card-body p-3">
                                <div className="d-flex justify-content-between align-items-center mb-3">
                                    <h6 className="fw-bold text-dark m-0">Inward Items (Trims)</h6>
                                    <button
                                    ...
                                    <table className="table table-borderless align-middle">
                                        <thead>
                                            <tr className="border-bottom">
                                                <th className="text-muted x-small fw-bold py-2" width="60%">TRIMS NAME / SKU</th>
                                                    ...
                                                    <td className="py-2">
                                                        <div className="position-relative">
                                                            <input
                                                                ...
                                                                onChange={(e) => handleTrimsInput(index, e)}
                                                                onKeyDown={(e) => handleKeyDown(e, index, 'trims')}
                                                                placeholder="Search SKU..."
                                                                autoComplete="off"
                                                            />
                                                            <AnimatePresence>
                                                                {activeIndex === index && trimsSuggestions.length > 0 && (
                                                                    <motion.ul
                                                                    ...
                                                                    >
                                                                        {trimsSuggestions.map((t, i) => (
                                                                            <li
                                                                                key={i}
                                                                                className={`list-group-item list-group-item-action border-0 ${highlightedIndex === i ? 'bg-indigo text-white' : ''}`}
                                                                                onClick={() => selectTrims(index, t)}
                                                                                onMouseEnter={() => setHighlightedIndex(i)}
                                                                                style={{ cursor: 'pointer', padding: '8px 12px', fontSize: '0.85rem' }}
                                                                            >
                                                                                <div className="d-flex justify-content-between">
                                                                                    <span className="fw-bold">{t.trims_sku}</span>
                                                                                    <small className={`${highlightedIndex === i ? 'text-white' : 'text-muted'} opacity-75`}>{t.trims_name}</small>
                                                                                </div>
                                                                            </li>
                                                                        ))}
                                                                    </motion.ul>
                            ...
                            <div className="card-footer bg-white border-0 p-3 pt-0 text-end">
                                <div className="d-flex justify-content-end gap-2">
                                    <button className="btn px-4 py-2 border-0 text-muted fw-bold" style={{ borderRadius: '14px', background: '#f8fafc', fontSize: '0.9rem' }} onClick={() => navigate("/trims-inward-list")}>Cancel</button>
                                    {selectedYear.is_closed ? (
