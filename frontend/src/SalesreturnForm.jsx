...
const getInitialState = () => ({
  header: {
    ...
    gst: "",
    sales_person: "",
    sales_return_date: new Date().toISOString().substring(0, 10),
...
const calculateItemTotal = (item) => {
  ...
  return { total: Number(total.toFixed(2)), discountAmount, gstAmount, afterDiscount };
};
...
function SalesreturnForm() {
  ...
  useEffect(() => {
    axios.get(`${API}/templates`)
      .then((res) => {
        ...
        // ⭐ Find salesreturn template
        const salesreturnTpl = templates.find(
          (t) => t.template_name.toLowerCase() === "salesreturn"
        );

        if (!salesreturnTpl || !salesreturnTpl.stock_action) {
          toast.error("No valid 'salesreturn' template found.");
          setSalesreturn((prev) => ({
            ...prev,
            template_id: null,
            stock_action: null,
        ...
        // ✅ Valid salesreturn template
        setSalesreturn((prev) => ({
          ...prev,
          stock_action: salesreturnTpl.stock_action,
          template_id: salesreturnTpl.id,
        }));
  ...
  const fetchSalesReturnNo = async () => {
    try {
      const res = await axios.get(`${API}/salesreturn/next-sales-return-no`);
      setSalesReturnNo(res.data.salesReturnNo);
    } catch (err) {
      console.error("Failed to load Sales Return number", err);
    }
  };
  ...
  const selectProduct = (index, product) => {
    const isDuplicate = salesreturn.items.some(
  ...
  const removeItem = (index) => {
    if (salesreturn.items.length > 1) {
      const newItems = salesreturn.items.filter((_, i) => i !== index);
      setSalesreturn((prev) => ({ ...prev, items: newItems }));
    }
  };
  ...
  // --- IGST toggle ---
  const toggleIGST = () =>
    setSalesreturn((prev) => ({ ...prev, igst: !prev.igst }));

  // --- Save salesreturn ---
  const saveSalesreturn = async () => {
    // 🛑 Stop if no template
    if (salesreturn.template_id == null) {
      toast.error("Cannot save salesreturn: valid 'salesreturn' template not selected.");
      return;
    }

    if (!salesreturn.items || salesreturn.items.length === 0) {
      toast.error("Cannot save salesreturn: No items added.");
      return;
    }
    ...
    const srDate = new Date(salesreturn.sales_return_date);
    const startDate = new Date(selectedYear.start_date);
    const endDate = new Date(selectedYear.end_date);
    if (srDate < startDate || srDate > endDate) {
      if (!window.confirm(`Warning: Sales Return date is outside the selected Accounting Year (AY ${selectedYear.year_name}). Do you want to proceed?`)) {
        return;
      }
    }

    try {
      const res = await axios.post(`${API}/salesreturn`, {
        customer_name: salesreturn.customer_name,
        ship_to: salesreturn.ship_to,
        mobile: salesreturn.mobile,
        sales_return_date: toDMY(salesreturn.sales_return_date),
        due_date: toDMY(salesreturn.due_date),
        sales_person: salesreturn.sales_person,
        billing_address: salesreturn.billing_address,
        sku: salesreturn.sku,
        total_qty: salesreturn.total_qty,
        discount_total: salesreturn.discount_total,
        grand_total: salesreturn.grand_total,
        items: salesreturn.items,
        template_id: salesreturn.template_id,
        year_id: selectedYear.year_id
      });

      toast.success(res.data.message || "salesreturn saved successfully!");
      ...
      console.error("Error saving salesreturn:", err.response?.data || err.message);
      toast.error(err.response?.data?.error || "Error saving salesreturn");
    }
  };
  ...
  // --- Clear form ---
  const clearForm = () => {
    if (window.confirm("Clear form?")) setSalesreturn(getInitialState());
  };
      ...
      <div className="card shadow p-4 w-90">
        <div className="mb-3 d-flex justify-content-between align-items-center border-bottom pb-3">
          <div className="d-flex align-items-center gap-4">
            <div>
              ...
              <div className="text-primary fw-bold fs-5">{salesReturnNo || "Loading..."}</div>
            </div>
          ...
          <div className="text-center flex-grow-1">
            <h3 className="fw-bold mb-0">Sales Return Form</h3>
          </div>
          ...
          <div className="row mb-3">
            ...
            <div className="col-md-6 mb-2 position-relative">
              ...
              {shipToSuggestions.length > 0 && (
                ...
                >
                  {shipToSuggestions
                    .filter(cust =>
                      (cust.customer_name ?? cust.name ?? "").toLowerCase().includes((salesreturn.ship_to || "").toLowerCase())
                    )
                    .slice(0, 10)
                    .map((cust, index) => (
                      <li
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                ...
                value={salesreturn.sales_return_date}
            ...
            <div className="col-md-6 mb-2 position-relative">
              ...
              <select
                name="sales_person"
                value={salesreturn.sales_person}
                onChange={handleHeaderChange}
                className="form-select"
              >
            ...
            <div className="col-md-12 mb-2">
              ...
              <textarea
                ...
                value={salesreturn.billing_address}
                onChange={handleHeaderChange}
                className="form-control"
                ...
                <td className="fw-bold">
                  {
                    (() => {
                      ...
                      salesreturn.total_qty = qtyTotal; // ✅ store inside salesreturn object
        ...
        <div className="border p-3 bg-light">
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={salesreturn.discount_total.toFixed(2)}
              readOnly
              className="form-control form-control-sm text-end w-25"
            />
          </div>
          ...
          {salesreturn.igst ? (
            <div className="d-flex justify-content-between mb-2">
              <label>IGST:</label>
              <input
                value={salesreturn.gst_total.toFixed(2)}
                readOnly
                className="form-control form-control-sm text-end w-25"
              />
            </div>
            ...
            <>
              <div className="d-flex justify-content-between mb-2">
                <label>CGST:</label>
                <input
                  value={(salesreturn.gst_total / 2).toFixed(2)}
                  readOnly
                  className="form-control form-control-sm text-end w-25"
                />
              </div>
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={salesreturn.round_off.toFixed(2)}
              readOnly
              className="form-control form-control-sm text-end w-25"
            />
          </div>
          ...
          <div className="d-flex justify-content-between">
            ...
            <input
              value={Number(salesreturn.grand_total.toFixed(2))}
              readOnly
        ...
        <div className="text-center mt-4">
          {selectedYear.is_closed ? (
            ...
            <>
              <button className="btn btn-success px-5 py-2 me-2 fw-bold shadow-sm rounded-pill" onClick={saveSalesreturn}>
                Save Sales Return
              </button>
