...
const YarnGRN = () => {
    ...
    const [items, setItems] = useState([
        { counts: "", yarn_name: "", yarn_sku: "", color: "", per_bag: "", per_bag_qty: "", qty: "" }
    ]);
        ...
        const fetchInitialData = async () => {
            try {
                const [grnRes, supRes, staffRes, orderRes, lotRes] = await Promise.all([
                    axios.get(`${API}/yarn-grn/next-no/gen`),
                ...
                if (location.state && location.state.po_data) {
                    ...
                    const fullPoRes = await axios.get(`${API}/yarn-po/${po.id}`);
    ...
    const loadDetails = () => {
        if (header.is_order_specific && header.order_id) {
            ...
            // Logic to fetch order items and populate 'items' state would go here
        } else if (header.is_lot_specific && header.lot_no) {
            toast.info(`Loaded details for Lot: ${header.lot_no}`);
            // Logic to fetch lot items would go here
        }
    };
    ...
    const addItem = () => {
        setItems([...items, { counts: "", yarn_name: "", yarn_sku: "", color: "", per_bag: "", per_bag_qty: "", qty: "" }]);
    };
    ...
    const saveGRN = async () => {
        ...
        if (header.po_no) {
            for (const item of items) {
                ...
                if (item.po_qty && Number(item.qty || 0) > Number(remaining.toFixed(3)) + 0.0001) {
                    return toast.error(`Quantity for ${item.yarn_name} exceeds remaining PO quantity (${remaining.toFixed(3)})`);
                }
            }
        }
        ...
        try {
            await axios.post(`${API}/yarn-grn`, {
                grn_no: grnNo,
                ...header,
                year_id: selectedYear.year_id,
                items
            });
            toast.success("Yarn GRN Created Successfully!");
            ...
            <div className="card shadow border-0 rounded-4">
                <div className="card-header bg-white py-3 border-bottom shadow-sm">
                    <div className="d-flex justify-content-between align-items-center">
                        ...
                        <div className="text-center flex-grow-1">
                            <h4 className="fw-bold mb-0 text-uppercase tracking-wider">Yarn GRN</h4>
                        </div>
                ...
                <div className="card-body p-4">
                    ...
                    <div className="card bg-light border-0 p-3 mb-4 rounded-3">
                        ...
                        <style>{` .fade-in { animation: fadeIn 0.5s; } @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } } `}</style>
                    </div>
                    ...
                    <div className="d-flex justify-content-end gap-2">
                        <button className="btn btn-secondary rounded-pill px-4" onClick={() => navigate('/yarn-grn-list')}>Cancel</button>
                        {selectedYear.is_closed ? (
                            ...
                            <button className="btn btn-success rounded-pill px-4 fw-bold shadow-sm" onClick={saveGRN}>
                                <i className="bi bi-check-lg me-2"></i>Save Yarn GRN
                            </button>
                        )}
                    </div>
                </div>
