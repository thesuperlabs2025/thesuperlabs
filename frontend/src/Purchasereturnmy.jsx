...
function Purchasereturnmy() {
  const [purchasereturn, setPurchasereturn] = useState([]);
  const [loading, setLoading] = useState(true);

  const [selectedIds, setSelectedIds] = useState([]);
  const [selectAll, setSelectAll] = useState(false);
  const [purchasrereturnPriv, setPurchasereturnPriv] = useState(null);
  ...
  //     if (filters.purchase_return_date) params.purchasereturn_date = filters.purchase_return_date;
  //     if (filters.due_date) params.due_date = filters.due_date;
  //     if (filters.status) params.status = filters.status;

  //     const res = await axios.get(`${API}/purchasereturn`, { params });
  ...
  //     alert("Error loading purchase return");
  //   } finally {
  //     setLoading(false);
  //   }
  ...
  const fetchPurchasereturn = useCallback(async () => {
    ...
    try {
      const params = {
        ...
        sort_by: sortBy,
        order,
        usertype_id: user.usertype_id, // ✅ added (same as Credit Note)
      };

      if (filters.customer_name) params.customer_name = filters.customer_name;
      if (filters.mobile) params.mobile = filters.mobile;
      if (filters.sales_person) params.sales_person = filters.sales_person;
      if (filters.purchase_return_date)
        params.purchase_return_date = filters.purchase_return_date; // ✅ unchanged logic
      if (filters.due_date) params.due_date = filters.due_date;
      if (filters.status) params.status = filters.status;
      ...
      // ✅ same pagination fallback logic as Credit Note / Estimate
      setPagination(
        res.data.pagination || {
          page: 1,
          limit: 20,
      ...
      setPurchasereturnPriv(res.data.privileges || {});

      setSelectedIds([]);
      setSelectAll(false);
    } catch (err) {
      console.error("Fetch Purchase Return Error:", err);
      alert(err.response?.data?.message || "Error loading Purchase Return");
    } finally {
      setLoading(false);
    }
  ...
  const handleConvert = () => {
    if (selectedIds.length === 0) {
      alert("Please select at least one Purchase Return to convert.");
      return;
    }

    const selectedReturns = purchasereturn.filter(p => selectedIds.includes(p.id));
    const sameCustomer = selectedReturns.every(p => p.customer_name === selectedReturns[0].customer_name);

    if (!sameCustomer) {
      alert("Please select Purchase Returns from the same customer for conversion.");
      return;
    }
  ...
  const confirmConvert = async () => {
    ...
    try {
      const fullDetails = await Promise.all(
        selectedIds.map(id => axios.get(`${API}/purchasereturn/${id}`).then(res => res.data))
      );
      ...
      alert("Failed to fetch purchase return details for conversion.");
    }
  };
  ...
  const handleFilterChange = (e) => {
    ...
    if (name === "customer_name") {
      if (!value) {
        ...
        const suggestions = purchasereturn
          .filter((purchasereturn) =>
            purchasereturn.customer_name?.toLowerCase().includes(value.toLowerCase())
          )
          .slice(0, 5);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API}/purchasereturn/${id}`);
      alert("Deleted");
      fetchPurchasereturn();
    } catch (err) {
      console.error(err);
  ...
  const handleSelectAll = () => {
    if (!selectAll) {
      setSelectedIds(purchasereturn.map((purchasereturn) => purchasereturn.id));
    } else {
      setSelectedIds([]);
    }
    setSelectAll((s) => !s);
  ...
  const handleDeleteSelected = async () => {
    if (selectedIds.length === 0) {
      alert("Select at least one purchase return");
      return;
    }
    if (!window.confirm("Delete selected purchase return?")) return;
    try {
    ...
    <div className="container-fluid py-4 px-md-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="fw-bold text-primary mb-0">
          <i className="bi bi-arrow-left-right me-2"></i>Purchase Return
        </h2>
        <div className="d-flex align-items-center gap-3">
          ...
          <div className="btn-group shadow-sm">
            {purchasrereturnPriv?.can_add === 1 && (
              <button className="btn btn-primary d-flex align-items-center fw-bold px-4 shadow-sm" onClick={() => navigate("/purchasereturnform")}>
                <i className="bi bi-plus-lg me-2"></i>New
              </button>
            )}
            {purchasrereturnPriv?.can_delete === 1 && (
        ...
        <div className="card-body p-4">
          <div className="row g-3">
            ...
            <div className="col-md-2">
              ...
              <input type="date" name="purchase_return_date" className="form-control" value={filters.purchase_return_date} onChange={handleFilterChange} />
            </div>
            <div className="col-md-3">
        ...
        <div className="card shadow-sm border-0">
              ...
              <table className="table table-hover table-striped mb-0 align-middle" style={{ fontSize: '16px' }}>
                <thead className="table-dark small">
                  <tr>
                    ...
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("id")}>
                      RETURN NO {sortBy === "id" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("purchase_return_date")}>
                      DATE {sortBy === "purchase_return_date" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                ...
                <tbody>
                  {purchasereturn.length === 0 ? (
                    <tr><td colSpan="8" className="text-center py-5 text-muted fw-bold">No Returns found.</td></tr>
                  ) : (
                    purchasereturn.map((p) => (
                      <tr key={p.id}>
                        ...
                        <td className="small">{p.purchase_return_date}</td>
                        <td className="fw-bold text-dark">{p.customer_name}</td>
                        <td className="text-end fw-bold">₹{p.grand_total}</td>
                        <td className="text-center small hide-on-mobile">{p.total_qty}</td>
                        <td className="hide-on-mobile"><span className="badge bg-light text-dark fw-normal border">{p.sales_person}</span></td>
                        <td className="text-end pe-4 no-print">
                          <div className="d-flex justify-content-end gap-2">
                            <button className="btn btn-outline-secondary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                              ...
                              onClick={() => window.open(`${API}/printpurchasereturn/purchasereturn/${p.id}`, "_blank")}>
                              <i className="bi bi-printer-fill"></i>
                            </button>
                            {purchasrereturnPriv?.can_update === 1 && (
                              <button className="btn btn-outline-primary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                                ...
                                onClick={() => navigate(`/Editpurchasereturnform/${p.id}`)}>
          ...
          <div className="card-footer bg-white py-3 border-0 d-flex justify-content-between align-items-center mb-5 no-print">
            <span className="text-muted small">
              Page {pagination?.page || 1} of {pagination?.pages || 1} &bull; Total {pagination?.total || 0} Purchase Returns
            </span>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg">
              <div className="modal-header bg-primary text-white border-0">
                <h5 className="modal-title fw-bold">Convert Purchase Return(s)</h5>
                <button type="button" className="btn-close btn-close-white" onClick={() => setShowConvertModal(false)}></button>
              </div>
              <div className="modal-body p-4">
                <p className="text-muted small mb-4">Select the target module to convert selected Purchase Returns into. Data like customer info and items will be carried over.</p>
                <div className="mb-3">
...
export default Purchasereturnmy;
