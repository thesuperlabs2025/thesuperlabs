...
function PrintModal({ rec, items, onClose }) {
    ...
    const downloadPDF = () => {
        ...
        const opt = {
            margin: 0,
            filename: `Int-Lot-Outward-${rec.outward_no || rec.id}.pdf`,
...
export default function InternalLotJobworkYarnDyeingList() {
    const navigate = useNavigate();
    const today = new Date().toISOString().substring(0, 10);
    const monthStart = today.substring(0, 8) + "01";

    const [filters, setFilters] = useState({ from_date: monthStart, to_date: today, party_name: "", outward_no: "", internal_lot_no: "" });
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const p = new URLSearchParams({ type: "internal", from_date: filters.from_date, to_date: filters.to_date });
    ...
    const records = useMemo(() => allRecords.filter(r => {
        ...
        if (filters.internal_lot_no && !(r.internal_lot_no || "").toLowerCase().includes(filters.internal_lot_no.toLowerCase())) return false;
        return true;
    ...
    const clearFilters = () => setFilters({ from_date: monthStart, to_date: today, party_name: "", outward_no: "", internal_lot_no: "" });
        ...
        <div className="container-fluid mt-3 mb-5 px-3 px-md-4">
            ...
            <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom flex-wrap">
                ...
                <div className="ms-auto d-flex gap-2 flex-wrap">
                    ...
                    <button className="btn btn-outline-primary rounded-pill px-3 btn-sm" onClick={() => navigate("/internal-lot-yarn-to-fabric-outward")}><i className="bi bi-plus-lg me-1"></i>New Outward</button>
                    <button className="btn btn-success rounded-pill px-3 btn-sm" onClick={() => navigate("/internal-lot-yarn-to-fabric-inward")}><i className="bi bi-box-arrow-in-down-right me-1"></i>New Inward</button>
                </div>
            </div>
            ...
            <div className="card border-0 shadow-sm rounded-4 mb-4">
                <div className="card-body p-3">
                    <div className="row g-2 align-items-end">
                        ...
                        <div className="col-md-2 col-6"><label className="form-label fw-semibold text-muted mb-1" style={{ fontSize: "0.72rem" }}>OUTWARD NO.</label><input type="text" className="form-control form-control-sm" name="outward_no" value={filters.outward_no} onChange={handleFilter} placeholder="e.g. YD-INT-0001" /></div>
                        <div className="col-md-2 col-6"><label className="form-label fw-semibold text-muted mb-1" style={{ fontSize: "0.72rem" }}>INTERNAL LOT NO.</label><input type="text" className="form-control form-control-sm" name="internal_lot_no" value={filters.internal_lot_no} onChange={handleFilter} placeholder="Filter internal lot..." /></div>
                        ...
                        <table className="table table-hover align-middle mb-0" style={{ fontSize: "0.83rem" }}>
                            <thead className="table-dark">
                                <tr>
                                    ...
                                    <th>OUTWARD NO.</th><th>DATE</th><th>INTERNAL LOT NO.</th><th>INTERNAL LOT NAME</th>
                                    <th>SUPPLIER</th><th>REF. NO.</th><th className="text-center">QTY (KG)</th><th className="text-center">ACTIONS</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                {!loading && records.map((r, idx) => (
                                    <React.Fragment key={r.id}>
                                        <tr className={selected.has(r.id) ? "table-primary" : ""}>
                                            ...
                                            <td className="fw-semibold">{r.internal_lot_no || "—"}</td>
                                            <td className="text-muted" style={{ maxWidth: 130, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.internal_lot_name || "—"}</td>
                                            <td>{r.party_name || "—"}</td>
                                            ...
                                            <td className="text-center">
                                                <div className="d-flex gap-1 justify-content-center">
                                                    ...
                                                    <button className="btn btn-sm rounded-circle" style={bs("#eff6ff", "#3b82f6")} title="Edit" onClick={() => navigate(`/edit-internal-lot-yarn-to-fabric-outward/${r.id}`)}><i className="bi bi-pencil"></i></button>
