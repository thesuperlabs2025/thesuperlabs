...
export default function OrderJobworkYarnToFabricReturn() {
    ...
    const fetchInitial = useCallback(async () => {
        try {
            const [noRes, orderRes, empRes] = await Promise.all([
                axios.get(`${API}/yarn-dyeing-return/next-no/order`),
                axios.get(`${API}/yarn-dyeing-inward/orders`), // Reuse inward orders fetch
    ...
    const loadPending = async () => {
        if (!form.order_no) { toast("Select Order No first", "warning"); return; }

        try {
            const res = await axios.get(`${API}/yarn-dyeing-return/pending-balance?type=order&no=${form.order_no}&process=${form.process}`);
            const items = (res.data || []).filter(it => (parseFloat(it.qty) || 0) > 0);
            if (items.length === 0) {
                toast("No pending balance items found for this Order & Process.", "info");
                return;
            }
            setForm(prev => ({
                ...prev,
            ...
            toast(`Loaded ${items.length} pending items (Out - In - Ret).`, "success");
        } catch (e) {
            toast("Failed to load pending items", "danger");
        }
    };
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const res = await axios.post(`${API}/yarn-dyeing-return`, {
                return_type: "order", ...form,
                total_qty: totalQty, items: form.rows,
            });
            toast(`Saved! Return No: ${res.data.return_no}`, "success");
            setTimeout(() => navigate("/order-jobwork-yarn-to-fabric-return-list"), 1500);
        } catch (err) {
    ...
    return (
        <div className="container-fluid mt-2 mb-3 px-2 px-md-3">
            ...
            <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom">
                <button className="btn btn-outline-secondary btn-sm rounded-pill px-3 py-1 fw-bold" onClick={() => navigate("/order-jobwork-yarn-to-fabric-return-list")}>
                    <i className="bi bi-arrow-left me-1"></i>Back
                </button>
                <div>
                    <div className="d-flex align-items-center gap-2 mb-1">
                        <span className="badge bg-dark rounded-pill">ORDER JOBWORKS</span>
                        <span className="badge bg-danger-subtle text-danger rounded-pill">RETURN</span>
                    </div>
                    <h4 className="fw-extrabold mb-0 text-dark" style={{ letterSpacing: "-0.02em" }}>
                        <i className="bi bi-box-arrow-left me-2 text-danger"></i>Jobwork Return
                    </h4>
                </div>
                ...
                <div className="card-body p-4">
                    ...
                    <div className="row g-3 mb-3">
                        <div className="col-md-2">
                            ...
                            <input type="date" className="form-control fw-semibold" name="return_date" value={form.return_date} onChange={handleHeader} />
                        </div>
                        <div className="col-md-2">
                        ...
                        <div className="col-md-2">
                            <label className="form-label text-danger">CURRENT PROCESS</label>
                        ...
                        <div className="col-md-1">
                            ...
                            <input type="text" className="form-control" name="ref_no" value={form.ref_no} onChange={handleHeader} placeholder="Ref" />
                        </div>
                        <div className="col-md-2 d-flex align-items-end">
                            <button className="btn btn-outline-danger btn-sm rounded-pill w-100 fw-bold py-2" onClick={loadPending}>
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-1">
                        ...
                        <button className="btn btn-danger btn-sm rounded-pill px-2" onClick={addRow}>
                    ...
                    <button className="btn btn-outline-danger btn-sm rounded-pill w-100 mt-1 mb-3" style={{ borderStyle: "dashed" }} onClick={addRow}>
                    ...
                    <div className="d-flex justify-content-end gap-2 pt-2 border-top">
                        ...
                        <button className="btn btn-secondary rounded-pill px-4" onClick={() => navigate("/order-jobwork-yarn-to-fabric-return-list")}>
                            <i className="bi bi-x-lg me-1"></i>Cancel
                        </button>
                        <button className="btn btn-danger rounded-pill px-5 fw-semibold" onClick={handleSave} disabled={saving}>
                            {saving ? <><span className="spinner-border spinner-border-sm me-2" />Saving...</> : <><i className="bi bi-check2-circle me-2"></i>Save Return</>}
                        </button>
                    </div>
                </div>
            </div>
