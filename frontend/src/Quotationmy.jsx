...
function Quotationmy() {
  const [quotation, setQuotation] = useState([]);
  const [loading, setLoading] = useState(true);

  const [selectedIds, setSelectedIds] = useState([]);
  const [selectAll, setSelectAll] = useState(false);
  const [quotationPriv, setQuotationPriv] = useState(null);
  ...
  //     if (filters.quotation_date) params.quotation_date = filters.quotation_date;
  //     if (filters.due_date) params.due_date = filters.due_date;
  //     if (filters.status) params.status = filters.status;

  //     const res = await axios.get(`${API}/quotation`, { params });
  ...
  const fetchQuotation = useCallback(async () => {
    ...
    try {
      const params = {
        ...
        sort_by: sortBy,
        order,
        usertype_id: user.usertype_id, // ✅ same as invoice
      };
      ...
      // ✅ same pagination fallback logic
      setPagination(
        res.data.pagination || {
          page: 1,
          limit: 20,
      ...
      // ✅ privilege support (same as invoice)
      setQuotationPriv(res.data.privileges || {});

      setSelectedIds([]);
      setSelectAll(false);
    } catch (err) {
      console.error("Fetch Quotation Error:", err);
      alert(err.response?.data?.message || "Error loading Quotation");
    } finally {
      setLoading(false);
    }
  }, [API, page, limit, sortBy, order, filters, user.usertype_id]);

  const handleConvert = () => {
    if (selectedIds.length === 0) {
      alert("Please select at least one Quotation to convert.");
      return;
    }

    const selectedQuos = quotation.filter(q => selectedIds.includes(q.id));
    const sameCustomer = selectedQuos.every(q => q.customer_name === selectedQuos[0].customer_name);

    if (!sameCustomer) {
      alert("Please select Quotations from the same customer for conversion.");
      return;
    }
  ...
  const confirmConvert = async () => {
    ...
    try {
      const fullDetails = await Promise.all(
        selectedIds.map(id => axios.get(`${API}/quotation/${id}`).then(res => res.data))
      );

      const first = fullDetails[0];
      const combinedItems = fullDetails.flatMap(q => q.items);
      ...
      alert("Failed to fetch Quotation details for conversion.");
    }
  };
    ...
    if (name === "customer_name") {
      if (!value) {
        ...
        const suggestions = quotation
          .filter((quo) =>
            quo.customer_name?.toLowerCase().includes(value.toLowerCase())
          )
          .slice(0, 5);
  ...
  const handleSelectAll = () => {
    if (!selectAll) {
      setSelectedIds(quotation.map((inv) => inv.id));
    } else {
      setSelectedIds([]);
    }
    setSelectAll((s) => !s);
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!window.confirm("Delete selected quotation?")) return;
    try {
    ...
    <div className="container-fluid py-4 px-md-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="fw-bold text-primary mb-0">
          <i className="bi bi-file-earmark-spreadsheet-fill me-2"></i>Quotation Management
        </h2>
        <div className="d-flex align-items-center gap-3">
          ...
          <div className="btn-group shadow-sm">
            {quotationPriv?.can_add === 1 && (
              <button className="btn btn-primary d-flex align-items-center fw-bold px-4 shadow-sm" onClick={() => navigate("/quotationform")}>
                <i className="bi bi-plus-lg me-2"></i>New
              </button>
            )}
            {quotationPriv?.can_delete === 1 && (
      ...
      <div className="card shadow-sm border-0 mb-4">
        <div className="card-body p-4">
          <div className="row g-3">
            ...
            <div className="col-md-2">
              <label className="form-label fw-bold small text-muted">Quotation Date</label>
              <input type="date" name="quotation_date" className="form-control" value={filters.quotation_date} onChange={handleFilterChange} />
            </div>
            <div className="col-md-3">
      ...
      {quotationPriv?.can_view ? (
        <div className="card shadow-sm border-0">
                ...
                <thead className="table-dark small">
                  <tr>
                    ...
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("id")}>
                      QUO NO {sortBy === "id" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("quotation_date")}>
                      DATE {sortBy === "quotation_date" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                    ...
                    <th className="fw-bold py-3 text-center">QTY</th>
                    <th className="fw-bold py-3">STAFF</th>
                    <th className="fw-bold py-3 text-end pe-4" width="160">ACTIONS</th>
                  </tr>
                </thead>
                <tbody>
                  {quotation.length === 0 ? (
                    <tr><td colSpan="8" className="text-center py-5 text-muted fw-bold">No quotations found.</td></tr>
                  ) : (
                    quotation.map((quo) => (
                      <tr key={quo.id}>
                        <td className="ps-4"><input className="form-check-input" type="checkbox" checked={selectedIds.includes(quo.id)} onChange={() => handleCheckboxChange(quo.id)} /></td>
                        <td className="fw-bold text-dark">#{quo.id}</td>
                        <td className="small">{quo.quotation_date}</td>
                        <td className="fw-bold text-dark">{quo.customer_name}</td>
                        <td className="text-end fw-bold">₹{quo.grand_total}</td>
                        <td className="text-center small">{quo.total_qty}</td>
                        <td><span className="badge bg-light text-dark fw-normal border">{quo.sales_person}</span></td>
                        <td className="text-end pe-4">
                          <div className="d-flex justify-content-end gap-2">
                            <button className="btn btn-outline-secondary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                              ...
                              onClick={() => window.open(`${API}/printquotation/quotation/${quo.id}`, "_blank")}>
                              <i className="bi bi-printer-fill"></i>
                            </button>
                            {quotationPriv?.can_update === 1 && (
                              <button className="btn btn-outline-primary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                                ...
                                onClick={() => navigate(`/Editquotationform/${quo.id}`)}>
          ...
          <div className="card-footer bg-white py-3 border-0 d-flex justify-content-between align-items-center mb-5">
            <span className="text-muted small">
              Page {pagination?.page || 1} of {pagination?.pages || 1} &bull; Total {pagination?.total || 0} Quotations
            </span>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg">
              <div className="modal-header bg-primary text-white border-0">
                <h5 className="modal-title fw-bold">Convert Quotation(s)</h5>
                <button type="button" className="btn-close btn-close-white" onClick={() => setShowConvertModal(false)}></button>
              </div>
              <div className="modal-body p-4">
                <p className="text-muted small mb-4">Select the target module to convert selected Quotations into. Data like customer info and items will be carried over.</p>
                <div className="mb-3">
