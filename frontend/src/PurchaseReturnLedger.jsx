...
export default function PurchaseReturnLedger() {
    const [data, setData] = useState([]);
    const [partySuggestions, setPartySuggestions] = useState([]);
    const [companyProfile, setCompanyProfile] = useState(null);
    const [filters, setFilters] = useState({ customer_name: "", from_date: "", to_date: "" });
    ...
    const handleSearch = async (f = filters) => { try { const res = await axios.get(`${API}/reports/purchasereturn/ledger`, { params: f }); setData(res.data); } catch (err) { console.error(err); } };
    ...
    return (
        <div className="container-fluid my-4">
            ...
            <div className="report-title-print"><span>Purchase Return Ledger Report</span><span>Period: {formatDate(filters.from_date)} To {formatDate(filters.to_date)}</span></div>
            <div className="d-flex justify-content-between align-items-center mb-4 no-print"><h2 className="fw-bold">Purchase Return Ledger Report</h2><button className="btn btn-outline-danger" onClick={printReport}>Print PDF</button></div>
            <div className="card p-3 mb-4 no-print shadow-sm">
                <div className="row g-2">
                    <div className="col-md-4 position-relative">
                        <input type="text" className="form-control" placeholder="Supplier Name" value={filters.customer_name} onChange={handlePartyInput} />
            ...
            <div className="table-responsive">
                <table className="table table-bordered table-hover shadow-sm">
                    ...
                    <tbody>{data.map((item, i) => (<tr key={i}><td>{item.id}</td><td>{item.customer_name}</td><td>{formatDate(item.tx_date)}</td><td>{item.total_qty}</td><td>{parseFloat(item.gst_total).toFixed(2)}</td><td className="fw-bold">{parseFloat(item.grand_total).toFixed(2)}</td></tr>))}</tbody>
                </table>
            </div>
        </div>
    );
