...
const getForm = () => ({
    inward_date: today, internal_lot_no: "", internal_lot_name: "",
    party_name: "", ship_to: "", process: "Yarn Dyeing", ref_no: "", staff_name: "", staff_remarks: "", remarks: "",
    rows: [initialRow()]
});

function LoadOutwardModal({ type, no, process, onClose, onSelect }) {
    const [list, setList] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        axios.get(`${API}/yarn-dyeing-inward/pending-outward?type=${type}&no=${no}&process=${process}`)
    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 10000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div className="bg-white rounded-4 shadow-lg w-100" style={{ maxWidth: 700, maxHeight: "80vh", overflow: "hidden", display: "flex", flexDirection: "column" }}>
                <div className="p-3 border-bottom d-flex justify-content-between align-items-center bg-light rounded-top-4">
                    <h6 className="mb-0 fw-bold small">Load Outward Details ({no})</h6>
                    <button className="btn-close" onClick={onClose}></button>
                </div>
                <div className="p-3 overflow-auto" style={{ flex: 1 }}>
                    {loading ? <div className="text-center py-4"><span className="spinner-border spinner-border-sm text-success"></span> Loading...</div> : (
                        list.length === 0 ? <div className="text-center py-4 text-muted small">No outward records found for this {type} and process.</div> : (
                            <div className="table-responsive">
                                <table className="table table-hover table-sm align-middle small">
                                    ...
                                    <tbody>
                                        {list.map(r => (
                                            <tr key={r.id}>
                                                <td className="fw-bold text-success" style={{ fontSize: '0.75rem' }}>{r.outward_no}</td>
                                                <td style={{ fontSize: '0.75rem' }}>{new Date(r.outward_date).toLocaleDateString()}</td>
                                                <td style={{ fontSize: '0.75rem' }}>{r.party_name}</td>
                                                <td className="text-end fw-bold" style={{ fontSize: '0.75rem' }}>{r.total_qty}</td>
                                                <td className="text-center">
                                                    <button className="btn btn-success btn-sm rounded-pill px-3 py-0" style={{ fontSize: '0.7rem' }} onClick={() => onSelect(r)}>Load</button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                ...
                <div className="p-3 border-top text-end bg-light rounded-bottom-4">
                    <button className="btn btn-secondary btn-sm rounded-pill px-4" onClick={onClose}>Close</button>
                </div>
            </div>
        </div>
    );
...
export default function InternalLotYarnToFabricInward() {
    ...
    const fetchInitial = useCallback(async () => {
        try {
            const [noRes, lotRes, empRes] = await Promise.all([
                axios.get(`${API}/yarn-dyeing-inward/next-no/internal`),
                axios.get(`${API}/yarn-dyeing-inward/internal-lots`),
                axios.get(`${API}/employees`),
            ]);
            setInwardNo(noRes.data.inward_no);
            setInternalLots(lotRes.data);
            setEmployees(empRes.data);
        } catch { setInwardNo("YDI-INT-0001"); }
    }, []);
    ...
    const handleLotSelect = (e) => {
        ...
        const found = internalLots.find(l => l.internal_lot_no === val);
    ...
    const handleRowChange = (index, e) => {
        const { name, value } = e.target;
        const rows = [...form.rows]; rows[index] = { ...rows[index], [name]: value }; setForm(prev => ({ ...prev, rows }));
        if (name === "yarn_name") {
            setActiveRowIdx(index); setYarnSuggIdx(-1);
            if (value.length >= 1) axios.get(`${API}/yarn-dyeing-inward/yarn-search?term=${encodeURIComponent(value)}`).then(r => setYarnSuggestions(r.data)).catch(() => setYarnSuggestions([]));
    ...
    const selectYarn = (index, yarn) => {
        const rows = [...form.rows]; rows[index] = { ...rows[index], yarn_name: yarn.yarn_name, counts: yarn.counts || rows[index].counts, color: yarn.color || rows[index].color };
        setForm(prev => ({ ...prev, rows })); setYarnSuggestions([]); setActiveRowIdx(null);
    };
    ...
    const handleLoadSelect = (outward) => {
        setForm(prev => ({
            ...prev,
            party_name: outward.party_name || prev.party_name,
            rows: outward.items.map(it => ({
        ...
        toast("Details loaded from " + outward.outward_no, "success");
    };
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const res = await axios.post(`${API}/yarn-dyeing-inward`, { inward_type: "internal", ...form, total_qty: totalQty, items: form.rows });
            toast(`Saved! Inward No: ${res.data.inward_no}`, "success");
            setTimeout(() => navigate("/internal-lot-yarn-to-fabric-inward-list"), 1500);
        } catch (err) { toast(err.response?.data?.error || "Failed to save.", "danger"); }
    ...
    return (
        <div className="container-fluid mt-3 mb-5 px-3 px-md-4">
            ...
            {showLoadModal && <LoadOutwardModal type="internal" no={form.internal_lot_no} process={form.process} onClose={() => setShowLoadModal(false)} onSelect={handleLoadSelect} />}
            <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom">
                <button className="btn btn-outline-secondary btn-sm rounded-pill px-3" onClick={() => navigate("/internal-lot-yarn-to-fabric-inward-list")}><i className="bi bi-arrow-left me-1"></i>Back</button>
                <div>
                    ...
                    <h4 className="fw-bold mb-0 mt-1 text-dark"><i className="bi bi-box-arrow-in-down-right me-2 text-success"></i>Yarn to Fabric Inward</h4>
                </div>
            ...
            <div className="card border-0 shadow-sm rounded-4">
                <div className="card-body p-4">
                    <div className="row g-3 mb-3">
                        <div className="col-md-2"><label className="form-label fw-semibold text-muted" style={{ fontSize: "0.8rem" }}>DATE</label><input type="date" className="form-control" name="inward_date" value={form.inward_date} onChange={handleHeader} /></div>
                        <div className="col-md-2">
                            <label className="form-label fw-semibold text-muted" style={{ fontSize: "0.8rem" }}>INTERNAL LOT NO <span className="text-danger">*</span></label>
                            <select className={`form-select ${errors.internal_lot_no ? "is-invalid" : ""}`} name="internal_lot_no" value={form.internal_lot_no} onChange={handleLotSelect}>
                                ...
                                {internalLots.map((l, i) => <option key={i} value={l.internal_lot_no}>{l.internal_lot_no}</option>)}
                                {internalLots.length === 0 && <option disabled>No internal lots found</option>}
                            </select>
                            {errors.internal_lot_no && <div className="invalid-feedback">{errors.internal_lot_no}</div>}
                            {form.internal_lot_no && (
                                <button className="btn btn-link btn-sm p-0 mt-1 fw-bold text-decoration-none text-success" onClick={() => setShowLoadModal(true)}>
                                    <i className="bi bi-download me-1"></i>Load Details
                                </button>
                            )}
                        </div>
                        <div className="col-md-2">
                        ...
                        <div className="col-md-2"><label className="form-label fw-semibold text-muted" style={{ fontSize: "0.8rem" }}>INTERNAL LOT NAME</label><input type="text" className="form-control bg-light" name="internal_lot_name" value={form.internal_lot_name} readOnly={!!form.internal_lot_no} onChange={handleHeader} placeholder="Auto-filled from lot" /></div>
                        <div className="col-md-2"><label className="form-label fw-semibold text-muted" style={{ fontSize: "0.8rem" }}>REF. / DC NO.</label><input type="text" className="form-control" name="ref_no" value={form.ref_no} onChange={handleHeader} placeholder="DC / Challan" /></div>
                        <div className="col-md-2"><label className="form-label fw-semibold text-muted" style={{ fontSize: "0.8rem" }}>TOTAL QTY (KG)</label><input className="form-control fw-bold text-center bg-light" readOnly value={dispTotal} style={{ color: "#16a34a" }} /></div>
                    </div>
                    <div className="row g-3 mb-4">
                        <div className="col-md-4 position-relative" ref={supplierRef}>
                            ...
                            <input type="text" className={`form-control ${errors.party_name ? "is-invalid" : ""}`} name="party_name" value={form.party_name} onChange={handlePartyChange} onKeyDown={handleSupplierKeyDown} placeholder="Type to search supplier..." autoComplete="off" />
                            ...
                            {supplierSuggestions.length > 0 && (
                                <ul className="list-group position-absolute w-100 shadow-lg" style={{ zIndex: 1050, top: "calc(100% - 1px)", maxHeight: 200, overflowY: "auto" }}>
                                    {supplierSuggestions.map((s, i) => <li key={s.id} className={`list-group-item list-group-item-action py-1 px-3 small ${supplierSuggIdx === i ? "active" : ""}`} style={{ cursor: "pointer" }} onMouseDown={() => selectSupplier(s.name)}><i className="bi bi-building me-2 text-muted"></i>{s.name}</li>)}
                                </ul>
                            )}
                        </div>
                        ...
                        <div className="col-md-4"><label className="form-label fw-semibold text-muted" style={{ fontSize: "0.8rem" }}>REMARKS</label><input type="text" className="form-control" name="remarks" value={form.remarks} onChange={handleHeader} placeholder="Optional remarks" /></div>
                    </div>
                    <div className="card bg-light border-0 rounded-3 mb-4 p-3">
                        <div className="d-flex align-items-center mb-3"><i className="bi bi-person-badge me-2 text-success fs-5"></i><h6 className="fw-bold mb-0 text-dark">Staff Details</h6></div>
                        <div className="row g-3">
                            <div className="col-md-4"><label className="form-label fw-semibold text-muted" style={{ fontSize: "0.8rem" }}>STAFF NAME</label><select className="form-select" name="staff_name" value={form.staff_name} onChange={handleHeader}><option value="">— Select Staff —</option>{employees.map((emp, i) => <option key={i} value={emp.employee_name}>{emp.employee_name}</option>)}</select></div>
                            <div className="col-md-8"><label className="form-label fw-semibold text-muted" style={{ fontSize: "0.8rem" }}>STAFF REMARKS</label><textarea className="form-control" name="staff_remarks" rows="2" value={form.staff_remarks} onChange={handleHeader} placeholder="Enter staff remarks or instructions..." /></div>
                        </div>
                    </div>
                    <div className="d-flex justify-content-between align-items-center mb-2">
                        <h6 className="fw-bold mb-0 text-dark"><i className="bi bi-table me-2 text-success"></i>Yarn Items (Received)</h6>
                        <button className="btn btn-success btn-sm rounded-pill px-3" onClick={addRow}><i className="bi bi-plus-lg me-1"></i>Add Row</button>
                    </div>
                                ...
                                {form.rows.map((row, idx) => (
                                    <tr key={idx}>
                                        <td className="position-relative">
                                            <input type="text" className="form-control form-control-sm" name="yarn_name" value={row.yarn_name} onChange={(e) => handleRowChange(idx, e)} onKeyDown={(e) => handleYarnKeyDown(e, idx)} placeholder="Search yarn..." autoComplete="off" />
                                            {activeRowIdx === idx && yarnSuggestions.length > 0 && (
                                                <ul className="list-group position-absolute w-100 shadow-lg" style={{ zIndex: 1050, top: "100%", maxHeight: 200, overflowY: "auto" }}>
                                                    {yarnSuggestions.map((y, i) => <li key={y.id} className={`list-group-item list-group-item-action py-1 px-2 small ${yarnSuggIdx === i ? "active" : ""}`} style={{ cursor: "pointer" }} onMouseDown={() => selectYarn(idx, y)}><strong>{y.yarn_name}</strong>{y.counts && <span className="text-muted ms-1">· {y.counts}</span>}{y.color && <span className="badge bg-secondary ms-1 rounded-pill" style={{ fontSize: "0.65rem" }}>{y.color}</span>}</li>)}
                                                </ul>
                                            )}
                                        </td>
                                        ...
                                        <td className="text-center"><button className="btn btn-sm btn-outline-danger rounded-circle" style={{ width: 28, height: 28, padding: 0 }} onClick={() => removeRow(idx)} disabled={form.rows.length === 1}><i className="bi bi-x"></i></button></td>
                                    </tr>
                                ))}
                            </tbody>
                            <tfoot><tr style={{ background: "#166534", color: "#fff" }}><td colSpan={form.process === "Knitting" ? 6 : 3} className="text-end fw-bold">Total Qty (KG):</td><td className="text-center fw-bold text-warning">{dispTotal}</td><td></td></tr></tfoot>
                        </table>
                    </div>
                    <button className="btn btn-outline-success btn-sm rounded-pill w-100 mt-2 mb-4" style={{ borderStyle: "dashed" }} onClick={addRow}><i className="bi bi-plus-circle me-2"></i>Add Another Row</button>
                    <div className="d-flex justify-content-end gap-2 pt-3 border-top">
                        <button className="btn btn-light rounded-pill px-4" onClick={() => { setForm(getForm()); setErrors({}); }}><i className="bi bi-arrow-counterclockwise me-1"></i>Reset</button>
                        <button className="btn btn-secondary rounded-pill px-4" onClick={() => navigate("/internal-lot-yarn-to-fabric-inward-list")}><i className="bi bi-x-lg me-1"></i>Cancel</button>
                        <button className="btn btn-success rounded-pill px-5 fw-semibold" onClick={handleSave} disabled={saving}>{saving ? <><span className="spinner-border spinner-border-sm me-2" />Saving...</> : <><i className="bi bi-check2-circle me-2"></i>Save Inward</>}</button>
                    </div>
                </div>
            </div>
        </div>
