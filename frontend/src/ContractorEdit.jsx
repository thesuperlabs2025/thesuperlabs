...
function ContractorEdit() {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                // Fetch countries
                const countryRes = await axios.get(`${API}/countries`);
                setCountries(countryRes.data);

                // Fetch contractor details
                const res = await axios.get(`${API}/contractor/${id}`);
                const data = res.data;
                setValues(data);

                // If they have a country, fetch its states
                if (data.billing_country) {
                    const country = countryRes.data.find(c => c.name === data.billing_country);
                    if (country) {
                        const stateRes = await axios.get(`${API}/states/${country.id}`);
                        setStates(stateRes.data);

                        // If they have a state, fetch its cities
                        if (data.billing_state) {
                            const state = stateRes.data.find(s => s.name === data.billing_state);
                            if (state) {
                                const cityRes = await axios.get(`${API}/cities/${state.id}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`${API}/contractor/${id}`, values);
        ...
        <div className="container py-5">
            <div className="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
                <h2 className="fw-bold text-primary mb-0">
                    <i className="bi bi-person-workspace me-2"></i>Edit Contractor
                </h2>
                        ...
                        <div className="card-body p-4">
                            <div className="row g-3">
                                ...
                                <div className="col-md-6">
                                    ...
                                    <input type="text" name="gst_tin" className="form-control" value={values.gst_tin} onChange={handleChange} />
                                </div>
                            </div>
                    ...
                    <div className="card shadow-sm border-0 h-100">
                        ...
                        <div className="card-footer bg-white border-0 p-4">
                            <button type="submit" className="btn btn-primary w-100 py-3 fw-bold rounded-pill">
                                <i className="bi bi-check-circle me-2"></i>UPDATE CONTRACTOR
                            </button>
                        </div>
                    </div>
                </div>
