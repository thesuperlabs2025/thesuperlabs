...
function PrintModal({ rec, items, onClose }) {
    ...
    const downloadPDF = () => {
        ...
        const opt = {
            margin: 0,
            filename: `Order-Inward-${rec.inward_no || rec.id}.pdf`,
...
export default function OrderJobworkYarnToFabricInwardList() {
    const navigate = useNavigate();
    const today = new Date().toISOString().substring(0, 10);
    const [allRecords, setAllRecords] = useState([]);
    const [loading, setLoading] = useState(true);
    const [selected, setSelected] = useState(new Set());
    const [filters, setFilters] = useState({ party: "", inward_no: "", order_no: "", from_date: "", to_date: today });
    ...
    }, [allRecords, filters.party, filters.inward_no, filters.order_no]);
        ...
        <div className="container-fluid py-4 px-4 h-100 d-flex flex-column">
            ...
            <div className="d-flex align-items-center justify-content-between mb-4 pb-3 border-bottom flex-wrap gap-3">
                <div className="d-flex align-items-center gap-3">
                    <div className="bg-success text-white rounded-4 d-flex align-items-center justify-content-center shadow-sm" style={{ width: "56px", height: "56px" }}>
                        <i className="bi bi-box-arrow-in-down-right fs-3"></i>
                    </div>
                    <div>
                        <h4 className="fw-extrabold mb-0 text-dark" style={{ letterSpacing: "-0.02em" }}>Order Yarn to Fabric Inward</h4>
                        <div className="text-muted fw-medium small">Manage and track your yarn to fabric inward receipts</div>
                    </div>
                </div>
                <div className="d-flex gap-2">
                    {selected.size > 0 && (
                        <button className="btn btn-danger rounded-pill px-3 btn-sm fw-bold shadow-sm" onClick={handleDelete}>
            ...
            <div className="card border-0 shadow-sm rounded-4 mb-4">
                <div className="card-body p-3">
                    <div className="row g-3 align-items-end">
                        ...
                        <div className="col-md-2">
                            <label className="form-label fw-bold text-muted small mb-1">INWARD NO</label>
                            <input type="text" className="form-control form-control-sm border-0 bg-light" placeholder="Search inward..." value={filters.inward_no} onChange={e => setFilters(p => ({ ...p, inward_no: e.target.value }))} />
                        </div>
                        <div className="col-md-2">
                        ...
                        <div className="col-md-1">
                            <button className="btn btn-outline-secondary btn-sm rounded-pill w-100 fw-bold border-0" onClick={() => setFilters({ party: "", inward_no: "", order_no: "", from_date: "", to_date: today })}>
            ...
            <div className="card border-0 shadow-sm rounded-4 flex-grow-1 overflow-hidden">
                <div className="table-responsive h-100">
                    <table className="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            {loading ? (
                                <tr><td colSpan={8} className="text-center py-5"><div className="spinner-border spinner-border-sm text-success me-2"></div>Loading records...</td></tr>
                            ) : filtered.length === 0 ? (
                                <tr><td colSpan={8} className="text-center py-5 text-muted"><i className="bi bi-inbox fs-2 d-block mb-2"></i>No records found</td></tr>
                            ) : filtered.map(r => (
                                <tr key={r.id} className={selected.has(r.id) ? "table-success-subtle" : ""}>
                                    <td className="ps-4">
                                        <input type="checkbox" className="form-check-input" checked={selected.has(r.id)} onChange={() => toggleOne(r.id)} />
                                    </td>
                                    <td>
                                        <span className="badge fw-bold px-3 py-2 rounded-pill" style={{ background: "#f0fdf4", color: "#16a34a", fontSize: "0.8rem" }}>
                                            {r.inward_no}
                                        </span>
                                    </td>
                                    <td>
                                        <span className={`badge badge-process rounded-pill bg-success-subtle text-success`}>
                                            {r.process || 'Yarn'}
                                        </span>
                                    </td>
                                    <td className="text-muted fw-semibold">{fmt(r.inward_date)}</td>
                                    <td>
                                    ...
                                    <td className="text-end fw-extrabold text-success">{parseFloat(r.total_qty || 0).toFixed(3)} <span className="text-muted fw-bold" style={{ fontSize: "0.7rem" }}>KG</span></td>
                                    <td className="text-center">
                                        <div className="d-flex gap-1 justify-content-center">
                                            <button className="btn btn-sm rounded-circle border-0 shadow-sm" style={{ width: 32, height: 32, background: "#eff6ff", color: "#3b82f6" }} onClick={() => navigate(`/edit-order-jobwork-yarn-to-fabric-inward/${r.id}`)} title="Edit">
