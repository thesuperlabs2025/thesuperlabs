...
export default function PurchaseSummary() {
    ...
    const [filters, setFilters] = useState({ supplier_name: "", sku: "", from_date: "", to_date: "" });
    ...
    const handleSearch = async (f = filters) => { try { const res = await axios.get(`${API}/reports/purchase/summary`, { params: f }); setData(res.data); } catch (err) { console.error(err); } };
    ...
    return (
        <div className="container-fluid my-4">
            ...
            <div className="report-title-print"><span>Purchase Summary Report</span><span>Period: {formatDate(filters.from_date)} To {formatDate(filters.to_date)}</span></div>
            <div className="d-flex justify-content-between align-items-center mb-4 no-print"><h2 className="fw-bold">Purchase Summary Report</h2><button className="btn btn-outline-danger" onClick={printReport}>Print PDF</button></div>
            ...
            <div className="table-responsive">
                <table className="table table-bordered table-hover shadow-sm">
                    <thead className="table-dark"><tr><th>ID</th><th>Date</th><th>Supplier</th><th>SKU</th><th>Qty</th><th>Rate</th><th>GST%</th><th>Total</th></tr></thead>
                    <tbody>{data.map((item, i) => (<tr key={i}><td>{item.tx_id}</td><td>{formatDate(item.tx_date)}</td><td>{item.party_name}</td><td>{item.sku}</td><td>{item.qty}</td><td>{item.rate}</td><td>{item.gst_percent}%</td><td className="fw-bold">{parseFloat(item.item_total).toFixed(2)}</td></tr>))}</tbody>
                </table>
            </div>
        </div>
    );
