...
export default function InternalLotYarnToFabricReturn() {
    ...
    const fetchInitial = useCallback(async () => {
        try {
            const [noRes, lotRes, empRes] = await Promise.all([
                axios.get(`${API}/yarn-dyeing-return/next-no/internal`),
    ...
    const loadPending = async () => {
        if (!form.internal_lot_no) { toast("Select Internal Lot No first", "warning"); return; }

        try {
            const res = await axios.get(`${API}/yarn-dyeing-return/pending-balance?type=internal&no=${form.internal_lot_no}&process=${form.process}`);
            const items = res.data;
            if (items.length === 0) {
                toast("No pending balance items found.", "info");
                return;
            }
            setForm(prev => ({
                ...prev,
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await axios.post(`${API}/yarn-dyeing-return`, {
                return_type: "internal", ...form,
                total_qty: totalQty, items: form.rows,
            });
            toast(`Saved! Return No: ${res.data.return_no}`, "success");
            setTimeout(() => navigate("/internal-lot-yarn-to-fabric-return-list"), 1500);
        } catch (err) {
    ...
    return (
        <div className="container-fluid mt-3 mb-5 px-3 px-md-4">
            ...
            <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom">
                <button className="btn btn-outline-secondary btn-sm rounded-pill px-3" onClick={() => navigate("/internal-lot-yarn-to-fabric-return-list")}>
            ...
            <div className="card border-0 shadow-sm rounded-4">
                <div className="card-body p-4">
                    ...
                    <div className="row g-3 mb-3">
                        ...
                        <div className="col-md-2">
                            ...
                            <select className={`form-select ${errors.internal_lot_no ? "is-invalid" : ""}`} name="internal_lot_no" value={form.internal_lot_no} onChange={handleLotSelect}>
                                ...
                                {internalLots.map(l => <option key={l.id} value={l.internal_lot_no}>{l.internal_lot_no}</option>)}
                            </select>
                        ...
                        <div className="col-md-3">
                            ...
                            <input type="text" className="form-control bg-light" name="internal_lot_name" value={form.internal_lot_name}
                                readOnly={!!form.internal_lot_no} onChange={handleHeader} placeholder="Auto-filled" />
                        </div>
                        <div className="col-md-2">
                    ...
                    {/* Staff details etc. same as above */}
                    ...
                    <div className="d-flex justify-content-end gap-2 pt-3 border-top">
                        ...
                        <button className="btn btn-secondary rounded-pill px-4" onClick={() => navigate("/internal-lot-yarn-to-fabric-return-list")}>
