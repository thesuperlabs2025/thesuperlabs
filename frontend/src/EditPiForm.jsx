...
function EditPIForm() {
  const { id } = useParams();
  const [pi, setPI] = useState(getInitialState());
  ...
  useEffect(() => {
    document.title = `Edit PI No: ${id || ""} - TSL ERP`;
  }, [id]);
  ...
  useEffect(() => {
    axios.get(`${API}/templates`)
      .then((res) => {
        ...
        const piTpl = templates.find((t) => t.template_name?.toLowerCase() === "pi");
        if (!piTpl || !piTpl.stock_action) {
          setPI((prev) => ({ ...prev, template_id: null, stock_action: null }));
          setError("No valid 'pi' template found.");
          return;
        }
        setPI((prev) => ({ ...prev, stock_action: piTpl.stock_action, template_id: piTpl.id }));
      })
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/pi/${id}`)
      .then((res) => {
        ...
        const raw = res.data;
        const data = raw.pi ? raw.pi : raw;       // supports both formats
      ...
      .catch((err) => {
        console.error("Error loading pi:", err);
  ...
  useEffect(() => {
    const totals = calculateTotals(pi.items);
    setPI((prev) => ({ ...prev, ...totals }));
  }, [pi.items]);

  // Header change handler
  const handleHeaderChange = (e) => {
    const { name, value } = e.target;
    setPI((prev) => ({ ...prev, [name]: value }));
  };
  ...
  const handleProductInput = async (index, e) => {
    const value = e.target.value;
    const newItems = [...pi.items];
    newItems[index].sku = value;
    setPI(prev => ({ ...prev, items: newItems }));
  ...
  const addItem = () => setPI((prev) => ({ ...prev, items: [...prev.items, { ...initialItem }] }));
  const removeItem = (index) => pi.items.length > 1 && setPI((prev) => ({
    ...prev,
    items: prev.items.filter((_, i) => i !== index),
  }));

  const toggleIGST = () => setPI((prev) => ({ ...prev, igst: !prev.igst }));

  // Save invoice
  const savePI = async () => {
    if (!pi.template_id) return setError("Cannot save pi: valid 'pi' template not selected.");
    if (!pi.items.length) return setError("Cannot save pi: No items added.");

    try {
      const payload = {
        ...pi,
        pi_date: inputToDMY(pi.pi_date),
        due_date: inputToDMY(pi.due_date),
      };
      if (id) {
        const res = await axios.put(`${API}/pi/${id}`, payload);


        setSuccess(res.data.message || "pi updated successfully!");
      } else {
        const res = await axios.post(`${API}/pi`, payload);
        setSuccess(res.data.message || "pi saved successfully!");
      }
      setTimeout(() => navigate("/pimy"), 1000);
    } catch (err) {
      console.error(err.response?.data || err.message);
      setError(err.response?.data?.error || "Error saving pi");
    }
  };

  const clearForm = () => window.confirm("Clear form?") && setPI(getInitialState());
    ...
    <div className="container my-4">
      ...
      <div className="card shadow p-4">
        <div style={{ position: "relative", marginBottom: "1rem" }}>
          ...
          >
            <label className="fw-bold mb-0 me-1 text-muted small">PI No:</label>
            <span className="text-primary">{id || "Loading..."}</span>
          </div>
          <h3 className="mb-0 text-center">PI Form</h3>
        </div>
        ...
        <div className="card shadow p-3 mb-3 mt-4">
          <div className="row mb-3">
            ...
            <div className="col-md-6 mb-2">
              ...
              <input type="text" name="ship_to" value={pi.ship_to} onChange={handleHeaderChange} className="form-control" />
            </div>

            <div className="col-md-6 mb-2">
              <label className="form-label">Sales Person</label>
              <select name="sales_person" value={pi.sales_person} onChange={handleHeaderChange} className="form-select">
            ...
            <div className="col-md-6 mb-2">
              ...
              <input type="text" name="mobile" value={pi.mobile} onChange={handleHeaderChange} className="form-control" />
            </div>
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                type="date"
                name="pi_date"
                value={pi.pi_date ? dmyToInput(pi.pi_date) : ""}
                onChange={(e) =>
                  setPI({
                    ...pi,
                    pi_date: inputToDMY(e.target.value),
                  })
                }
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                type="date"
                name="due_date"
                value={pi.due_date ? dmyToInput(pi.due_date) : ""}
                onChange={(e) =>
                  setPI({
                    ...pi,
            ...
            <div className="col-md-12 mb-2">
              ...
              <textarea name="billing_address" value={pi.billing_address} onChange={handleHeaderChange} className="form-control" />
            </div>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end mb-3">
          <div className="form-check">
            <input type="checkbox" className="form-check-input" checked={pi.igst} onChange={toggleIGST} />
            ...
            <tfoot>
              <tr>
                ...
                <td className="fw-bold">
                  {(() => {
                    const qtyTotal = pi.items.reduce(
                      (sum, item) => sum + Number(item.qty || 0),
                      0
                    );
                    pi.total_qty = qtyTotal; // store in invoice
                    return qtyTotal;
                  })()}
                </td>
                <td colSpan="6"></td>
        ...
        <div className="border p-3 bg-light">
          <div className="d-flex justify-content-between mb-2">
            ...
            <input value={pi.discount_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          {pi.igst ? (
            <div className="d-flex justify-content-between mb-2">
              <label>IGST:</label>
              <input value={pi.gst_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
            </div>
          ) : (
            <>
              <div className="d-flex justify-content-between mb-2">
                <label>CGST:</label>
                <input value={(pi.gst_total / 2).toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
              </div>
              <div className="d-flex justify-content-between mb-2">
                <label>SGST:</label>
                <input value={(pi.gst_total / 2).toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input value={pi.net_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          <div className="d-flex justify-content-between mb-2">
            <label>Round Off:</label>
            <input value={pi.round_off.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          <div className="d-flex justify-content-between">
            <label>Grand Total:</label>
            <input
              value={pi.grand_total.toFixed(2)}
              readOnly
        ...
        <div className="d-flex justify-content-end mt-3">
          ...
          <button onClick={savePI} className="btn btn-success">
            Save PI
          </button>
        </div>
      </div>
