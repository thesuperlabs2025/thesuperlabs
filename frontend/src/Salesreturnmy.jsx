...
function Salesreturnmy() {
  const [salesreturn, setSalesreturn] = useState([]);
  const [loading, setLoading] = useState(true);

  const [selectedIds, setSelectedIds] = useState([]);
  const [selectAll, setSelectAll] = useState(false);
  const [salesreturnPriv, setSalesReturnPriv] = useState(null);
  ...
  //     if (filters.salesreturn_date) params.salesreturn_date = filters.salesreturn_date;
  //     if (filters.due_date) params.due_date = filters.due_date;
  //     if (filters.status) params.status = filters.status;

  //     const res = await axios.get(`${API}/salesreturn`, { params });

  //     setSalesreturn(res.data.data || []);
  //     setPagination(res.data.pagination || null);
  //     setSelectedIds([]);
  //     setSelectAll(false);
  //   } catch (err) {
  //     console.error("Fetch sales return Error:", err);
  //     alert("Error loading sales return");
  //   } finally {
  //     setLoading(false);
  //   }
  ...
  const fetchSalesreturn = useCallback(async () => {
    ...
    try {
      const params = {
        ...
        sort_by: sortBy,
        order,
        usertype_id: user.usertype_id, // ✅ REQUIRED (same as Estimate)
      };
      ...
      // ⚠️ backend expects `sales_return_date`
      if (filters.sales_return_date)
        params.sales_return_date = filters.sales_return_date;
      ...
      // ✅ same pagination fallback as Estimate / DC / GRN
      setPagination(
        res.data.pagination || {
          page: 1,
          limit: 20,
      ...
      setSalesReturnPriv(res.data.privileges || {});

      setSelectedIds([]);
      setSelectAll(false);
    } catch (err) {
      console.error("Fetch Sales Return Error:", err);
      alert(err.response?.data?.message || "Error loading Sales Return");
    } finally {
      setLoading(false);
    }
  ...
  const handleConvert = () => {
    if (selectedIds.length === 0) {
      alert("Please select at least one Sales Return to convert.");
      return;
    }

    const selectedReturns = salesreturn.filter(s => selectedIds.includes(s.id));
    const sameCustomer = selectedReturns.every(s => s.customer_name === selectedReturns[0].customer_name);

    if (!sameCustomer) {
      alert("Please select Sales Returns from the same customer for conversion.");
      return;
    }
  ...
  const confirmConvert = async () => {
    ...
    try {
      const fullDetails = await Promise.all(
        selectedIds.map(id => axios.get(`${API}/salesreturn/${id}`).then(res => res.data))
      );

      const first = fullDetails[0];
      const combinedItems = fullDetails.flatMap(s => s.items);
      ...
      alert("Failed to fetch sales return details for conversion.");
    }
  };
  ...
  const handleFilterChange = (e) => {
    ...
    if (name === "customer_name") {
      if (!value) {
        ...
        const suggestions = salesreturn
          .filter((salesreturn) =>
            salesreturn.customer_name?.toLowerCase().includes(value.toLowerCase())
          )
          .slice(0, 5);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this salesreturn?")) return;
    try {
      await axios.delete(`${API}/salesreturn/${id}`);
      alert("Deleted");
      fetchSalesreturn();
    } catch (err) {
      console.error(err);
  ...
  const handleSelectAll = () => {
    if (!selectAll) {
      setSelectedIds(salesreturn.map((salesreturn) => salesreturn.id));
    } else {
      setSelectedIds([]);
    }
    setSelectAll((s) => !s);
  ...
  const handleDeleteSelected = async () => {
    if (selectedIds.length === 0) {
      alert("Select at least one salesreturn");
      return;
    }
    if (!window.confirm("Delete selected salesreturn?")) return;
    try {
    ...
    <div className="container-fluid py-4 px-md-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="fw-bold text-primary mb-0">
          <i className="bi bi-arrow-counterclockwise me-2"></i>Sales Return
        </h2>
        <div className="d-flex align-items-center gap-3">
          ...
          <div className="btn-group shadow-sm">
            {salesreturnPriv?.can_add === 1 && (
              <button className="btn btn-primary d-flex align-items-center fw-bold px-4 shadow-sm" onClick={() => navigate("/salesreturnform")}>
                <i className="bi bi-plus-lg me-2"></i>New
              </button>
            )}
            {salesreturnPriv?.can_delete === 1 && (
        ...
        <div className="card-body p-4">
          <div className="row g-3">
            ...
            <div className="col-md-2">
              ...
              <input type="date" name="sales_return_date" className="form-control" value={filters.sales_return_date} onChange={handleFilterChange} />
            </div>
            <div className="col-md-3">
        ...
        <div className="card shadow-sm border-0">
              ...
              <table className="table table-hover table-striped mb-0 align-middle" style={{ fontSize: '16px' }}>
                <thead className="table-dark small">
                  <tr>
                    ...
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("sales_return_date")}>
                      DATE {sortBy === "sales_return_date" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                ...
                <tbody>
                  {salesreturn.length === 0 ? (
                    ...
                    salesreturn.map((s) => (
                      <tr key={s.id}>
                        <td className="ps-4 no-print"><input className="form-check-input" type="checkbox" checked={selectedIds.includes(s.id)} onChange={() => handleCheckboxChange(s.id)} /></td>
                        <td className="fw-bold text-dark">#{s.id}</td>
                        <td className="small">{s.sales_return_date}</td>
                        <td className="fw-bold text-dark">{s.customer_name}</td>
                        <td className="text-end fw-bold">₹{s.grand_total}</td>
                        <td className="text-center small hide-on-mobile">{s.total_qty}</td>
                        <td className="hide-on-mobile"><span className="badge bg-light text-dark fw-normal border">{s.sales_person}</span></td>
                        <td className="text-end pe-4 no-print">
                          <div className="d-flex justify-content-end gap-2">
                            <button className="btn btn-outline-secondary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                              ...
                              onClick={() => window.open(`${API}/printsalesreturn/salesreturn/${s.id}`, "_blank")}>
                              <i className="bi bi-printer-fill"></i>
                            </button>
                            {salesreturnPriv?.can_update === 1 && (
                              <button className="btn btn-outline-primary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                                ...
                                onClick={() => navigate(`/Editsalesreturnform/${s.id}`)}>
          ...
          <div className="card-footer bg-white py-3 border-0 d-flex justify-content-between align-items-center mb-5 no-print">
            <span className="text-muted small">
              Page {pagination?.page || 1} of {pagination?.pages || 1} &bull; Total {pagination?.total || 0} Sales Returns
            </span>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg">
              <div className="modal-header bg-primary text-white border-0">
                <h5 className="modal-title fw-bold">Convert Sales Return(s)</h5>
                <button type="button" className="btn-close btn-close-white" onClick={() => setShowConvertModal(false)}></button>
              </div>
              <div className="modal-body p-4">
                <p className="text-muted small mb-4">Select the target module to convert selected Sales Returns into. Data like customer info and items will be carried over.</p>
                <div className="mb-3">
