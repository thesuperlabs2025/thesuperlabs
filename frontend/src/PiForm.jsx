...
const getInitialState = () => ({
  header: {
    ...
    gst: "",
    sales_person: "",
    pi_date: new Date().toISOString().substring(0, 10),
...
function PiForm() {
  ...
  useEffect(() => {
    document.title = `PI No: ${piNo || ""} - TSL ERP`;
  }, [piNo]);
  ...
  useEffect(() => {
    axios.get(`${API}/templates`)
      .then((res) => {
        ...
        // ⭐ Find PI template
        const piTpl = templates.find(
          (t) => t.template_name.toLowerCase() === "pi"
        );

        if (!piTpl || !piTpl.stock_action) {
          toast.error("No valid 'pi' template found.");
          setPI((prev) => ({
            ...prev,
            template_id: null,
            stock_action: null,
        ...
        setPI((prev) => ({
          ...prev,
          stock_action: piTpl.stock_action,
          template_id: piTpl.id,
        }));
  ...
  const fetchPiNo = async () => {
    try {
      const res = await axios.get(`${API}/pi/next-pi-no`);
      setPiNo(res.data.piNo);
    } catch (err) {
      console.error("Failed to load PI number", err);
    }
  };
  ...
  const selectProduct = (index, product) => {
    const isDuplicate = pi.items.some(
  ...
  const removeItem = (index) => {
    if (pi.items.length > 1) {
      const newItems = pi.items.filter((_, i) => i !== index);
      setPI((prev) => ({ ...prev, items: newItems }));
    }
  };
  ...
  // --- IGST toggle ---
  const toggleIGST = () =>
    setPI((prev) => ({ ...prev, igst: !prev.igst }));

  // --- Save PI ---
  const savePI = async () => {
    // 🛑 Stop if no template
    if (pi.template_id == null) {
      toast.error("Cannot save pi: valid 'pi' template not selected.");
      return;
    }

    if (!pi.items || pi.items.length === 0) {
      toast.error("Cannot save pi: No items added.");
      return;
    }
    ...
    const piDateVal = new Date(pi.pi_date);
    const startDate = new Date(selectedYear.start_date);
    const endDate = new Date(selectedYear.end_date);
    if (piDateVal < startDate || piDateVal > endDate) {
      if (!window.confirm(`Warning: PI date is outside the selected Accounting Year (AY ${selectedYear.year_name}). Do you want to proceed?`)) {
        return;
      }
    }

    try {
      const res = await axios.post(`${API}/pi`, {
        customer_name: pi.customer_name,
        ship_to: pi.ship_to,
        mobile: pi.mobile,
        pi_date: toDMY(pi.pi_date),
        due_date: toDMY(pi.due_date),
        sales_person: pi.sales_person,
      ...
      toast.success(res.data.message || "PI saved successfully!");
      ...
      console.error("Error saving pi:", err.response?.data || err.message);
      toast.error(err.response?.data?.error || "Error saving pi");
    }
  };
  ...
  // --- Clear form ---
  const clearForm = () => {
    if (window.confirm("Clear form?")) setPI(getInitialState());
  };
      ...
      <div className="card shadow p-4 w-90">
        <div className="mb-3 d-flex justify-content-between align-items-center border-bottom pb-3">
          <div className="d-flex align-items-center gap-4">
            <div>
              ...
              <div className="text-primary fw-bold fs-5">{piNo || "Loading..."}</div>
            </div>
          ...
          <div className="text-center flex-grow-1">
            <h3 className="fw-bold mb-0">PI Form</h3>
          </div>
        ...
        <div className="card shadow p-3 mb-3">
          <div className="row mb-3">
            ...
            <div className="col-md-6 mb-2 position-relative">
              ...
              {shipToSuggestions.length > 0 && (
                ...
                >
                  {shipToSuggestions
                    .filter(cust =>
                      (cust.customer_name ?? cust.name ?? "").toLowerCase().includes((pi.ship_to || "").toLowerCase())
                    )
                    .slice(0, 10)
                    .map((cust, index) => (
                      <li
            ...
            <div className="col-md-6 mb-2 position-relative">
              ...
              <select
                name="sales_person"
                value={pi.sales_person}
                onChange={handleHeaderChange}
                className="form-select"
              >
                <option value="">Select Saless Person</option>
                ...
                <td className="fw-bold">
                  {
                    (() => {
                      ...
                      pi.total_qty = qtyTotal; // ✅ store inside PI object
        ...
        <div className="border p-3 bg-light">
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input
              value={pi.discount_total.toFixed(2)}
              readOnly
              className="form-control form-control-sm text-end w-25"
            />
          </div>
          ...
          {pi.igst ? (
            ...
            <>
              <div className="d-flex justify-content-between mb-2">
                <label>CGST:</label>
                <input
                  value={(pi.gst_total / 2).toFixed(2)}
                  readOnly
                  className="form-control form-control-sm text-end w-25"
                />
              </div>
        ...
        <div className="text-center mt-4">
          {selectedYear.is_closed ? (
            ...
            <>
              <button className="btn btn-success px-5 py-2 me-2 fw-bold shadow-sm rounded-pill" onClick={savePI}>
                Save PI
              </button>
