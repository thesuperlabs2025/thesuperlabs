...
const DirectInwardList = () => {
    ...
    const fetchInwards = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${API}/direct-inward`);
            setInwards(res.data);
        } catch (err) {
            console.error(err);
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this inward? Stock will be reversed.")) {
            try {
                await axios.delete(`${API}/direct-inward/${id}`);
            ...
            <div className="mb-3 mx-auto" style={{
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                    <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            <h3 className="fw-bold mb-1" style={{ letterSpacing: '-0.02em' }}>Direct Inward</h3>
                            <p className="opacity-75 mb-0 small">Track general inventory stock entries</p>
                        </div>
                        <div className="d-flex gap-2">
                            <motion.button
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                                className="btn btn-light px-4 fw-bold text-primary shadow-sm"
                                onClick={() => navigate("/direct-inward/new")}
                ...
                <div className="card-header bg-white border-0 py-3 px-4">
                    <div className="row align-items-center">
                        <div className="col-md-6">
                            <h6 className="mb-0 fw-bold text-dark">Recent Inwards <span className="badge bg-blue-100 text-blue-700 ms-2">{filtered.length}</span></h6>
                        </div>
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <tr key={inward.id} className="border-bottom border-light">
                                        ...
                                        <td className="px-4 py-3">
                                            {inward.order_no ? (
                                                <span className="badge bg-blue-50 text-blue-600 border border-blue-100 fw-bold px-2 py-1" style={{ fontSize: '0.75rem' }}>
                                                    {inward.order_no}
                                                </span>
                                        ...
                                        <td className="px-4 py-3 text-end">
                                            <div className="d-flex justify-content-end gap-2">
                                                <button
                                                    ...
                                                    style={{ background: '#eff6ff', color: '#1d4ed8', borderRadius: '8px' }}
                                                    onClick={() => navigate("/direct-inward/edit", { state: { id: inward.id } })}
                                                >
...
export default DirectInwardList;
