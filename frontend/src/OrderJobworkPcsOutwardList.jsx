...
function PrintModal({ rec, items, onClose }) {
    ...
    const downloadPDF = () => {
        ...
        const opt = {
            margin: 0,
            filename: `Pcs-Outward-${rec.outward_no || rec.id}.pdf`,
                ...
                <div className="yd-no-print d-flex align-items-center justify-content-between px-4 py-3 border-bottom sticky-top bg-white rounded-top-4" style={{ zIndex: 10 }}>
                    <div className="d-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <div className="text-muted" style={{ fontSize: "0.65rem" }}>Professional Pcs Outward</div>
                        </div>
                    </div>
                ...
                <div id="pcs-print-content" style={{ padding: "40px", fontFamily: "'Inter', 'Segoe UI', sans-serif", color: "#000", background: "#fff" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "30px" }}>
                        ...
                        <div style={{ textAlign: "right" }}>
                            <div style={{ fontSize: "12px", color: "#9ca3af", textTransform: "uppercase", fontWeight: "700", marginBottom: "4px" }}>Jobwork Challenger</div>
                            <div style={{ fontSize: "32px", fontWeight: "900", color: "#000", lineHeight: "1" }}>PCS OUTWARD</div>
                            <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end", marginTop: "15px" }}>
                                <div style={{ padding: "6px 12px", background: "#f9fafb", borderRadius: "8px", textAlign: "center", border: "1px solid #f3f4f6" }}>
                                    ...
                                    <div style={{ fontSize: "12px", fontWeight: "700" }}>{rec.outward_no || rec.id}</div>
                                </div>
                                <div style={{ padding: "6px 12px", background: "#f9fafb", borderRadius: "8px", textAlign: "center", border: "1px solid #f3f4f6" }}>
                                    <div style={{ fontSize: "9px", color: "#6b7280", fontWeight: "600", textTransform: "uppercase" }}>Date</div>
                                    <div style={{ fontSize: "12px", fontWeight: "700" }}>{fmt(rec.outward_date)}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "40px", marginBottom: "30px", padding: "15px", background: "#fafafa", borderRadius: "12px" }}>
                        <div>
                            <div style={{ fontSize: "10px", fontWeight: "700", color: "#6b7280", textTransform: "uppercase", marginBottom: "8px" }}>Issued From (Buyer)</div>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: "12px", fontWeight: "700", marginTop: "8px", color: "#059669" }}>Process: {rec.process}</div>
                        </div>
                    </div>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "20px" }}>
                        <thead>
                            <tr style={{ background: "#000", color: "#fff" }}>
                                <th style={{ padding: "10px 15px", textAlign: "center", fontSize: "11px", fontWeight: "700", border: "1px solid #000" }}>#</th>
                                <th style={{ padding: "10px 15px", textAlign: "left", fontSize: "11px", fontWeight: "700", border: "1px solid #000" }}>STYLE NAME</th>
                                <th style={{ padding: "10px 15px", textAlign: "center", fontSize: "11px", fontWeight: "700", border: "1px solid #000" }}>COLOR</th>
                                <th style={{ padding: "10px 15px", textAlign: "center", fontSize: "11px", fontWeight: "700", border: "1px solid #000" }}>SIZE</th>
                                <th style={{ padding: "10px 15px", textAlign: "right", fontSize: "11px", fontWeight: "700", border: "1px solid #000" }}>PCS</th>
                            </tr>
                        </thead>
                        <tbody>
                            {(() => {
                                ...
                                return (items || []).flatMap((it, i) => {
                                    let szData = {};
                                    try {
                                        szData = it.sizes_data;
                                        if (typeof szData === 'string') szData = JSON.parse(szData);
                        ...
                        <tfoot>
                            <tr style={{ background: "#f9fafb" }}>
                                ...
                                <td style={{ padding: "10px 15px", textAlign: "right", fontSize: "11px", fontWeight: "900", border: "1px solid #e5e7eb", color: "#000" }}>{totalPcs}</td>
                            </tr>
                        </tfoot>
                    </table>
...
/* ─── Main Component ──────────────────────────────────────────────────────── */
export default function OrderJobworkPcsOutwardList() {
    const navigate = useNavigate();
    const [data, setData] = useState([]);
    const [loading, setLoading] = useState(true);
    const [searchTerm, setSearchTerm] = useState({ supplier: "", outward_no: "", order_no: "" });
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${API}/pcs-outward`);
            setData(res.data || []);
    ...
    const filtered = useMemo(() => {
        return data.filter(r => (
            ...
            r.outward_no?.toLowerCase().includes(searchTerm.outward_no.toLowerCase()) &&
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`${API}/pcs-outward/${id}`);
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            await axios.delete(`${API}/pcs-outward/bulk`, { data: { ids: selectedIds } });
    ...
    const handlePrint = async (r) => {
        try {
            const res = await axios.get(`${API}/pcs-outward/${r.id}`);
            ...
            <div className="mx-auto" style={{ maxWidth: "1400px" }}>
                <div className="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
                    <div>
                        <div className="d-flex align-items-center gap-2 mb-1">
                            <span className="badge bg-indigo rounded-pill px-3 py-2 text-white" style={{ fontSize: "0.65rem", letterSpacing: "0.05em", background: "#4f46e5" }}>ORDER JOBWORK</span>
                        ...
                        <h2 className="fw-900 mb-0 ls-1 text-dark" style={{ fontSize: "1.75rem" }}>Pcs <span className="text-primary">Outward</span> List</h2>
                    </div>
                    <div className="d-flex gap-2">
                        <button className="btn btn-white btn-premium shadow-sm border" onClick={fetchData}>
                            <i className="bi bi-arrow-clockwise me-2"></i>Refresh
                        </button>
                        <button className="btn btn-primary btn-premium shadow-sm fw-bold" style={{ background: "#4f46e5", borderColor: "#4f46e5" }} onClick={() => navigate("/order-jobwork-pcs-outward")}>
                            <i className="bi bi-plus-lg me-2"></i>New Job Outward
                        </button>
                    </div>
                </div>
                ...
                <div className="card mb-4 overflow-hidden">
                    <div className="card-body p-4 bg-white border-bottom">
                        <div className="row g-3">
                            <div className="col-md-4">
                                <label className="form-label small fw-bold text-muted mb-2 text-uppercase ls-1" style={{ fontSize: '0.65rem' }}>Supplier / Contractor</label>
                                <div className="input-group">
                                    <span className="input-group-text bg-light border-0"><i className="bi bi-search text-muted"></i></span>
                                    <input type="text" className="form-control form-control-premium" placeholder="Search by name..." value={searchTerm.supplier} onChange={e => setSearchTerm(p => ({ ...p, supplier: e.target.value }))} />
                                </div>
                            </div>
                            <div className="col-md-4">
                                <label className="form-label small fw-bold text-muted mb-2 text-uppercase ls-1" style={{ fontSize: '0.65rem' }}>Outward Number</label>
                                <div className="input-group">
                                    <span className="input-group-text bg-light border-0"><i className="bi bi-hash text-muted"></i></span>
                                    <input type="text" className="form-control form-control-premium" placeholder="Search by outward no..." value={searchTerm.outward_no} onChange={e => setSearchTerm(p => ({ ...p, outward_no: e.target.value }))} />
                                </div>
                            </div>
                            <div className="col-md-4">
                                ...
                                <div className="input-group">
                                    ...
                                    <input type="text" className="form-control form-control-premium" placeholder="Search by order no..." value={searchTerm.order_no} onChange={e => setSearchTerm(p => ({ ...p, order_no: e.target.value }))} />
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                {loading ? (
                                    ...
                                    <tr><td colSpan="8" className="text-center py-5">
                                        <div className="text-muted py-4">
                                            ...
                                            <div className="fw-medium">No records found matching your search</div>
                                        </div>
                                    </td></tr>
                                ) : filtered.map(r => (
                                    <tr key={r.id}>
                                        ...
                                        <td className="fw-medium text-dark">{fmt(r.outward_date)}</td>
                                        <td>
                                            <span className="badge rounded-pill px-3 py-2 fw-bold" style={{ background: "#eef2ff", color: "#4f46e5", fontSize: '0.75rem' }}>
                                                {r.outward_no}
                                            </span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            <div className="text-muted" style={{ fontSize: '0.7rem' }}>
                                                {r.work_type === "Contractor" ? <span className="text-success fw-bold">CONTRACTOR</span> : "JOBWORK"}
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td className="text-end pe-4">
                                            <div className="d-flex justify-content-end gap-2">
                                                ...
                                                <button className="btn btn-sm btn-light rounded-circle border-0 action-btn p-0"
                                                    style={{ width: 32, height: 32, background: '#eff6ff' }} onClick={() => navigate(`/edit-order-jobwork-pcs-outward/${r.id}`)} title="Edit">
                    ...
                    {selectedIds.length > 0 && (
                        <div className="bg-dark text-white p-3 d-flex align-items-center justify-content-between animate__animated animate__fadeInUp">
