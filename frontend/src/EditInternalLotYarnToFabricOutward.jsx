...
export default function EditInternalLotYarnToFabricOutward() {
    const navigate = useNavigate();
    const { id } = useParams();
    const { toasts, add: toast, remove } = useToast();
    const [form, setForm] = useState({
        outward_date: new Date().toISOString().substring(0, 10),
        internal_lot_no: "", internal_lot_name: "", party_name: "", ref_no: "",
        process: "Yarn Dyeing",
    ...
    const fetchData = useCallback(async () => {
        try {
            const [recRes, lotsRes, empRes] = await Promise.all([
            ...
            setInternalLots(lotsRes.data || []);
            setEmployees(empRes.data || []);
            setForm({
                outward_date: rec.outward_date ? rec.outward_date.substring(0, 10) : "",
                internal_lot_no: rec.internal_lot_no || "",
                internal_lot_name: rec.internal_lot_name || "",
        ...
        } catch { toast("Failed to load record.", "danger"); }
    ...
    const handleSave = async () => {
        ...
        try {
            await axios.put(`${API}/yarn-dyeing-outward/${id}`, {
                outward_type: "internal", ...form, total_qty: totalQty, items: form.rows,
            });
            toast("Updated successfully!", "success");
            setTimeout(() => navigate("/internal-lot-yarn-to-fabric-list"), 1500);
        } catch (err) {
    ...
    return (
        <div className="container-fluid mt-3 mb-5 px-3 px-md-4">
            ...
            <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom">
                <button className="btn btn-outline-secondary btn-sm rounded-pill px-3" onClick={() => navigate("/internal-lot-yarn-to-fabric-list")}>
            ...
            <div className="card border-0 shadow-sm rounded-4">
                <div className="card-body p-4">
                    <div className="row g-3 mb-3">
                        ...
                        <div className="col-md-2">
                            <label className="form-label fw-semibold text-muted" style={{ fontSize: "0.8rem" }}>INTERNAL LOT NAME</label>
                            <input type="text" className="form-control bg-light" name="internal_lot_name" value={form.internal_lot_name} readOnly={!!form.internal_lot_no} onChange={handleHeader} placeholder="Auto-filled" />
                        </div>
                        <div className="col-md-2">
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <button className="btn btn-primary btn-sm rounded-pill px-3" onClick={addRow}><i className="bi bi-plus-lg me-1"></i>Add Row</button>
                    </div>
                    ...
                    <div className="d-flex justify-content-end gap-2 pt-3 border-top">
                        <button className="btn btn-secondary rounded-pill px-4" onClick={() => navigate("/internal-lot-yarn-to-fabric-list")}>
