...
function POmy() {
  ...
  const [poPriv, setPOPriv] = useState(null);
  ...
  const fetchPO = useCallback(async () => {
    ...
    try {
      ...
      if (filters.po_date) params.po_date = filters.po_date;
      if (filters.due_date) params.due_date = filters.due_date;
      if (filters.status) params.status = filters.status;

      const res = await axios.get(`${API}/po`, { params });
      ...
      setPOPriv(res.data.privileges || {});
      setSelectedIds([]);
      setSelectAll(false);
    } catch (err) {
      console.error("Fetch PO Error:", err);
      alert(err.response?.data?.message || "Error loading PO");
    } finally {
      setLoading(false);
    }
  ...
  const handleConvert = () => {
    if (selectedIds.length === 0) {
      alert("Please select at least one PO to convert.");
      return;
    }

    const selectedPOs = po.filter(p => selectedIds.includes(p.id));
    const sameCustomer = selectedPOs.every(p => p.customer_name === selectedPOs[0].customer_name);

    if (!sameCustomer) {
      alert("Please select POs from the same customer for conversion.");
      return;
    }
  ...
  const confirmConvert = async () => {
    ...
    try {
      const fullDetails = await Promise.all(
        selectedIds.map(id => axios.get(`${API}/po/${id}`).then(res => res.data))
      );
      ...
      alert("Failed to fetch PO details for conversion.");
    }
  };
    ...
    if (name === "customer_name") {
      if (!value) {
        ...
        const suggestions = po
          .filter((po) =>
            po.customer_name?.toLowerCase().includes(value.toLowerCase())
          )
          .slice(0, 5);
  ...
  const handleSelectAll = () => {
    if (!selectAll) {
      setSelectedIds(po.map((po) => po.id));
    } else {
      setSelectedIds([]);
    }
    setSelectAll((s) => !s);
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!window.confirm("Delete selected po?")) return;
    try {
    ...
    <div className="container-fluid py-4 px-md-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="fw-bold text-primary mb-0">
          <i className="bi bi-file-earmark-plus-fill me-2"></i>Purchase Order
        </h2>
        <div className="d-flex align-items-center gap-3">
          ...
          <div className="btn-group shadow-sm">
            {poPriv?.can_add === 1 && (
              <button className="btn btn-primary d-flex align-items-center fw-bold px-4 shadow-sm" onClick={() => navigate("/poform")}>
      ...
      <div className="card shadow-sm border-0 mb-4 no-print">
        <div className="card-body p-4">
          <div className="row g-3">
            ...
            <div className="col-md-2">
              <label className="form-label fw-bold small text-muted">PO Date</label>
              <input type="date" name="po_date" className="form-control" value={filters.po_date} onChange={handleFilterChange} />
            </div>
            <div className="col-md-3">
        ...
        <div className="card shadow-sm border-0">
          <div className="card-body p-0 text-truncate">
            <div className="table-responsive">
              <table className="table table-hover table-striped mb-0 align-middle" style={{ fontSize: '16px' }}>
                <thead className="table-dark small">
                  <tr>
                    ...
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("id")}>
                      PO NO {sortBy === "id" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("po_date")}>
                      DATE {sortBy === "po_date" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                ...
                <tbody>
                  {po.length === 0 ? (
                    <tr><td colSpan="8" className="text-center py-5 text-muted fw-bold">No POs found.</td></tr>
                  ) : (
                    po.map((p) => (
                      <tr key={p.id}>
                        ...
                        <td className="small">{p.po_date}</td>
                        ...
                        <td className="text-end pe-4 no-print">
                          <div className="d-flex justify-content-end gap-2">
                            <button className="btn btn-outline-secondary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                              ...
                              onClick={() => window.open(`${API}/printpo/po/${p.id}`, "_blank")}>
                            ...
                            {poPriv?.can_update === 1 && (
                              <button className="btn btn-outline-primary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                                ...
                                onClick={() => navigate(`/Editpoform/${p.id}`)}>
          ...
          <div className="card-footer bg-white py-3 border-0 d-flex justify-content-between align-items-center mb-5 no-print">
            <span className="text-muted small">
              Page {pagination?.page || 1} of {pagination?.pages || 1} &bull; Total {pagination?.total || 0} Purchase Orders
            </span>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg">
              <div className="modal-header bg-primary text-white border-0">
                <h5 className="modal-title fw-bold">Convert PO(s)</h5>
                <button type="button" className="btn-close btn-close-white" onClick={() => setShowConvertModal(false)}></button>
              </div>
              <div className="modal-body p-4">
                <p className="text-muted small mb-4">Select the target module to convert selected POs into. Data like customer info and items will be carried over.</p>
                <div className="mb-3">
