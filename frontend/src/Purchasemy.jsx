...
function Purchasemy() {
  ...
  const [purchasePriv, setPurchasePriv] = useState(null);
  ...
  const fetchPurchases = useCallback(async () => {
    ...
    try {
      ...
      if (filters.purchase_person) params.purchase_person = filters.purchase_person;
      if (filters.purchase_date) params.purchase_date = filters.purchase_date;
      ...
      setPurchasePriv(res.data.privileges || {});
      setSelectedIds([]);
      setSelectAll(false);
    } catch (err) {
      console.error("Fetch purchases error:", err);
      alert(err.response?.data?.message || "Error loading purchases");
    } finally {
      setLoading(false);
    }
  ...
  const handleConvert = () => {
    if (selectedIds.length === 0) {
      alert("Please select at least one purchase to convert.");
      return;
    }

    const selectedPurchases = purchases.filter(p => selectedIds.includes(p.id));
    const sameSupplier = selectedPurchases.every(p => p.supplier_name === selectedPurchases[0].supplier_name);

    if (!sameSupplier) {
      alert("Please select purchases from the same supplier for conversion.");
      return;
    }
  ...
  const confirmConvert = async () => {
    ...
    try {
      const fullDetails = await Promise.all(
        selectedIds.map(id => axios.get(`${API}/purchases/${id}`).then(res => res.data))
      );

      const first = fullDetails[0];
      const combinedItems = fullDetails.flatMap(p => p.items);

      const convertData = {
        customer_id: first.supplier_id, // Map supplier to customer_id if needed
        customer_name: first.supplier_name,
        ship_to: first.ship_to || first.supplier_name,
        mobile: first.mobile,
        billing_address: first.billing_address,
        sales_person: first.purchase_person,
      ...
      alert("Failed to fetch purchase details for conversion.");
    }
  };
  ...
  const handleFilterChange = (e) => {
    ...
    if (name === "supplier_name") {
      if (!value) setNameSuggestions([]);
      else {
        const suggestions = purchases
          .filter((p) => p.supplier_name?.toLowerCase().includes(value.toLowerCase()))
          .slice(0, 5);
  ...
  const handleNameSelect = (name) => {
    setFilters((prev) => ({ ...prev, supplier_name: name }));
    setNameSuggestions([]);
    setPage(1);
  };
  ...
  const handleSelectAll = () => {
    if (!selectAll) setSelectedIds(purchases.map((p) => p.id));
    else setSelectedIds([]);
    setSelectAll((s) => !s);
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!window.confirm("Delete selected purchases?")) return;
    try {
  ...
  const goNext = () => { if (pagination && page < pagination.pages) setPage((p) => p + 1); };
    ...
    <div className="container-fluid py-4 px-md-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="fw-bold text-primary mb-0">
          <i className="bi bi-cart-fill me-2"></i>Purchase Management
        </h2>
        <div className="d-flex align-items-center gap-3">
          ...
          <div className="btn-group shadow-sm">
            {purchasePriv?.can_add === 1 && (
              <button className="btn btn-primary d-flex align-items-center fw-bold px-4" onClick={() => navigate("/purchaseform")}>
                <i className="bi bi-plus-lg me-2"></i>New
              </button>
            )}
            {purchasePriv?.can_delete === 1 && (
          ...
          <div className="row g-3">
            <div className="col-md-3">
              ...
              <div className="input-group">
                ...
                <input type="text" name="supplier_name" className="form-control border-start-0 ps-0" placeholder="Search supplier..." value={filters.supplier_name} onChange={handleFilterChange} autoComplete="off" />
              </div>
              {nameSuggestions.length > 0 && (
                <ul className="list-group position-absolute shadow" style={{ zIndex: 1000, marginTop: '5px', maxHeight: "200px", overflowY: "auto" }}>
                  {nameSuggestions.map((s, i) => (
                    <li key={i} className="list-group-item list-group-item-action fw-bold small" onClick={() => handleNameSelect(s.supplier_name)} style={{ cursor: "pointer" }}>
                      {s.supplier_name}
                    </li>
                  ))}
                </ul>
            ...
            <div className="col-md-2">
              <label className="form-label fw-bold small text-muted">Purchase Person</label>
              <input type="text" name="purchase_person" className="form-control" placeholder="Staff name..." value={filters.purchase_person} onChange={handleFilterChange} />
            </div>
            <div className="col-md-2">
              <label className="form-label fw-bold small text-muted">Purchase Date</label>
              <input type="date" name="purchase_date" className="form-control" value={filters.purchase_date} onChange={handleFilterChange} />
            </div>
            <div className="col-md-2">
        ...
        <div className="card shadow-sm border-0">
          <div className="card-body p-0 text-truncate">
            <div className="table-responsive">
              <table className="table table-hover table-striped mb-0 align-middle" style={{ fontSize: '16px' }}>
                <thead className="table-dark small">
                  <tr>
                    ...
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("id")}>
                      PURCHASE NO {sortBy === "id" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("purchase_date")}>
                      DATE {sortBy === "purchase_date" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("supplier_name")}>
                      SUPPLIER {sortBy === "supplier_name" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                    <th className="fw-bold py-3 text-center hide-on-mobile">TOTAL QTY</th>
                ...
                <tbody>
                  {purchases.length === 0 ? (
                    <tr><td colSpan="8" className="text-center py-5 text-muted fw-bold">No purchases found.</td></tr>
                  ) : (
                    purchases.map((p) => (
                      <tr key={p.id}>
                        <td className="ps-4 no-print"><input className="form-check-input" type="checkbox" checked={selectedIds.includes(p.id)} onChange={() => handleCheckboxChange(p.id)} /></td>
                        <td className="fw-bold text-dark">#{p.id}</td>
                        <td className="small">{p.purchase_date}</td>
                        <td className="fw-bold text-dark">{p.supplier_name}</td>
                        <td className="text-center small hide-on-mobile">{p.total_qty}</td>
                        <td className="hide-on-mobile"><span className="badge bg-light text-dark fw-normal border">{p.purchase_person}</span></td>
                        <td className="text-center">
                          <span className={`badge rounded-pill ${p.status === "Paid" ? "bg-success" : "bg-warning text-dark"}`}>
                            {p.status}
                          </span>
                        </td>
                        <td className="text-end pe-4 no-print">
                          <div className="d-flex justify-content-end gap-2">
                            <button className="btn btn-outline-secondary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                              ...
                              onClick={() => window.open(`${API}/printpurchase/purchase/${p.id}`, "_blank")}>
                              <i className="bi bi-printer-fill"></i>
                            </button>
                            {purchasePriv?.can_update === 1 && (
                              <button className="btn btn-outline-primary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                                ...
                                onClick={() => navigate(`/editpurchaseform/${p.id}`)}>
          ...
          <div className="card-footer bg-white py-3 border-0 d-flex justify-content-between align-items-center mb-5 no-print">
            <span className="text-muted small">
              Page {pagination?.page || 1} of {pagination?.pages || 1} &bull; Total {pagination?.total || 0} Purchases
            </span>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg">
              <div className="modal-header bg-primary text-white border-0">
                <h5 className="modal-title fw-bold">Convert Purchase(s)</h5>
                <button type="button" className="btn-close btn-close-white" onClick={() => setShowConvertModal(false)}></button>
              </div>
              <div className="modal-body p-4">
                <p className="text-muted small mb-4">Select the target module to convert selected purchases into. Data like supplier info and items will be carried over.</p>
                <div className="mb-3">
