...
function PrintModal({ rec, items, onClose }) {
    ...
    const downloadPDF = () => {
        ...
        const opt = {
            margin: 0,
            filename: `Lot-Fabric-Tops-Outward-${rec.outward_no || rec.id}.pdf`,
                ...
                <div className="yd-no-print d-flex align-items-center justify-content-between px-4 py-3 border-bottom sticky-top bg-white rounded-top-4" style={{ zIndex: 10 }}>
                    <div className="d-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <div className="text-muted" style={{ fontSize: "0.65rem" }}>Professional Fabric Tops Outward</div>
                        </div>
                    </div>
                ...
                <div id="yd-print-content" style={{ padding: "40px", fontFamily: "'Inter', 'Segoe UI', sans-serif", color: "#000", background: "#fff" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "30px" }}>
                        ...
                        <div style={{ textAlign: "right" }}>
                            <div style={{ fontSize: "22px", fontWeight: "900", letterSpacing: "1px", textTransform: "uppercase", color: "#000" }}>FABRIC TOPS OUTWARD</div>
                        ...
                        <table style={{ width: "100%", borderCollapse: "collapse" }}>
                            <thead>
                                <tr className="print-black-header" style={{ background: "#000", color: "#fff" }}>
                                    ...
                                    <th style={{ padding: "8px 10px", fontSize: "10px", textTransform: "uppercase", textAlign: "left", borderRight: "1.5px solid #fff", width: "30%" }}>FABRIC NAME</th>
                                    ...
                                    <th style={{ padding: "8px 10px", fontSize: "10px", textTransform: "uppercase", textAlign: "center", borderRight: "1.5px solid #fff", width: "15%" }}>COLOR</th>
                            ...
                            <tbody>
                                ...
                                {(!items || items.length === 0) && (
                                    <tr><td colSpan={6} style={{ padding: "30px", textAlign: "center", fontStyle: "italic", color: "#9ca3af" }}>No items records found</td></tr>
                                )}
                            </tbody>
                            <tfoot>
                                <tr style={{ background: "#f9fafb", borderTop: "1.5px solid #000" }}>
                                    <td colSpan={5} style={{ padding: "10px", textAlign: "right", fontSize: "11px", fontWeight: "800", borderRight: "1.5px solid #000" }}>TOTAL:</td>
...
export default function LotJobworkFabricToPcsOutwardList() {
    ...
    const fetchData = useCallback(async () => {
        ...
        setSelected(new Set());
        try {
            const params = new URLSearchParams({ type: "lot", from_date: filters.from_date, to_date: filters.to_date });
            const res = await axios.get(`${API}/fabric-to-pcs-outward?${params}`);
    ...
    const toggleExpand = async (rec) => {
        ...
        if (!itemsMap[rec.id]) {
            try {
                const res = await axios.get(`${API}/fabric-to-pcs-outward/${rec.id}`);
    ...
    const handleDelete = async (id) => {
        ...
        await axios.delete(`${API}/fabric-to-pcs-outward/${id}`);
        fetchData();
    };
    ...
    const bulkDelete = async () => {
        ...
        try {
            await axios.delete(`${API}/fabric-to-pcs-outward/bulk`, { data: { ids: Array.from(selected) } });
            fetchData();
        ...
        <div className="container-fluid py-4 min-vh-100 bg-light px-3 px-md-4">
            ...
            <div className="mx-auto" style={{ maxWidth: "1500px" }}>
                    ...
                    <div className="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <div className="d-flex align-items-center gap-2 mb-1">
                                <span className="badge bg-dark rounded-pill px-3 py-2 text-white" style={{ fontSize: "0.65rem", letterSpacing: "0.05em" }}>LOT JOBWORK</span>
                                <span className="badge rounded-pill px-3 py-2 text-white" style={{ background: "#3b82f6", fontSize: "0.65rem" }}>Fabric Tops Outward</span>
                            </div>
                            <h2 className="fw-900 mb-0 ls-1 text-dark" style={{ fontSize: "1.75rem" }}>Fabric Outward List</h2>
                        </div>
                    </div>
                    <div className="d-flex gap-2">
                        {selected.size > 0 && (
                        ...
                        <button className="btn btn-primary btn-premium shadow-sm fw-bold" style={{ background: "#3b82f6", borderColor: "#3b82f6" }} onClick={() => navigate("/lot-jobwork-fabric-to-pcs-outward")}>
                            <i className="bi bi-plus-lg me-2"></i>New Outward
                        </button>
                    </div>
                </div>
                ...
                <div className="row g-3 mb-4">
                    {[
                        { label: "Total Outwards", value: records.length, grad: "#3b82f6, #2563eb", icon: "bi-layers" },
                ...
                <div className="card mb-4 overflow-hidden">
                    <div className="card-body p-4 bg-white border-bottom">
                        <div className="row g-3">
                            ...
                            <div className="col-md-2">
                                <label className="form-label small fw-bold text-muted mb-2 text-uppercase ls-1" style={{ fontSize: '0.65rem' }}>Outward No.</label>
                                <input type="text" className="form-control form-control-premium" name="outward_no" value={filters.outward_no} onChange={handleFilter} placeholder="Search outward..." />
                            </div>
                            <div className="col-md-2">
                                <label className="form-label small fw-bold text-muted mb-2 text-uppercase ls-1" style={{ fontSize: '0.65rem' }}>Lot No.</label>
                                <input type="text" className="form-control form-control-premium" name="lot_no" value={filters.lot_no} onChange={handleFilter} placeholder="Search lot..." />
                            </div>
                                ...
                                {loading ? (
                                    ...
                                    <React.Fragment key={r.id}>
                                        <tr className={selected.has(r.id) ? "table-light" : ""}>
                                            ...
                                            <td>
                                                <span className="badge rounded-pill px-3 py-2 fw-bold" style={{ background: "#eff6ff", color: "#2563eb", fontSize: '0.75rem' }}>
                                                    {r.outward_no}
                                                </span>
                                            </td>
                                            <td className="fw-medium text-dark">{fmt(r.outward_date)}</td>
                                            <td>
                                                <div className="fw-bold text-dark">{r.lot_no || "—"}</div>
                                                <div className="text-muted small" style={{ fontSize: '0.7rem' }}>{r.lot_name || "—"}</div>
                                            </td>
                                            <td>
                                                <div className="fw-semibold text-dark">{r.party_name || "—"}</div>
                                            </td>
                                            <td>
                                                <span className="badge bg-light text-primary fw-bold border rounded-pill px-3" style={{ fontSize: '0.7rem' }}>
                                                    {r.process || "—"}
                                                </span>
                                            </td>
                                            ...
                                            <td className="text-end pe-4">
                                                <div className="d-flex justify-content-end gap-2">
                                                    ...
                                                    <button className="btn btn-sm btn-light rounded-circle border-0 action-btn p-0"
                                                        style={{ width: 32, height: 32, background: '#eff6ff' }} onClick={() => navigate(`/edit-lot-jobwork-fabric-to-pcs-outward/${r.id}`)} title="Edit">
                                                ...
                                                <td colSpan={9} className="p-0 border-0">
                                                    <div className="bg-light p-4 mx-4 mb-4 rounded-4 shadow-sm animate__animated animate__fadeIn">
                                                        <div className="mb-3 d-flex justify-content-between align-items-center">
                                                            <h6 className="fw-bold m-0 text-uppercase ls-1 text-primary" style={{ fontSize: '0.75rem' }}>Detailed Item Information</h6>
