...
export default function OrderJobworkPcsOutward() {
    ...
    const fetchInitial = useCallback(async () => {
        try {
            const [noRes, orderRes, empRes, procRes, chartRes, contractRes] = await Promise.all([
                axios.get(`${API}/pcs-outward/next-no/order`),
    ...
    const handleHeader = (e) => {
        ...
        if (name === "work_type") {
            setForm(p => ({
                ...
                contractor_name: value === "Contractor" ? p.contractor_name : ""
            }));
        } else {
    ...
    const handleLoadPrevious = async () => {
        ...
        try {
            let res;
            if (form.previous_process.toLowerCase().includes("cutting")) {
                res = await axios.get(`${API}/fabric-to-pcs-inward/order-process-items`, {
                    params: { order_no: form.order_no, process: "Cutting" }
                });
            } else {
                res = await axios.get(`${API}/pcs-inward/order-process-items`, {
            ...
            if (filteredItems.length === 0) {
                toast(`All quantities from ${form.previous_process} have already been processed.`, "info");
                return;
            }

            // Group items by style and color to show in one row (Size Chart)
            const groupedMap = {};
            filteredItems.forEach(it => {
                const styleKey = (it.style_name || it.item_name || it.fabric_name || "").trim().toLowerCase();
                const colorKey = (it.style_color || it.fabric_color || it.color || "").trim().toLowerCase();
                ...
                if (sizesData && Object.keys(sizesData).length > 0) {
                    Object.entries(sizesData).forEach(([sz, qty]) => {
                        const val = parseFloat(qty) || 0;
                        groupedMap[key].sizes_data[sz] = (groupedMap[key].sizes_data[sz] || 0) + val;
                    });
                } else if (it.size) {
            ...
            const loadedRows = Object.values(groupedMap).map(r => {
                const rowTotal = Object.values(r.sizes_data).reduce((a, b) => a + b, 0) + (r.pcs || 0);
                return { ...r, pcs: rowTotal, max_pcs: rowTotal };
            });
            ...
            toast(`Loaded ${loadedRows.length} styles from ${form.previous_process} inward.`, "success");
        } catch (err) {
    ...
    const handleContractorKeyDown = (e, index) => {
        if (!contractorSuggestions.length) return;
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const res = await axios.post(`${API}/pcs-outward`, payload);
            ...
            <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom">
                ...
                <div>
                    <div className="d-flex align-items-center gap-2">
                        <span className="badge bg-primary rounded-pill px-3" style={{ fontSize: "0.7rem", letterSpacing: "0.06em" }}>ORDER JOBWORK</span>
                    ...
                    <h4 className="fw-bold mb-0 mt-1 text-dark">
                        <i className="bi bi-box-arrow-up-right me-2 text-primary"></i>Pcs Outward
                    </h4>
                </div>
                ...
                <div className="card-body p-4">
                    ...
                    <div className="row g-3 mb-3">
                        ...
                        <div className="col-md-3">
                            ...
                            <Select
                                options={orders.map(o => ({ value: o.order_no, label: o.order_no, ...o }))}
                        ...
                        <div className="col-md-2">
                            ...
                            <select className={`form-select ${errors.process ? "is-invalid" : ""} fw-bold text-primary`} name="process" value={form.process} onChange={handleHeader} disabled={form.rows.some(r => r.item_name || r.pcs)}>
                                <option value="">— Select —</option>
                                {lifecycle
                                    .filter(p => (p.process_type || "").toUpperCase() === "PCS" && (p.process_name || "").toLowerCase() !== "cutting" && (p.custom_name || "").toLowerCase() !== "cutting")
                                    .map((p, i) => {
                        ...
                        <div className="col-md-2">
                            ...
                            <div className="d-flex gap-2">
                                ...
                                {form.previous_process && (
                                    <button className="btn btn-dark btn-sm rounded-3 px-3" onClick={handleLoadPrevious} disabled={form.rows.some(r => r.item_name || r.pcs)}>Load</button>
                                )}
                            </div>
                        </div>
                        <div className="col-md-2">
                    ...
                    <div className="row g-3 mb-4 align-items-end">
                        <div className="col-md-3">
                            ...
                            <div className="btn-group w-100 shadow-sm" style={{ borderRadius: "10px", overflow: "hidden" }}>
                                ...
                                <label className="btn btn-outline-primary py-2 fw-bold" htmlFor="wt_jobwork">Jobwork</label>
                                ...
                                <label className="btn btn-outline-primary py-2 fw-bold" htmlFor="wt_contractor">Contractor</label>
                            </div>
                        </div>
                            ...
                            <thead className="table-dark">
                                {form.size_chart_name ? (
                                    <tr style={{ fontSize: "0.75rem" }}>
                                        ...
                                        <th style={{ width: "80px" }} className="text-center text-primary fw-bold">TOTAL</th>
                                ...
                                {form.rows.map((row, idx) => {
                                    ...
                                    if (form.size_chart_name) {
                                        return (
                                            <tr key={idx}>
                                                ...
                                                <td className="text-center fw-bold text-primary" style={{ backgroundColor: "#f0f7ff" }}>{row.pcs || 0}</td>
                                    ...
                                    return (
                                        <tr key={idx}>
                                            ...
                                            <td>
                                                <input type="text" className="form-control form-control-sm" name="size" value={row.size} onChange={(e) => handleRowChange(idx, e)} placeholder="Size" />
                                            </td>
                                            {form.work_type === "Contractor" && (
                                                <td className="position-relative p-1" ref={activeContractorRowIdx === idx ? contractorRef : null}>
                                                    <input type="text" className="form-control form-control-sm"
