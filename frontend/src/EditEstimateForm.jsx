...
function EditEstimateForm() {
  const { id } = useParams();
  const [estimate, setEstimate] = useState(getInitialState());
  ...
  useEffect(() => {
    document.title = `Edit Estimate No: ${id || ""} - TSL ERP`;
  }, [id]);
  ...
  useEffect(() => {
    axios.get(`${API}/templates`)
      .then((res) => {
        ...
        const estimateTpl = templates.find((t) => t.template_name?.toLowerCase() === "estimate");
        if (!estimateTpl || !estimateTpl.stock_action) {
          setEstimate((prev) => ({ ...prev, template_id: null, stock_action: null }));
          setError("No valid 'estimate' template found.");
          return;
        }
        setEstimate((prev) => ({ ...prev, stock_action: estimateTpl.stock_action, template_id: estimateTpl.id }));
      })
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/estimate/${id}`)
      .then((res) => {


        const raw = res.data;
        const data = raw.estimate ? raw.estimate : raw;       // supports both formats
  ...
  const addItem = () => setEstimate((prev) => ({ ...prev, items: [...prev.items, { ...initialItem }] }));
  const removeItem = (index) => estimate.items.length > 1 && setEstimate((prev) => ({
    ...prev,
    items: prev.items.filter((_, i) => i !== index),
  }));

  const toggleIGST = () => setEstimate((prev) => ({ ...prev, igst: !prev.igst }));

  // Save invoice
  const saveEstimate = async () => {
    if (!estimate.template_id) return setError("Cannot save estimate: valid 'estimate' template not selected.");
    if (!estimate.items.length) return setError("Cannot save estimate: No items added.");

    try {
      const payload = {
        ...estimate,
        estimate_date: inputToDMY(estimate.estimate_date),
        due_date: inputToDMY(estimate.due_date),
      };
      if (id) {
        const res = await axios.put(`${API}/estimate/${id}`, payload);


        setSuccess(res.data.message || "estimate updated successfully!");
      } else {
        const res = await axios.post(`${API}/estimate`, payload);
        setSuccess(res.data.message || "estimate saved successfully!");
      }
      setTimeout(() => navigate("/estimatemy"), 1000);
    } catch (err) {
      console.error(err.response?.data || err.message);
      setError(err.response?.data?.error || "Error saving estimate");
    }
  };

  const clearForm = () => window.confirm("Clear form?") && setEstimate(getInitialState());
    ...
    <div className="container my-4">
      ...
      <div className="card shadow p-4">
        <div style={{ position: "relative", marginBottom: "1rem" }}>
          ...
          >
            <label className="fw-bold mb-0 me-1 text-muted small">Estimate No:</label>
            <span className="text-primary">{id || "Loading..."}</span>
          </div>
          <h3 className="mb-0 text-center">Estimate Form</h3>
        </div>
        ...
        <div className="card shadow p-3 mb-3 mt-4">
          <div className="row mb-3">
            ...
            <div className="col-md-6 mb-2">
              ...
              <input type="text" name="ship_to" value={estimate.ship_to} onChange={handleHeaderChange} className="form-control" />
            </div>

            <div className="col-md-6 mb-2">
              <label className="form-label">Sales Person</label>
              <select name="sales_person" value={estimate.sales_person} onChange={handleHeaderChange} className="form-select">
            ...
            <div className="col-md-6 mb-2">
              ...
              <input type="text" name="mobile" value={estimate.mobile} onChange={handleHeaderChange} className="form-control" />
            </div>
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                type="date"
                name="estimate_date"
                value={estimate.estimate_date ? dmyToInput(estimate.estimate_date) : ""}
                onChange={(e) =>
                  setEstimate({
                    ...estimate,
                    estimate_date: inputToDMY(e.target.value),
                  })
                }
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                type="date"
                name="due_date"
                value={estimate.due_date ? dmyToInput(estimate.due_date) : ""}
                onChange={(e) =>
                  setEstimate({
                    ...estimate,
            ...
            <div className="col-md-12 mb-2">
              ...
              <textarea name="billing_address" value={estimate.billing_address} onChange={handleHeaderChange} className="form-control" />
            </div>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end mb-3">
          <div className="form-check">
            <input type="checkbox" className="form-check-input" checked={estimate.igst} onChange={toggleIGST} />
            ...
            <tfoot>
              <tr>
                ...
                <td className="fw-bold">
                  {(() => {
                    const qtyTotal = estimate.items.reduce(
                      (sum, item) => sum + Number(item.qty || 0),
                      0
                    );
                    estimate.total_qty = qtyTotal; // store in invoice
                    return qtyTotal;
                  })()}
                </td>
                <td colSpan="6"></td>
        ...
        <div className="border p-3 bg-light">
          <div className="d-flex justify-content-between mb-2">
            ...
            <input value={estimate.discount_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          {estimate.igst ? (
            <div className="d-flex justify-content-between mb-2">
              <label>IGST:</label>
              <input value={estimate.gst_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
            </div>
          ) : (
            <>
              <div className="d-flex justify-content-between mb-2">
                <label>CGST:</label>
                <input value={(estimate.gst_total / 2).toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
              </div>
              <div className="d-flex justify-content-between mb-2">
                <label>SGST:</label>
                <input value={(estimate.gst_total / 2).toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input value={estimate.net_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          <div className="d-flex justify-content-between mb-2">
            <label>Round Off:</label>
            <input value={estimate.round_off.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          <div className="d-flex justify-content-between">
            <label>Grand Total:</label>
            <input
              value={estimate.grand_total.toFixed(2)}
              readOnly
        ...
        <div className="d-flex justify-content-end mt-3">
          ...
          <button onClick={saveEstimate} className="btn btn-success">
            Save Estimate
          </button>
        </div>
      </div>
