...
function EditQuotationForm() {
  const { id } = useParams();
  const [quotation, setQuotation] = useState(getInitialState());
  ...
  useEffect(() => {
    document.title = `Edit Quotation No: ${id || ""} - TSL ERP`;
  }, [id]);
  ...
  useEffect(() => {
    axios.get(`${API}/templates`)
      .then((res) => {
        ...
        const quotationTpl = templates.find((t) => t.template_name?.toLowerCase() === "quotation");
        if (!quotationTpl || !quotationTpl.stock_action) {
          setQuotation((prev) => ({ ...prev, template_id: null, stock_action: null }));
          setError("No valid 'quotation' template found.");
          return;
        }
        setQuotation((prev) => ({ ...prev, stock_action: quotationTpl.stock_action, template_id: quotationTpl.id }));
      })
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/quotation/${id}`)
      .then((res) => {
        ...
        const raw = res.data;
        const data = raw.quotation ? raw.quotation : raw;       // supports both formats
  ...
  const addItem = () => setQuotation((prev) => ({ ...prev, items: [...prev.items, { ...initialItem }] }));
  const removeItem = (index) => quotation.items.length > 1 && setQuotation((prev) => ({
    ...prev,
    items: prev.items.filter((_, i) => i !== index),
  }));

  const toggleIGST = () => setQuotation((prev) => ({ ...prev, igst: !prev.igst }));

  // Save invoice
  const saveQuotation = async () => {
    if (!quotation.template_id) return setError("Cannot save quotation: valid 'quotation' template not selected.");
    if (!quotation.items.length) return setError("Cannot save quotation: No items added.");

    try {
      const payload = {
        ...quotation,
        quotation_date: inputToDMY(quotation.quotation_date),
        due_date: inputToDMY(quotation.due_date),
      };
      if (id) {
        const res = await axios.put(`${API}/quotation/${id}`, payload);


        setSuccess(res.data.message || "quotation updated successfully!");
      } else {
        const res = await axios.post(`${API}/quotation`, payload);
        setSuccess(res.data.message || "quotation saved successfully!");
      }
      setTimeout(() => navigate("/quotationmy"), 1000);
    } catch (err) {
      console.error(err.response?.data || err.message);
      setError(err.response?.data?.error || "Error saving quotation");
    }
  };

  const clearForm = () => window.confirm("Clear form?") && setQuotation(getInitialState());
    ...
    <div className="container my-4">
      ...
      <div className="card shadow p-4">
        <div style={{ position: "relative", marginBottom: "1rem" }}>
          ...
          >
            <label className="fw-bold mb-0 me-1 text-muted small">Quotation No:</label>
            <span className="text-primary">{id || "Loading..."}</span>
          </div>
          <h3 className="mb-0 text-center">Quotation Form</h3>
        </div>
        ...
        <div className="card shadow p-3 mb-3 mt-4">
          <div className="row mb-3">
            ...
            <div className="col-md-6 mb-2">
              ...
              <input type="text" name="ship_to" value={quotation.ship_to} onChange={handleHeaderChange} className="form-control" />
            </div>

            <div className="col-md-6 mb-2">
              <label className="form-label">Sales Person</label>
              <select name="sales_person" value={quotation.sales_person} onChange={handleHeaderChange} className="form-select">
            ...
            <div className="col-md-6 mb-2">
              ...
              <input type="text" name="mobile" value={quotation.mobile} onChange={handleHeaderChange} className="form-control" />
            </div>
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                type="date"
                name="quotation_date"
                value={quotation.quotation_date ? dmyToInput(quotation.quotation_date) : ""}
                onChange={(e) =>
                  setQuotation({
                    ...quotation,
                    quotation_date: inputToDMY(e.target.value),
                  })
                }
            ...
            <div className="col-md-6 mb-2">
              ...
              <input
                type="date"
                name="due_date"
                value={quotation.due_date ? dmyToInput(quotation.due_date) : ""}
                onChange={(e) =>
                  setQuotation({
                    ...quotation,
            ...
            <div className="col-md-12 mb-2">
              ...
              <textarea name="billing_address" value={quotation.billing_address} onChange={handleHeaderChange} className="form-control" />
            </div>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end mb-3">
          <div className="form-check">
            <input type="checkbox" className="form-check-input" checked={quotation.igst} onChange={toggleIGST} />
            ...
            <tfoot>
              <tr>
                ...
                <td className="fw-bold">
                  {(() => {
                    const qtyTotal = quotation.items.reduce(
                      (sum, item) => sum + Number(item.qty || 0),
                      0
                    );
                    quotation.total_qty = qtyTotal; // store in invoice
                    return qtyTotal;
                  })()}
                </td>
                <td colSpan="6"></td>
        ...
        <div className="border p-3 bg-light">
          <div className="d-flex justify-content-between mb-2">
            ...
            <input value={quotation.discount_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          {quotation.igst ? (
            <div className="d-flex justify-content-between mb-2">
              <label>IGST:</label>
              <input value={quotation.gst_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
            </div>
          ) : (
            <>
              <div className="d-flex justify-content-between mb-2">
                <label>CGST:</label>
                <input value={(quotation.gst_total / 2).toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
              </div>
              <div className="d-flex justify-content-between mb-2">
                <label>SGST:</label>
                <input value={(quotation.gst_total / 2).toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          ...
          <div className="d-flex justify-content-between mb-2">
            ...
            <input value={quotation.net_total.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          <div className="d-flex justify-content-between mb-2">
            <label>Round Off:</label>
            <input value={quotation.round_off.toFixed(2)} readOnly className="form-control form-control-sm text-end w-25" />
          </div>

          <div className="d-flex justify-content-between">
            <label>Grand Total:</label>
            <input
              value={quotation.grand_total.toFixed(2)}
              readOnly
