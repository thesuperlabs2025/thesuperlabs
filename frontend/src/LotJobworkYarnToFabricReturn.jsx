...
export default function LotJobworkYarnToFabricReturn() {
    ...
    const fetchInitial = useCallback(async () => {
        try {
            const [noRes, lotRes, empRes] = await Promise.all([
                axios.get(`${API}/yarn-dyeing-return/next-no/lot`),
    ...
    const loadPending = async () => {
        ...
        try {
            ...
            if (items.length === 0) {
                toast("No pending balance items found for this Lot & Process.", "info");
                return;
            }
            setForm(prev => ({
                ...prev,
