...
const TrimsGRN = () => {
    ...
    const [items, setItems] = useState([
        { trims_name: "", trims_sku: "", color: "", size: "", qty: "" }
    ]);
        ...
        const fetchInitialData = async () => {
            try {
                const [grnRes, supRes, staffRes, orderRes, lotRes] = await Promise.all([
                    axios.get(`${API}/trims-grn/next-no/gen`),
                ...
                if (location.state && location.state.po_data) {
                    ...
                    const fullPoRes = await axios.get(`${API}/trims-po/${po.id}`);
    ...
    const addItem = () => {
        setItems([...items, { trims_name: "", trims_sku: "", color: "", size: "", qty: "" }]);
    };
    ...
    const saveGRN = async () => {
        ...
        if (header.po_no) {
            for (const item of items) {
                ...
                if (item.po_qty && Number(item.qty || 0) > Number(remaining.toFixed(3)) + 0.0001) {
                    return toast.error(`Quantity for ${item.trims_name} exceeds remaining PO quantity (${remaining.toFixed(3)})`);
                }
            }
        }
        ...
        try {
            await axios.post(`${API}/trims-grn`, {
                grn_no: grnNo,
                ...header,
                year_id: selectedYear.year_id,
                items
            });
            toast.success("Trims GRN Created Successfully!");
            setTimeout(() => navigate('/trims-grn-list'), 1500);
        } catch (err) {
            ...
            <div className="card shadow border-0 rounded-4">
                <div className="card-header bg-white py-3 border-bottom shadow-sm">
                    <div className="d-flex justify-content-between align-items-center">
                        ...
                        <div className="text-center flex-grow-1">
                            <h4 className="fw-bold mb-0 text-uppercase tracking-wider">Trims GRN</h4>
                        </div>
                ...
                <div className="card-body p-4">
                        ...
                        <table className="table table-bordered table-hover align-middle">
                            ...
                            <tbody>
                                {items.map((item, index) => (
                                    <tr key={index}>
                                        <td><input type="text" name="trims_sku" className="form-control form-control-sm" value={item.trims_sku} onChange={(e) => handleItemChange(index, e)} /></td>
                    ...
                    <div className="d-flex justify-content-end gap-2">
                        <button className="btn btn-secondary rounded-pill px-4" onClick={() => navigate('/trims-grn-list')}>Cancel</button>
                        {selectedYear.is_closed ? (
                            ...
                            <button className="btn btn-success rounded-pill px-4 fw-bold shadow-sm" onClick={saveGRN}>
                                <i className="bi bi-check-lg me-2"></i>Save Trims GRN
                            </button>
                        )}
                    </div>
                </div>
