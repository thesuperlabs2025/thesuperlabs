...
function Creditnotemy() {
  const [creditnote, setCreditnote] = useState([]);
  const [loading, setLoading] = useState(true);

  const [selectedIds, setSelectedIds] = useState([]);
  const [selectAll, setSelectAll] = useState(false);
  const [creditnotePriv, setCreditnotePriv] = useState(null);
  ...
  const fetchCreditnote = useCallback(async () => {
    ...
    try {
      const params = {
        ...
        sort_by: sortBy,
        order,
        usertype_id: user.usertype_id, // ✅ same as Estimate
      };
      ...
      if (filters.credit_note_date)
        params.credit_note_date = filters.credit_note_date;
      if (filters.due_date) params.due_date = filters.due_date;
      if (filters.status) params.status = filters.status;

      const res = await axios.get(`${API}/creditnote`, { params });

      setCreditnote(res.data.data || []);

      // ✅ same pagination fallback logic as Estimate / DC / GRN
      setPagination(
        res.data.pagination || {
          page: 1,
          limit: 20,
      ...
      setCreditnotePriv(res.data.privileges || {});

      setSelectedIds([]);
      setSelectAll(false);
    } catch (err) {
      console.error("Fetch Credit Note Error:", err);
      alert(err.response?.data?.message || "Error loading Credit Note");
    } finally {
      setLoading(false);
    }
  ...
  const handleConvert = () => {
    if (selectedIds.length === 0) {
      alert("Please select at least one Credit Note to convert.");
      return;
    }

    const selectedNotes = creditnote.filter(c => selectedIds.includes(c.id));
    const sameCustomer = selectedNotes.every(c => c.customer_name === selectedNotes[0].customer_name);

    if (!sameCustomer) {
      alert("Please select Credit Notes from the same customer for conversion.");
      return;
    }
  ...
  const confirmConvert = async () => {
    ...
    try {
      const fullDetails = await Promise.all(
        selectedIds.map(id => axios.get(`${API}/creditnote/${id}`).then(res => res.data))
      );

      const first = fullDetails[0];
      const combinedItems = fullDetails.flatMap(c => c.items);
      ...
      alert("Failed to fetch credit note details for conversion.");
    }
  };
  ...
  //     if (filters.mobile) params.mobile = filters.mobile;
  //     if (filters.sales_person) params.sales_person = filters.sales_person;
  //     if (filters.credit_note_date) params.credit_note_date = filters.credit_note_date;
  //     if (filters.due_date) params.due_date = filters.due_date;
  //     if (filters.status) params.status = filters.status;
  ...
  const handleFilterChange = (e) => {
    ...
    if (name === "customer_name") {
      if (!value) {
        ...
        const suggestions = creditnote
          .filter((creditnote) =>
            creditnote.customer_name?.toLowerCase().includes(value.toLowerCase())
          )
          .slice(0, 5);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API}/credit_note/${id}`);
      alert("Deleted");
      fetchCreditnote();
    } catch (err) {
      console.error(err);
  ...
  const handleSelectAll = () => {
    if (!selectAll) {
      setSelectedIds(creditnote.map((creditnote) => creditnote.id));
    } else {
      setSelectedIds([]);
    }
    setSelectAll((s) => !s);
  ...
  const handleDeleteSelected = async () => {
    if (selectedIds.length === 0) {
      alert("Select at least one credit note");
      return;
    }
    if (!window.confirm("Delete selected credit note?")) return;
    try {
    ...
    <div className="container-fluid py-4 px-md-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="fw-bold text-primary mb-0">
          <i className="bi bi-file-earmark-plus-fill me-2"></i>Credit Note
        </h2>
        <div className="d-flex align-items-center gap-3">
          ...
          <div className="btn-group shadow-sm">
            {creditnotePriv?.can_add === 1 && (
              <button className="btn btn-primary d-flex align-items-center fw-bold px-4 shadow-sm" onClick={() => navigate("/creditnoteform")}>
                <i className="bi bi-plus-lg me-2"></i>New
              </button>
            )}
            {creditnotePriv?.can_delete === 1 && (
              <button className="btn btn-danger d-flex align-items-center text-truncate" onClick={handleDeleteSelected} disabled={selectedIds.length === 0}>
      ...
      <div className="card shadow-sm border-0 mb-4 no-print">
        <div className="card-body p-4">
          <div className="row g-3">
            ...
            <div className="col-md-2">
              <label className="form-label fw-bold small text-muted">Credit Note Date</label>
              <input type="date" name="credit_note_date" className="form-control" value={filters.credit_note_date} onChange={handleFilterChange} />
            </div>
            <div className="col-md-3">
        ...
        <div className="card shadow-sm border-0">
          <div className="card-body p-0 text-truncate">
            <div className="table-responsive">
              <table className="table table-hover table-striped mb-0 align-middle" style={{ fontSize: '16px' }}>
                <thead className="table-dark small">
                  <tr>
                    ...
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("id")}>
                      NOTE NO {sortBy === "id" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("credit_note_date")}>
                      DATE {sortBy === "credit_note_date" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                ...
                <tbody>
                  {creditnote.length === 0 ? (
                    <tr><td colSpan="8" className="text-center py-5 text-muted fw-bold">No Credit Notes found.</td></tr>
                  ) : (
                    creditnote.map((c) => (
                      <tr key={c.id}>
                        <td className="ps-4 no-print"><input className="form-check-input" type="checkbox" checked={selectedIds.includes(c.id)} onChange={() => handleCheckboxChange(c.id)} /></td>
                        <td className="fw-bold text-dark">#{c.id}</td>
                        <td className="small">{c.credit_note_date}</td>
                        <td className="fw-bold text-dark">{c.customer_name}</td>
                        <td className="text-end fw-bold">₹{c.grand_total}</td>
                        <td className="text-center small hide-on-mobile">{c.total_qty}</td>
                        <td className="hide-on-mobile"><span className="badge bg-light text-dark fw-normal border">{c.sales_person}</span></td>
                        <td className="text-end pe-4 no-print">
                          <div className="d-flex justify-content-end gap-2">
                            <button className="btn btn-outline-secondary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                              ...
                              onClick={() => window.open(`${API}/printcreditnote/creditnote/${c.id}`, "_blank")}>
                              <i className="bi bi-printer-fill"></i>
                            </button>
                            {creditnotePriv?.can_update === 1 && (
                              <button className="btn btn-outline-primary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                                ...
                                onClick={() => navigate(`/Editcreditnoteform/${c.id}`)}>
          ...
          <div className="card-footer bg-white py-3 border-0 d-flex justify-content-between align-items-center mb-5 no-print">
            <span className="text-muted small">
              Page {pagination?.page || 1} of {pagination?.pages || 1} &bull; Total {pagination?.total || 0} Credit Notes
            </span>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg">
              <div className="modal-header bg-primary text-white border-0">
                <h5 className="modal-title fw-bold">Convert Credit Note(s)</h5>
                <button type="button" className="btn-close btn-close-white" onClick={() => setShowConvertModal(false)}></button>
              </div>
              <div className="modal-body p-4">
                <p className="text-muted small mb-4">Select the target module to convert selected Credit Notes into. Data like customer info and items will be carried over.</p>
                <div className="mb-3">
