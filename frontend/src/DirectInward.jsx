...
const DirectInward = () => {
    ...
    const fetchNextNo = async () => {
        try {
            const res = await axios.get(`${API}/direct-inward/next-no`);
    ...
    const fetchInward = async (id) => {
        try {
            const res = await axios.get(`${API}/direct-inward/${id}`);
            const data = res.data;
            setFormData({
                ...data,
                ...
                items: data.items.map(i => ({
                    ...
                    qty: i.qty,
                    sku: i.sku,
                    display_value: i.sku || i.item_name
                }))
            });
        } catch (err) {
            console.error(err);
    ...
    const selectProduct = (index, product) => {
        const newItems = [...formData.items];
        newItems[index].item_name = product.product_name;
        newItems[index].sku = product.sku;
        newItems[index].display_value = product.sku || product.product_name;
        setFormData(prev => ({ ...prev, items: newItems }));
        setItemSuggestions([]);
        setActiveIndex(null);
    ...
    const handleSubmit = async () => {
        ...
        if (formData.items.some(i => !i.item_name || !i.qty)) return toast.error("Please complete all items");
        ...
        try {
            if (location.state && location.state.id) {
                await axios.put(`${API}/direct-inward/${location.state.id}`, payload);
                toast.success("Updated successfully");
            } else {
                await axios.post(`${API}/direct-inward`, payload);
                toast.success("Created successfully");
            }
            navigate("/direct-inward");
        } catch (err) {
            console.error(err);
    ...
    const handleKeyDown = (e, index, type) => {
        if (type === 'supplier') {
            ...
            if (e.key === 'ArrowDown') {
                setHighlightedIndex(prev => Math.min(prev + 1, itemSuggestions.length - 1));
            } else if (e.key === 'ArrowUp') {
                setHighlightedIndex(prev => Math.max(prev - 1, 0));
            } else if (e.key === 'Enter' && highlightedIndex >= 0) {
                selectProduct(index, itemSuggestions[highlightedIndex]);
            }
        }
    };
    ...
    return (
        <div className="container-fluid py-4 bg-light min-vh-100">
            ...
            <div className="mx-auto" style={{ maxWidth: '1200px' }}>
                <div className="mb-4" style={{
                    ...
                    <div className="d-flex align-items-center gap-4">
                        <div>
                            <h4 className="fw-bold mb-1">General Inward</h4>
                        ...
                        {selectedYear.is_closed ? (
                            <span className="badge bg-danger border border-white border-opacity-25 px-3 py-2 rounded-pill shadow-sm">
                                <i className="bi bi-lock-fill me-1"></i>LOCKED
                            </span>
                        ) : (
                            <span className="badge bg-success border border-white border-opacity-25 px-3 py-2 rounded-pill shadow-sm">
                                    ...
                                    {supplierSuggestions.length > 0 && (
                                        <ul className="list-group position-absolute w-100 shadow mt-1" style={{ zIndex: 1000, borderRadius: '10px', overflow: 'hidden' }}>
                                            {supplierSuggestions.map((s, i) => (
                                                <li
                                                    key={i}
                                                    className={`list-group-item list-group-item-action border-0 ${highlightedIndex === i ? 'bg-primary text-white' : ''}`}
                        ...
                        <div className="card border-0 shadow-sm h-100" style={{ borderRadius: '20px' }}>
                            <div className="card-body p-3">
                                <div className="d-flex justify-content-between align-items-center mb-3">
                                    <h6 className="fw-bold text-dark m-0">Inward Items (General)</h6>
                                    <button
                                        className="btn btn-sm px-2 py-1 fw-bold text-primary"
                                        style={{ background: '#eff6ff', borderRadius: '8px', fontSize: '0.8rem' }}
                                        onClick={addItem}
                                    >
                                    ...
                                    <table className="table table-borderless align-middle">
                                        <thead>
                                            <tr className="border-bottom">
                                                <th className="text-muted x-small fw-bold py-2" width="60%">PRODUCT / SKU</th>
                                                <th className="text-muted x-small fw-bold py-2" width="25%">QTY</th>
                                                ...
                                                <tr key={index} className="border-bottom border-light">
                                                    <td className="py-2">
                                                        <div className="position-relative">
                                                            ...
                                                            />
                                                            <AnimatePresence>
                                                                {activeIndex === index && itemSuggestions.length > 0 && (
                                                                    <motion.ul
                                                                        ...
                                                                        {itemSuggestions.map((p, i) => (
                                                                            ...
                                                                            >
                                                                                <div className="d-flex justify-content-between">
                                                                                    <span className="fw-bold">{p.sku}</span>
                                                                                    <small className={`${highlightedIndex === i ? 'text-white' : 'text-muted'} opacity-75`}>{p.product_name}</small>
                                                                                </div>
                                                                            </li>
                                                                        ))}
                                                                    </motion.ul>
                            ...
                            <div className="card-footer bg-white border-0 p-3 pt-0 text-end">
                                <div className="d-flex justify-content-end gap-2 align-items-center">
                                    <button className="btn px-4 py-2 border-0 text-muted fw-bold" style={{ borderRadius: '14px', background: '#f8fafc', fontSize: '0.9rem' }} onClick={() => navigate("/direct-inward")}>Cancel</button>
                                    {selectedYear.is_closed ? (
                                        ...
                                        <motion.button
                                            ...
                                            style={{ borderRadius: '14px', background: 'linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%)', fontSize: '0.9rem' }}
            ...
            <style>{`
                ...
                .form-control:focus { box-shadow: none; background-color: #fff !important; border-color: #3b82f6 !important; }
            `}</style>
        </div>
    );
};
