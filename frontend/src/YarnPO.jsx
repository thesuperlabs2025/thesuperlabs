...
const YarnPO = () => {
    ...
    const [items, setItems] = useState([
        { counts: "", yarn_name: "", yarn_sku: "", color: "", per_bag: "", per_bag_qty: "", req_qty: "", qty: "", rate: "", gst_per: "", total: 0 }
    ]);

    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                const [poRes, supRes, staffRes, orderRes, lotRes, yarnRes] = await Promise.all([
                    axios.get(`${API}/yarn-po/next-no/gen`, { headers: { 'x-year-id': selectedYear.year_id } }),
                ...
                // Handle pre-populated data from navigation state
                if (location.state) {
        ...
        fetchInitialData();
    }, [location.state, selectedYear.year_id]);
    ...
    const handleItemChange = (index, e) => {
        ...
        // Auto calculate qty if per_bag and per_bag_qty are set
    ...
    const addItem = () => {
        setItems([...items, { counts: "", yarn_name: "", yarn_sku: "", color: "", per_bag: "", per_bag_qty: "", qty: "", rate: "", gst_per: "", total: 0 }]);
    };
    ...
    const savePO = async () => {
        ...
        try {
            await axios.post(`${API}/yarn-po`, {
                ...
                headers: { 'x-year-id': selectedYear.year_id }
            });
            toast.success("Yarn PO Created Successfully!");
            setTimeout(() => navigate('/yarn-po-list'), 1500);
        } catch (err) {
            ...
            <div className="card shadow border-0 rounded-4">
                <div className="card-header bg-white py-3 border-bottom shadow-sm">
                    <div className="d-flex justify-content-between align-items-center">
                        ...
                        <div className="text-center flex-grow-1">
                            <h4 className="fw-bold mb-0 text-uppercase tracking-wider">Yarn Purchase Order</h4>
                        </div>
                ...
                <div className="card-body p-3">
                        ...
                        <table className="table table-bordered table-hover align-middle">
                            <thead className="table-dark text-white" style={{ fontSize: '0.75rem' }}>
                                <tr>
                                    ...
                                    <th>Yarn Name</th>
                                    <th>Color</th>
                                    <th className="text-center">Per Bag (KG)</th>
                                    <th className="text-center">Per Bag Qty</th>
                                    ...
                                    <tr key={index}>
                                        ...
                                        <td style={{ minWidth: '180px' }}>
                                            <Select
                                                ...
                                                onChange={(opt) => {
                                                    const newItems = [...items];
                                                    newItems[index].yarn_name = opt?.label || "";
                                                    newItems[index].yarn_sku = opt?.sku || "";
                                                    newItems[index].counts = opt?.counts || newItems[index].counts;
                                                    setItems(newItems);
                                                }}
                                                placeholder="Select Yarn"
                                                value={yarns.find(y => y.value === item.yarn_name) || (item.yarn_name ? { label: item.yarn_name, value: item.yarn_name } : null)}
                                                styles={{
                                                    control: (b) => ({ ...b, minHeight: '30px', fontSize: '0.75rem' }),
                                        ...
                                        <td><input type="text" name="per_bag" className="form-control form-control-sm text-center" value={item.per_bag || ""} onChange={(e) => handleItemChange(index, e)} placeholder="0.00" /></td>
                                        <td><input type="text" name="per_bag_qty" className="form-control form-control-sm text-center" value={item.per_bag_qty || ""} onChange={(e) => handleItemChange(index, e)} placeholder="0" /></td>
                    ...
                    <div className="d-flex justify-content-end gap-2">
                        <button className="btn btn-secondary rounded-pill px-4" onClick={() => navigate('/yarn-po-list')}>Cancel</button>
                        {selectedYear.is_closed ? (
                            ...
                            <button className="btn btn-success rounded-pill px-4 fw-bold shadow-sm" onClick={savePO} disabled={loading}>
                                <i className="bi bi-check-lg me-2"></i>{loading ? "Saving..." : "Save Yarn PO"}
                            </button>
                        )}
                    </div>
                </div>
