...
export default function DCLedger() {
    ...
    const handleSearch = async (f = filters) => { try { const res = await axios.get(`${API}/reports/dc/ledger`, { params: f }); setData(res.data); } catch (err) { console.error(err); } };
    ...
    return (
        <div className="container-fluid my-4">
            ...
            <div className="report-title-print"><span>DC Ledger Report</span><span>Period: {formatDate(filters.from_date)} To {formatDate(filters.to_date)}</span></div>
            <div className="d-flex justify-content-between align-items-center mb-4 no-print"><h2 className="fw-bold">DC Ledger Report</h2><button className="btn btn-outline-danger" onClick={printReport}>Print PDF</button></div>
