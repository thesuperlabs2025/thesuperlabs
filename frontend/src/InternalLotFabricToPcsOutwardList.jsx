...
function PrintModal({ rec, items, onClose }) {
    ...
    const downloadPDF = () => {
        ...
        const opt = {
            margin: 0,
            filename: `Int-Lot-Fabric-Tops-Outward-${rec.outward_no || rec.id}.pdf`,
...
export default function InternalLotFabricToPcsOutwardList() {
    const navigate = useNavigate();
    const today = new Date().toISOString().substring(0, 10);
    const [filters, setFilters] = useState({ from_date: "", to_date: today, party_name: "", outward_no: "", internal_lot_no: "" });
    ...
    }, [allRecords, filters.party_name, filters.outward_no, filters.internal_lot_no]);

    const handleFilter = e => setFilters(p => ({ ...p, [e.target.name]: e.target.value }));
    const clearFilters = () => setFilters({ from_date: "", to_date: today, party_name: "", outward_no: "", internal_lot_no: "" });
    ...
    return (
        <div className="container-fluid mt-3 mb-5 px-3 px-md-4">
            ...
            <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom">
                ...
                <div>
                    <div className="d-flex align-items-center gap-2">
                        ...
                        <span className="badge rounded-pill px-3 text-white" style={{ background: "#0ea5e9", fontSize: "0.7rem" }}>Fabric Tops</span>
                    </div>
                    <h4 className="fw-bold mb-0 mt-1 text-dark">
                        <i className="bi bi-journal-text me-2 text-primary"></i>Fabric Tops Outward List
                    </h4>
                </div>
                <div className="ms-auto d-flex gap-2 flex-wrap">
                    ...
                    <button className="btn btn-primary rounded-pill px-3 btn-sm fw-bold shadow-sm" onClick={() => navigate("/internal-lot-fabric-to-pcs-outward")}>
            ...
            <div className="card border-0 shadow-sm rounded-4 mb-4">
                <div className="card-body p-3">
                    <div className="row g-2 align-items-end">
                        ...
                        <div className="col-md-2 col-6">
                            <label className="form-label fw-semibold text-muted mb-1" style={{ fontSize: "0.72rem" }}>OUTWARD NO.</label>
                            <input type="text" className="form-control form-control-sm border-0 bg-light rounded-pill" name="outward_no" value={filters.outward_no} onChange={handleFilter} placeholder="Filter outward..." />
                        </div>
                        <div className="col-md-2 col-6">
                            <label className="form-label fw-semibold text-muted mb-1" style={{ fontSize: "0.72rem" }}>INT. LOT NO.</label>
                            <input type="text" className="form-control form-control-sm border-0 bg-light rounded-pill" name="internal_lot_no" value={filters.internal_lot_no} onChange={handleFilter} placeholder="Filter lot..." />
                        </div>
            ...
            <div className="row g-3 mb-4">
                {[
                    { label: "Total Outwards", value: records.length, grad: "#0ea5e9,#0284c7", icon: "bi-layers" },
                    { label: "Total Qty (KG)", value: totalQty.toFixed(3), grad: "#0284c7,#0369a1", icon: "bi-graph-up-arrow" },
                ].map((c, i) => (
                        ...
                        <table className="table table-hover align-middle mb-0" style={{ fontSize: "0.83rem" }}>
                            <thead className="table-dark">
                                <tr>
                                    ...
                                    <th style={{ width: "13%" }}>OUTWARD NO.</th>
                                        ...
                                        <tr className={selected.has(r.id) ? "table-primary" : ""}>
                                            ...
                                            <td>
                                                <span className="badge fw-semibold px-2 py-1 rounded-pill"
                                                    style={{ background: "#eef2ff", color: "#6366f1", fontSize: "0.73rem" }}>
                                                    {r.outward_no}
                                                </span>
                                            </td>
                                            ...
                                            <td><span className="badge bg-light text-info border-0 small fw-normal">{r.process || "—"}</span></td>
                                            ...
                                            <td className="text-center">
                                                <div className="d-flex gap-1 justify-content-center">
                                                    <button className="btn btn-sm rounded-circle border-0 action-btn"
                                                        style={{ width: 28, height: 28, padding: 0, background: "#f1f5f9", color: "#6366f1" }}
                                                    ...
                                                    <button className="btn btn-sm rounded-circle border-0 action-btn"
                                                        style={{ width: 28, height: 28, padding: 0, background: "#f0fdf4", color: "#10b981" }}
                                                    ...
                                                    <button className="btn btn-sm rounded-circle border-0 action-btn text-warning"
                                                        ...
                                                        title="Edit record" onClick={() => navigate(`/edit-internal-lot-fabric-to-pcs-outward/${r.id}`)}>
                                                ...
                                                <td colSpan={9} className="p-0 border-0">
                                                    <div className="bg-light p-3 mx-3 mb-3 border rounded-4 shadow-sm" style={{ animation: "slideDown 0.2s ease" }}>
                                                        <div className="mb-2 d-flex justify-content-between align-items-center">
                                                            <h6 className="fw-bold m-0 small text-uppercase ls-wide text-primary">Item Breakdown</h6>
