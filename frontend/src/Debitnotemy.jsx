...
function Debitnotemy() {
  const [debitnote, setDebitnote] = useState([]);
  const [loading, setLoading] = useState(true);

  const [selectedIds, setSelectedIds] = useState([]);
  const [selectAll, setSelectAll] = useState(false);
  const [debitnotePriv, setDebitnotePriv] = useState(null);
  ...
  const fetchDebitnote = useCallback(async () => {
    ...
    try {
      ...
      if (filters.debit_note_date)
        params.debit_note_date = filters.debit_note_date;
      if (filters.due_date) params.due_date = filters.due_date;
      if (filters.status) params.status = filters.status;

      const res = await axios.get(`${API}/debitnote`, { params });

      setDebitnote(res.data.data || []);

      // ✅ pagination fallback (same as Credit / Estimate / GRN)
      setPagination(
        res.data.pagination || {
          page: 1,
          limit: 20,
      ...
      // ✅ privilege support
      setDebitnotePriv(res.data.privileges || {});

      setSelectedIds([]);
      setSelectAll(false);
    } catch (err) {
      console.error("Fetch Debit Note Error:", err);
      alert(err.response?.data?.message || "Error loading Debit Note");
    } finally {
      setLoading(false);
    }
  ...
  const handleConvert = () => {
    if (selectedIds.length === 0) {
      alert("Please select at least one Debit Note to convert.");
      return;
    }

    const selectedNotes = debitnote.filter(d => selectedIds.includes(d.id));
    const sameCustomer = selectedNotes.every(d => d.customer_name === selectedNotes[0].customer_name);

    if (!sameCustomer) {
      alert("Please select Debit Notes from the same customer for conversion.");
      return;
    }
  ...
  const confirmConvert = async () => {
    ...
    try {
      const fullDetails = await Promise.all(
        selectedIds.map(id => axios.get(`${API}/debitnote/${id}`).then(res => res.data))
      );
      ...
      alert("Failed to fetch debit note details for conversion.");
    }
  };
  ...
  //     if (filters.mobile) params.mobile = filters.mobile;
  //     if (filters.sales_person) params.sales_person = filters.sales_person;
  //     if (filters.debit_note_date) params.debit_note_date = filters.debit_note_date;
  //     if (filters.due_date) params.due_date = filters.due_date;
  //     if (filters.status) params.status = filters.status;
  ...
  //   } catch (err) {
  //     console.error("Fetch debit note Error:", err);
  //     alert("Error loading debit note");
  //   } finally {
  //     setLoading(false);
  //   }
  ...
  const handleFilterChange = (e) => {
    ...
    if (name === "customer_name") {
      if (!value) {
        ...
        const suggestions = debitnote
          .filter((debitnote) =>
            debitnote.customer_name?.toLowerCase().includes(value.toLowerCase())
          )
          .slice(0, 5);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API}/debitnote/${id}`);
      alert("Deleted");
      fetchDebitnote();
    } catch (err) {
      console.error(err);
  ...
  const handleSelectAll = () => {
    if (!selectAll) {
      setSelectedIds(debitnote.map((debitnote) => debitnote.id));
    } else {
      setSelectedIds([]);
    }
    setSelectAll((s) => !s);
  ...
  const handleDeleteSelected = async () => {
    if (selectedIds.length === 0) {
      alert("Select at least one debit note");
      return;
    }
    if (!window.confirm("Delete selected debit note?")) return;
    try {
    ...
    <div className="container-fluid py-4 px-md-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="fw-bold text-primary mb-0">
          <i className="bi bi-file-earmark-minus-fill me-2"></i>Debit Note
        </h2>
        <div className="d-flex align-items-center gap-3">
          ...
          <div className="btn-group shadow-sm">
            {debitnotePriv?.can_add === 1 && (
              <button className="btn btn-primary d-flex align-items-center fw-bold px-4 shadow-sm" onClick={() => navigate("/debitnoteform")}>
                <i className="bi bi-plus-lg me-2"></i>New
              </button>
            )}
            {debitnotePriv?.can_delete === 1 && (
          ...
          <div className="row g-3">
            <div className="col-md-3">
              ...
              <div className="input-group">
                ...
                <input type="text" name="customer_name" className="form-control border-start-0 ps-0" placeholder="Search supplier..." value={filters.customer_name} onChange={handleFilterChange} autoComplete="off" />
              </div>
            ...
            <div className="col-md-2">
              <label className="form-label fw-bold small text-muted">Debit Note Date</label>
              <input type="date" name="debit_note_date" className="form-control" value={filters.debit_note_date} onChange={handleFilterChange} />
            </div>
            <div className="col-md-3">
        ...
        <div className="card shadow-sm border-0">
              ...
              <table className="table table-hover table-striped mb-0 align-middle" style={{ fontSize: '16px' }}>
                <thead className="table-dark small">
                  <tr>
                    ...
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("debit_note_date")}>
                      DATE {sortBy === "debit_note_date" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                    <th className="fw-bold py-3 pointer" onClick={() => toggleSort("customer_name")}>
                      SUPPLIER {sortBy === "customer_name" ? (order === "ASC" ? "▲" : "▼") : ""}
                    </th>
                ...
                <tbody>
                  {debitnote.length === 0 ? (
                    <tr><td colSpan="8" className="text-center py-5 text-muted fw-bold">No Debit Notes found.</td></tr>
                  ) : (
                    debitnote.map((d) => (
                      <tr key={d.id}>
                        ...
                        <td className="small">{d.debit_note_date}</td>
                        ...
                        <td className="text-end pe-4 no-print">
                          <div className="d-flex justify-content-end gap-2">
                            <button className="btn btn-outline-secondary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                              ...
                              onClick={() => window.open(`${API}/printdebitnote/debitnote/${d.id}`, "_blank")}>
                              <i className="bi bi-printer-fill"></i>
                            </button>
                            {debitnotePriv?.can_update === 1 && (
                              <button className="btn btn-outline-primary btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 "
                                ...
                                onClick={() => navigate(`/Editdebitnoteform/${d.id}`)}>
          ...
          <div className="card-footer bg-white py-3 border-0 d-flex justify-content-between align-items-center mb-5 no-print">
            <span className="text-muted small">
              Page {pagination?.page || 1} of {pagination?.pages || 1} &bull; Total {pagination?.total || 0} Debit Notes
            </span>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg">
              <div className="modal-header bg-primary text-white border-0">
                <h5 className="modal-title fw-bold">Convert Debit Note(s)</h5>
                <button type="button" className="btn-close btn-close-white" onClick={() => setShowConvertModal(false)}></button>
              </div>
              <div className="modal-body p-4">
                <p className="text-muted small mb-4">Select the target module to convert selected Debit Notes into. Data like customer/supplier info and items will be carried over.</p>
                <div className="mb-3">
