...
export default function EditLotJobworkFabricToPcsReturn() {
    const { id } = useParams();
        ...
        const fetchData = async () => {
            try {
                const [recRes, lotRes, empRes, procRes, fabRes, supRes] = await Promise.all([
                    axios.get(`${API}/fabric-to-pcs-return/${id}`),
                ...
                setForm({
                    ...rec,
                    return_date: rec.return_date ? new Date(rec.return_date).toISOString().substring(0, 10) : "",
    ...
    const handleHeader = (e) => {
        ...
        setForm(prev => {
            ...
            if (name === "lot_no") {
                const found = lots.find(o => o.lot_no === value);
                next.lot_name = found ? found.lot_name : "";
            }
            return next;
        });
    };
    ...
    useEffect(() => {
        ...
        setForm(prev => ({ ...prev, total_qty: dispTotalQty }));
    }, [form.rows]);

    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!form.party_name || !form.lot_no) return toast.warning("Please fill required fields");
        if (form.rows.some(r => !r.qty)) return toast.warning("Qty is required for all items");
        try {
            await axios.put(`${API}/fabric-to-pcs-return/${id}`, { ...form, items: form.rows });
            toast.success("Lot Return updated successfully");
            navigate("/lot-jobwork-fabric-to-pcs-return-list");
        } catch (err) {
            ...
            <div className="card border-0 shadow-sm rounded-4 mx-auto" style={{ maxWidth: "1200px" }}>
                <div className="card-header bg-white border-0 py-3 px-4 d-flex justify-content-between align-items-center">
                    <div className="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h5 className="fw-bold mb-0 text-danger">Edit Lot Fabric Return</h5>
                            <small className="text-muted">Return No: <span className="fw-bold text-dark">{form.return_no}</span></small>
                        </div>
                    </div>
                    <span className="badge bg-danger bg-opacity-10 text-danger px-3 py-2 rounded-pill">LOT JOBWORK</span>
                </div>
                    ...
                    <form onSubmit={handleSubmit} onKeyDown={handleKeyDown}>
                        ...
                        <div className="row g-3 mb-4 p-3 bg-light rounded-4 border mx-0">
                            <div className="col-md-3">
                                ...
                                <input type="date" className="form-control border-0 shadow-sm" name="return_date" value={form.return_date} onChange={handleHeader} required />
                            </div>
                            <div className="col-md-3">
                                <label className="form-label small fw-bold text-muted">LOT NO.</label>
                                <select className="form-select border-0 shadow-sm" name="lot_no" value={form.lot_no} onChange={handleHeader} required>
                                    <option value="">Select Lot</option>
                                    {lots.map((o, i) => <option key={i} value={o.lot_no}>{o.lot_no}</option>)}
                                </select>
                            </div>
                            <div className="col-md-3">
                                <label className="form-label small fw-bold text-muted">LOT NAME</label>
                                <input type="text" className="form-control border-0 shadow-sm bg-white" value={form.lot_name} readOnly placeholder="Auto-filled" />
                            </div>
                            ...
                            <div className="col-md-3 mt-3">
                                ...
                                <select className="form-select border-0 shadow-sm fw-semibold text-danger" name="process" value={form.process} onChange={handleHeader}>
                            ...
                            <div className="col-md-3 mt-3">
                                <label className="form-label small fw-bold text-muted">SENDER NAME</label>
                        ...
                        <div className="mt-5 d-flex gap-3 justify-content-end">
                            ...
                            <button type="submit" className="btn btn-danger rounded-pill px-5 fw-bold shadow-lg">UPDATE RETURN RECORD</button>
                        </div>
                    </form>
                </div>
            </div>
