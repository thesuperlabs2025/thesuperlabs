...
export default function EditOrderJobworkFabricToPcsInward() {
    const { id } = useParams();
    ...
    const initialRow = () => ({ counts: "", fabric_name: "", fabric_sku: "", style_name: "", gsm: "", dia: "", color: "", fabric_color: "", style_color: "", qty: "", sizes_data: {}, cut_pcs_wt: "", waste_pcs_wt: "" });
    ...
    const [activeField, setActiveField] = useState(null); // { type: 'fabric', index: 0 } or { type: 'supplier' }

    useEffect(() => {
        const fetchData = async () => {
            try {
                const [recRes, empRes, procRes, fabRes, supRes, chartRes] = await Promise.all([
                ...
                setForm({
                    ...
                    rows: (rec.items && rec.items.length) ? rec.items.map(it => ({
                        ...
                        dia: it.dia || "",
                        color: it.color || "",
                        fabric_color: it.fabric_color || it.color || "",
                        style_color: it.style_color || it.color || "",
                        qty: it.qty || "",
                        sizes_data: typeof it.sizes_data === 'string' ? JSON.parse(it.sizes_data) : (it.sizes_data || {}),
                ...
                // Fetch order specific colors if order_no exists
                if (rec.order_no) {
                    const foundOrder = (await axios.get(`${API}/yarn-dyeing-outward/orders`)).data.find(o => o.order_no === rec.order_no);
                    if (foundOrder) {
                        const colorRes = await axios.get(`${API}/size-quantity/order/${foundOrder.id}`);
                        if (colorRes.data?.items) {
                            setStyleColors([...new Set(colorRes.data.items.map(it => it.color).filter(Boolean))]);
                        }
                    }
                }
    ...
    const handleSizeQtyChange = (index, size, value) => {
        ...
        const szData = { ...(newRows[index].sizes_data || {}), [size]: value };
        const total = Object.values(szData).reduce((s, v) => s + (parseFloat(v) || 0), 0);
        newRows[index] = { ...newRows[index], sizes_data: szData, qty: total };
    ...
    const handleFabricSelect = (idx, fabricName) => {
        ...
        // Optionally auto-fill GSM/DIA if available in fabrics master
        const fab = fabrics.find(f => f.fabric_name === fabricName);
        if (fab) {
            newRows[idx].fabric_sku = fab.fabric_sku || "";
    ...
    useEffect(() => {
        const qty = form.rows.reduce((sum, row) => sum + (parseFloat(row.qty) || 0), 0);
        setForm(prev => ({ ...prev, total_qty: qty.toFixed(3) }));
    }, [form.rows]);

    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!form.party_name || !form.order_no) return toast.warning("Please fill required fields");
        try {
            await axios.put(`${API}/fabric-to-pcs-inward/${id}`, { ...form, items: form.rows });
            toast.success("Order Inward updated successfully");
            navigate("/order-jobwork-fabric-to-pcs-inward-list");
        } catch (err) {
            ...
            <div className="card border-0 shadow-sm rounded-4 mx-auto" style={{ maxWidth: "1200px" }}>
                <div className="card-header bg-white border-0 py-3 px-4 d-flex justify-content-between align-items-center">
                    <div className="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h5 className="fw-bold mb-0 text-success">Edit Order Fabric Inward</h5>
                            <small className="text-muted">Inward No: <span className="fw-bold text-dark">{form.inward_no}</span></small>
                        </div>
                    </div>
                    <span className="badge bg-success bg-opacity-10 text-success px-3 py-2 rounded-pill">ORDER JOBWORK</span>
                </div>
                ...
                <div className="card-body p-4">
                    <form onSubmit={handleSubmit} onKeyDown={handleKeyDown}>
                        ...
                        <div className="row g-3 mb-4 p-3 bg-light rounded-4 border mx-0">
                            ...
                            <div className="col-md-3">
                                <label className="form-label small fw-bold text-muted">ORDER NO.</label>
                                <input type="text" className="form-control border-0 shadow-sm bg-light" value={form.order_no} readOnly />
                            </div>
                            <div className="col-md-3">
                                <label className="form-label small fw-bold text-muted">ORDER NAME</label>
                                <input type="text" className="form-control border-0 shadow-sm bg-white" value={form.order_name} readOnly placeholder="Auto-filled" />
                            </div>
                            ...
                            {form.process?.toLowerCase() === "cutting" && (
                                <div className="col-md-3 mt-3">
                                    <label className="form-label small fw-bold text-muted">SIZE CHART</label>
                                    <select className="form-select border-0 shadow-sm" name="size_chart_name" value={form.size_chart_name} onChange={handleHeader}>
                                    ...
                                    {form.rows.map((row, idx) => {
                                            ...
                                            return (
                                                <tr key={idx}>
                                                    ...
                                                    <td className="text-center">
                                                        <button type="button" className="btn btn-sm btn-outline-danger border-0 p-0"
                                        ...
                                        return (
                                            <tr key={idx}>
                                                ...
                                                <td><input type="text" className="form-control form-control-sm border-0 bg-light" name="style_name" value={row.style_name} onChange={(e) => handleRowChange(idx, e)} placeholder="Style" /></td>
                                                ...
                                                <td>
                                                    {form.process?.toLowerCase().includes("dyeing") || form.process?.toLowerCase() === "cutting" ? (
                                                        <select className="form-select form-select-sm border-0 bg-light" name="color" value={row.color} onChange={(e) => handleRowChange(idx, e)}>
                                                            <option value="">— Color —</option>
                                                            {styleColors.map((c, i) => <option key={i} value={c}>{c}</option>)}
                                                        </select>
                                                    ) : (
                                                        <input type="text" className="form-control form-control-sm border-0 bg-light" name="color" value={row.color} onChange={(e) => handleRowChange(idx, e)} placeholder="Color" />
                                                    )}
                                                </td>
                        ...
                        <div className="d-flex justify-content-between align-items-center mb-4">
                            ...
                            <div className="d-flex gap-4">
                                <div className="text-end">
                                    ...
                                    <span className="fs-5 fw-bold text-primary">{form.total_qty} <small className="text-muted">{form.process?.toLowerCase() === "cutting" ? "PCS" : "KG"}</small></span>
